use crate::{
    context::SystemContext, errors::SystemProgramError,
    instruction_data::ZOutputCompressedAccountWithPackedContext,
    sdk::event::MerkleTreeSequenceNumber,
};
use account_compression::{
    context::AcpAccount, insert_into_queues::AppendNullifyCreateAddressInputs,
};
...
pub fn create_outputs_cpi_data<'a, 'info>(
    ...
    for (j, account) in output_compressed_accounts.iter().enumerate() {
            ...
            let pubkey = match &accounts[current_index as usize] {
                ...
                AcpAccount::StateTree((pubkey, tree)) => {
                    sequence_numbers.push(MerkleTreeSequenceNumber {
                        pubkey: *pubkey,
                        seq: tree.sequence_number() as u64,
                    });
                    hashed_merkle_tree = context.legacy_merkle_context[current_index as usize]
                        .1
                        .hashed_pubkey;
                    rollover_fee = context.legacy_merkle_context[current_index as usize]
                        .1
                        .rollover_fee;
        ...
        context.set_rollover_fee(index_merkle_tree_account - 1, rollover_fee);
    }

    cpi_ix_data.num_unique_appends = context.account_indices.len() as u8;
    Ok(hash_chain)
}
