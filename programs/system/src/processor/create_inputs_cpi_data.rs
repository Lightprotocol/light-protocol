use account_compression::{
    ...
    processor::insert_nullifiers::InsertNullifierInput,
};
...
use crate::{
    ...
    instruction_data::ZPackedCompressedAccountWithMerkleContext,
};
...
pub fn create_inputs_cpi_data<'a, 'b, 'c: 'info, 'info>(
        ...
        .enumerate()
    {
        ...
        if let Some(address) = &input_compressed_account_with_context
            ...
            .address
        {
            context.addresses[j] = Some(**address);
        }
                ...
                .merkle_tree_pubkey_index as i16
        {
            ...
            current_hashed_mt = match &accounts[current_mt_index as usize] {
                AcpAccount::OutputQueue(queue) => queue.hashed_merkle_tree_pubkey,
                AcpAccount::StateTree(_) => {
                    context.legacy_merkle_context[current_mt_index as usize]
                        .1
                        .hashed_pubkey
                }
                _ => {
