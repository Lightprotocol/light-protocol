...
impl ChangelogEventV1 {
    pub fn new<const HEIGHT: usize>(
        ...
        changelog_entries: &[Box<ChangelogEntry<HEIGHT>>],
        seq: u64,
    ) -> Result<Self> {
        ...
        for changelog_entry in changelog_entries.iter() {
            ...
            let path_len = u32::try_from(path_len).map_err(|_| ErrorCode::IntegerOverflow)?;

            // Add all nodes from the changelog path.
            for (level, node) in changelog_entry.path.iter().enumerate() {
                let level = u32::try_from(level).map_err(|_| ErrorCode::IntegerOverflow)?;
        ...
        let index: u32 = changelog_entries
            .first()
            .ok_or(ErrorCode::EventNoChangelogEntry)?
            .index
            .try_into()
...
mod test {
    ...
    fn test_changelog_event_v1() {
        ...
        for i in 0..leaves {
            let changelog_entry = Box::new(merkle_tree.changelog[i]);
            let changelog_event =
