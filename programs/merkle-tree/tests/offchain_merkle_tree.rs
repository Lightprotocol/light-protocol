...
use anchor_lang::{prelude::Pubkey, solana_program::program_pack::Pack};
use ark_crypto_primitives::{
    crh::{TwoToOneCRH, CRH},
    Error,
};
use ark_ed_on_bn254::{self, Fq, Fr};
use ark_ff::{
    bytes::{FromBytes, ToBytes},
    BigInteger, BigInteger256, Fp256, PrimeField,
};
use ark_serialize::{Read, Write};
use ark_std::{test_rng, vec::Vec, UniformRand};
use arkworks_gadgets::{
    poseidon::{circom::CircomCRH, sbox::PoseidonSbox, PoseidonError, PoseidonParameters, Rounds},
    utils::{get_mds_poseidon_circom_bn254_x5_3, get_rounds_poseidon_circom_bn254_x5_3, parse_vec},
};
...
use merkle_tree_program::{
    ...
    poseidon_merkle_tree::{
        state,
        state::MerkleTree as MerkleTreeOnchain,
        update_merkle_tree_lib::{
            instructions::insert_last_double, processor::compute_updated_merkle_tree,
            MerkleTreeUpdateState,
        },
    },
    utils::{config, config::ENCRYPTED_UTXOS_LENGTH},
};
...
use merkle_tree_program::{
    poseidon_merkle_tree::initialize_new_merkle_tree_18,
    utils::config::{MERKLE_TREE_HISTORY_SIZE, ZERO_BYTES_MERKLE_TREE_18},
};
...
fn test_initialize() {
    ...
    let mut mt = MerkleTreeOnchain {
        ...
        pubkey_locked: Pubkey::try_from([0u8; 32]).map_err(|_| ErrorCode::PubkeyTryFromFailed)?,
        time_locked: 0u64,
        height: 0u64,
        merkle_tree_nr: 0u64,
        lock_duration: 20u64,
    ...
    use ark_bn254::{Fq as FqBn, Fr as FrBn};
...
fn batch_update_smt_test() {
    ...
    for i in 0..ITERATIONS {
        ...
        let mut tmp_pda = MerkleTreeUpdateState {
            ...
            leaf_left: [0u8; 32],
            leaf_right: [0u8; 32],
            merkle_tree_pda_pubkey: Pubkey::try_from([0u8; 32])
                .map_err(|_| ErrorCode::PubkeyTryFromFailed)?,
            // verifier_tmp_pda: vec![0u8; 32],
            relayer: Pubkey::try_from([0u8; 32]).map_err(|_| ErrorCode::PubkeyTryFromFailed)?,
