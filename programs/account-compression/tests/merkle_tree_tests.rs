...
async fn test_nullify_leaves() {
    ...
    println!("root {:?}", concurrent_merkle_tree.root().unwrap());

    // println!("leaf_nodes {:?}", reference_merkle_tree.leaf_nodes[0]);
    // println!("leaf_nodes {:?}", reference_merkle_tree.leaf_nodes[1]);
    let proof: Vec<[u8; 32]> = reference_merkle_tree
        ...
        .to_vec();
    println!("proof {:?}", proof);
    println!("leaf 0 {:?}", reference_merkle_tree.leaf(0));
