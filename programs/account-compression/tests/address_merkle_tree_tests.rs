...
use light_indexed_merkle_tree::{
    array::{IndexedArray, RawIndexedElement},
    reference,
};
use light_test_utils::{create_account_instruction, get_hash_set, AccountZeroCopy};
use light_utils::bigint::bigint_to_le_bytes_array;
...
fn initialize_address_queue_ix(context: &ProgramTestContext, pubkey: Pubkey) -> Instruction {
    let instruction_data = InitializeAddressQueue {
        ...
        sequence_threshold: ADDRESS_QUEUE_SEQUENCE_THRESHOLD,
    };
...
async fn relayer_update(
    ...
    loop {
        ...
        let low_address_proof = relayer_merkle_tree
            ...
            .unwrap();
        let old_low_address: RawIndexedElement<usize, 32> = old_low_address.try_into().unwrap();
        ...
        let update_successful = match update_merkle_tree(
            ...
            bigint_to_le_bytes_array(&address.value_biguint()).unwrap(),
            address_bundle.new_element.next_index,
            bigint_to_le_bytes_array(&address_bundle.new_element_next_value).unwrap(),
            old_low_address,
            bigint_to_le_bytes_array(&old_low_address_next_value).unwrap(),
...
async fn test_address_queue() {
    ...
    let addresses: Vec<[u8; 32]> = vec![
        bigint_to_le_bytes_array(&address1).unwrap(),
        bigint_to_le_bytes_array(&address2).unwrap(),
    ];
    insert_addresses(
        &mut context,
...
async fn test_insert_invalid_low_element() {
    ...
    let addresses: Vec<[u8; 32]> = vec![bigint_to_le_bytes_array(&address3).unwrap()];
    insert_addresses(
        &mut context,
        address_queue_keypair.pubkey(),
        address_merkle_tree_keypair.pubkey(),
    ...
    assert!(update_merkle_tree(
        ...
        address_queue_keypair.pubkey(),
        address_merkle_tree_keypair.pubkey(),
        bigint_to_le_bytes_array(&address3).unwrap(),
        next_index,
        bigint_to_le_bytes_array(&next_value).unwrap(),
        low_element.try_into().unwrap(),
        bigint_to_le_bytes_array(&low_element_next_value).unwrap(),
    ...
    let addresses: Vec<[u8; 32]> = vec![bigint_to_le_bytes_array(&address4).unwrap()];
    insert_addresses(
        &mut context,
        address_queue_keypair.pubkey(),
        address_merkle_tree_keypair.pubkey(),
    ...
    assert!(update_merkle_tree(
        ...
        address_queue_keypair.pubkey(),
        address_merkle_tree_keypair.pubkey(),
        bigint_to_le_bytes_array(&address4).unwrap(),
        next_index,
        bigint_to_le_bytes_array(&next_value).unwrap(),
        low_element.try_into().unwrap(),
        bigint_to_le_bytes_array(&low_element_next_value).unwrap(),
