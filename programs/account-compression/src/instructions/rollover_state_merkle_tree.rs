...
use crate::{
    ...
    process_initialize_state_merkle_tree, state::StateMerkleTreeAccount, transfer_lamports,
    utils::check_registered_or_signer::GroupAccounts, IndexedArrayAccount, RegisteredProgram,
};

#[derive(Accounts)]
pub struct RolloverStateMerkleTreeNullifierQueuePair<'info> {
...
impl<'info> GroupAccounts<'info> for RolloverStateMerkleTreeNullifierQueuePair<'info> {
...
pub fn process_rollover_state_merkle_tree_nullifier_queue_pair<'a, 'b, 'c: 'info, 'info>(
    ...
    // check_registered_or_signer::<RolloverStateMerkleTreeNullifierQueuePair, StateMerkleTreeAccount>(
    //     &ctx,
    //     &merkle_tree,
    // )?;
    let index;
    ...
    let queue_delegate;
    {
        ...
        rollover_threshold = merkle_tree_account_loaded.rollover_threshold;
        ...
        process_initialize_state_merkle_tree(
            ...
            Some(ctx.accounts.new_nullifier_queue.key()),
            tip,
    ...
    let nullifier_queue = unsafe { indexed_array_from_bytes_zero_copy_mut(&mut nullifier_queue)? };
    process_initialize_indexed_array(
        ...
        queue_owner,
        Some(queue_delegate),
        Some(ctx.accounts.new_state_merkle_tree.key()),
