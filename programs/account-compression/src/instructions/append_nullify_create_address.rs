...
impl<'a> AppendNullifyCreateAddressInputs<'a> {
    ...
    pub fn required_size_for_capacity(
        ...
        35 + ZeroCopySliceMut::<u8, AppendLeavesInput, false>::required_size_for_capacity(
            leaves_capacity,
    ...
    pub fn new(
        ...
        let (meta, bytes) = bytes.split_at_mut(35);
...
pub fn deserialize_nullify_append_create_address_inputs<'a>(
    ...
    let (metadata, bytes) = bytes.split_at_mut(35);
...
//     // pub registered_program_pda: Option<AcpAccount<'info, RegisteredProgram>>,
...
mod test {
    ...
    #[test]
    fn heterogenous_accounts() {
        let account1 = TestAccount1 { a: 1, b: 2 };
        let account2 = TestAccount2 { c: 3, d: 4 };
        let vec = vec![
            TestAccount::Account1(account1),
            TestAccount::Account2(account2),
        ];
        do_sth(vec);
    }

    fn do_sth(vec: Vec<TestAccount>) {
        for account in vec {
            match account {
                TestAccount::Account1(account) => {
                    println!("a: {}", account.a)
                }
                TestAccount::Account2(account) => {
                    println!("c: {}", account.c)
                }
            }
        }
