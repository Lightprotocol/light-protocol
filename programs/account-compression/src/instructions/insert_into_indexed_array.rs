...
pub fn process_insert_into_indexed_arrays<'a, 'b, 'c: 'info, 'info>(
    ...
    for (mt, elements) in array_map.values() {
        ...
        // let array_account_info = array.to_account_info();
        // let hash_set =
        //     &mut array_account_info.data.borrow_mut()[8 + mem::size_of::<IndexedArrayAccount>()..];
        // let hash_set = unsafe { HashSet::<u16>::from_bytes(hash_set) };
        // let mut array_account = array.load_mut()?;
        let indexed_array = AccountLoader::<IndexedArrayAccount>::try_from(mt).unwrap();
        {
            let mut indexed_array_account = indexed_array.load_mut()?;
        ...
        for element in elements.iter() {
            ...
            let element = BigUint::from_bytes_le(element.as_slice());
            indexed_array
                .insert(&element, 0)
...
pub type IndexedArray = HashSet<u16>;
pub type IndexedArrayZeroCopy = HashSetZeroCopy<u16>;
...
pub unsafe fn indexed_array_from_bytes_copy(
    ...
    let queue = IndexedArray::from_bytes_copy(data).map_err(ProgramError::from)?;
    Ok(queue)
}
...
pub unsafe fn indexed_array_from_bytes_zero_copy_mut(
    mut data: RefMut<'_, &mut [u8]>,
) -> Result<IndexedArrayZeroCopy> {
    let data = &mut data[8 + mem::size_of::<IndexedArrayAccount>()..];
    let queue = IndexedArrayZeroCopy::from_bytes_zero_copy_mut(data).map_err(ProgramError::from)?;
    Ok(queue)
}
...
pub unsafe fn indexed_array_from_bytes_zero_copy_init(
    ...
    let queue = IndexedArrayZeroCopy::from_bytes_zero_copy_init(
        data,
        capacity_indices,
        capacity_values,
        sequence_threshold,
