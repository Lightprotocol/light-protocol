...
pub type AddressQueue = HashSet<u16>;
pub type AddressQueueZeroCopy = HashSetZeroCopy<u16>;
...
pub unsafe fn address_queue_from_bytes_copy(
    ...
    let queue = AddressQueue::from_bytes_copy(data).map_err(ProgramError::from)?;
    Ok(queue)
}
...
pub unsafe fn address_queue_from_bytes_zero_copy_mut(
    mut data: RefMut<'_, &mut [u8]>,
) -> Result<AddressQueueZeroCopy> {
    let data = &mut data[8 + mem::size_of::<AddressQueueAccount>()..];
    let queue = AddressQueueZeroCopy::from_bytes_zero_copy_mut(data).map_err(ProgramError::from)?;
    Ok(queue)
}
...
pub unsafe fn address_queue_from_bytes_zero_copy_init(
    ...
    let data = &mut data[8 + mem::size_of::<AddressQueueAccount>()..];
    let queue = AddressQueueZeroCopy::from_bytes_zero_copy_init(
        data,
        capacity_indices,
        capacity_values,
        sequence_threshold,
