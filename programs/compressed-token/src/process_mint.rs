...
pub fn process_mint_to<'info>(
    ...
    light_heap::GLOBAL_ALLOCATOR.log_total_heap("proces_mint_to: create the vec");
    ...
    light_heap::GLOBAL_ALLOCATOR
        .log_total_heap("proces_mint_to: create_output_compressed_accounts");
    #[cfg(target_os = "solana")]
    ...
    #[cfg(target_os = "solana")]
    light_heap::GLOBAL_ALLOCATOR.log_total_heap("proces_mint_to: cpi");
...
pub fn create_output_compressed_accounts(
    output_compressed_accounts: &mut Vec<CompressedAccount>,
    mint_pubkey: Pubkey,
    pubkeys: &[Pubkey],
    amounts: &[u64],
    lamports: Option<&[Option<u64>]>,
) {
    // ) -> BoundedVec<CompressedAccount> {
    // let mut output_compressed_accounts = BoundedVec::with_capacity(pubkeys.len());
    for (i, (pubkey, amount)) in pubkeys.iter().zip(amounts.iter()).enumerate() {
        ...
        // SAFETY: `hash()` allocates almost 1KB of memory for constructing the
        // Poseidon hash inputs. We need only the hash output, which is returned
        // by the function and stored on the stack.
        // TODO: `#[heap_neutral]` in `hash()` doesn't seem to have any effect,
        ...
        output_compressed_accounts[i] = account;
    }
}
...
pub fn cpi_execute_compressed_transaction_mint_to<'info>(
    ...
    let inputs_struct = InstructionDataTransfer {
        ...
        output_state_merkle_tree_account_indices: vec![0u8; output_len],
    ...
    let capacity = mem::size_of::<InstructionDataTransfer>()
        + output_len
        + output_len * (mem::size_of::<CompressedAccount>() + mem::size_of::<TokenData>())
        + inputs_struct
            .signer_seeds
            .as_ref()
            .map(|ref seeds| seeds.iter().map(|v| v.len()).sum())
            .unwrap_or(0);
    msg!("cpi: assumed capacity: {}", capacity);
    let mut inputs = Vec::with_capacity(capacity);
    InstructionDataTransfer::serialize(&inputs_struct, &mut inputs).unwrap();
    msg!("cpi: real len: {}", inputs.len());
    ...
    #[cfg(target_os = "solana")]
    light_heap::GLOBAL_ALLOCATOR.log_total_heap("cpi: 6");
    ...
    #[cfg(target_os = "solana")]
    light_heap::GLOBAL_ALLOCATOR.log_total_heap("cpi: 7");
