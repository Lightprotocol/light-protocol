...
impl CompressedMint {
    pub fn hash(&self) -> std::result::Result<[u8; 32], HasherError> {
        ...
        let hashed_mint_authority_option = if let Some(mint_authority) = self.mint_authority {
            hashed_mint_authority = hash_to_bn254_field_size_be(mint_authority.to_bytes().as_slice());
        ...
        let hashed_freeze_authority_option = if let Some(freeze_authority) = self.freeze_authority {
            hashed_freeze_authority = hash_to_bn254_field_size_be(freeze_authority.to_bytes().as_slice());
...
pub mod test {
    ...
    fn test_equivalency_of_hash_functions() {
        ...
        let hashed_spl_mint = hash_to_bn254_field_size_be(compressed_mint.spl_mint.to_bytes().as_slice());
        let mut supply_bytes = [0u8; 32];
        ...
        let hashed_mint_authority = hash_to_bn254_field_size_be(
            compressed_mint.mint_authority.unwrap().to_bytes().as_slice()
        );
        let hashed_freeze_authority = hash_to_bn254_field_size_be(
            compressed_mint.freeze_authority.unwrap().to_bytes().as_slice()
        );
    ...
    fn equivalency_of_hash_functions_rnd_iters<const ITERS: usize>() {
        ...
        for _ in 0..ITERS {
            let compressed_mint = CompressedMint {
                ...
                mint_authority: if rng.gen_bool(0.5) { Some(Pubkey::new_unique()) } else { None },
                freeze_authority: if rng.gen_bool(0.5) { Some(Pubkey::new_unique()) } else { None },
            ...
            let hashed_mint_authority;
            let hashed_mint_authority_option = if let Some(mint_authority) = compressed_mint.mint_authority {
                hashed_mint_authority = hash_to_bn254_field_size_be(mint_authority.to_bytes().as_slice());
                Some(&hashed_mint_authority)
            } else {
                None
            ...
            let hashed_freeze_authority;
            let hashed_freeze_authority_option = if let Some(freeze_authority) = compressed_mint.freeze_authority {
                hashed_freeze_authority = hash_to_bn254_field_size_be(freeze_authority.to_bytes().as_slice());
                Some(&hashed_freeze_authority)
            } else {
                None
    ...
    fn test_authority_hash_collision_prevention() {
        ...
        assert_ne!(hash1, hash3, "Hash collision between freeze-only and both authorities!");
        assert_ne!(hash2, hash3, "Hash collision between mint-only and both authorities!");
        ...
        assert_ne!(hash1, hash4, "Hash collision with different freeze authority!");
