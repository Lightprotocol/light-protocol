...
impl TokenData {
    ...
    pub fn hash_with_delegate_hashed_values(
        mint: &[u8; 32],
        owner: &[u8; 32],
        amount_bytes: &[u8; 8],
...
impl DataHasher for TokenData {
    fn hash(&self) -> std::result::Result<[u8; 32], HasherError> {
        ...
        println!("hash_inputs: {:?}", hash_inputs);
        Poseidon::hashv(
            &hash_inputs.as_slice(), // &close_authority,
        )
    }
}
...
pub mod test {
    ...
    fn equivalency_of_hash_functions() {
        ...
        let hashed_delegate =
            hash_to_bn254_field_size_be(token_data.delegate.unwrap().to_bytes().as_slice())
                .unwrap()
                .0;
        let hashed_token_data_with_hashed_values = TokenData::hash_with_delegate_hashed_values(
            &hashed_mint,
            &hashed_owner,
    ...
    #[test]
    fn equivalency_of_hash_functions_iters() {
