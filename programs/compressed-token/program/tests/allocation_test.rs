...
fn test_extension_allocation_only() {
    ...
    println!("Expected mint size with extensions: {} bytes", expected_mint_size_with_ext);
    println!("Expected mint size without extensions: {} bytes", expected_mint_size_no_ext);
    ...
    assert_eq!(cpi_instruction_struct.output_compressed_accounts.len(), 1, "Should have exactly 1 output account");
    assert_eq!(cpi_instruction_struct.input_compressed_accounts.len(), 0, "Should have no input accounts");
    ...
    if let Some(ref account_data) = output_account.compressed_account.data {
        ...
        assert!(test_mint_result.is_ok(), "Allocated space should be valid for zero-copy CompressedMint creation");
        
        println!("✅ Allocation test successful - {} bytes exactly allocated for mint with extensions", available_space);
    } else {
...
fn test_progressive_extension_sizes() {
    ...
    for (name_len, symbol_len, uri_len) in base_sizes {
        ...
        let (cpi_instruction_struct, _) = InstructionDataInvokeCpiWithReadOnly::new_zero_copy(&mut cpi_bytes[8..], config)
            .expect(&format!("CPI instruction creation should succeed for sizes: name={}, symbol={}, uri={}", name_len, symbol_len, uri_len));

        // Verify allocation correctness with zero-copy compatibility
        assert_eq!(cpi_instruction_struct.output_compressed_accounts.len(), 1, "Should have exactly 1 output account for sizes: name={}, symbol={}, uri={}", name_len, symbol_len, uri_len);
        assert_eq!(cpi_instruction_struct.input_compressed_accounts.len(), 0, "Should have no input accounts for sizes: name={}, symbol={}, uri={}", name_len, symbol_len, uri_len);
        ...
        if let Some(ref account_data) = output_account.compressed_account.data {
            ...
            panic!("Sizes name={}, symbol={}, uri={}: Output account must have data space allocated", name_len, symbol_len, uri_len);
        }
    }
}
