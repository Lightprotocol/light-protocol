...
fn test_extension_allocation_only() {
    ...
    println!("No extensions - CPI bytes length: {}", cpi_bytes_no_ext.len());
    ...
    println!("With extensions - CPI bytes length: {}", cpi_bytes_with_ext.len());
    println!("Difference: {}", cpi_bytes_with_ext.len() - cpi_bytes_no_ext.len());
...
fn test_progressive_extension_sizes() {
    ...
    for (name_len, symbol_len, uri_len) in base_sizes {
        println!("\n--- Testing sizes: name={}, symbol={}, uri={} ---", name_len, symbol_len, uri_len);
