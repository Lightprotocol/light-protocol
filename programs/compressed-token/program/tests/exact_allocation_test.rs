use light_compressed_token::{
    extensions::state::ExtensionStructConfig,
    extensions::token_metadata::{TokenMetadataConfig, MetadataConfig, AdditionalMetadataConfig},
...
fn test_exact_allocation_assertion() {
    ...
    println!("Available for instruction data: {} bytes", cpi_bytes.len() - 8);
    ...
    let compressed_account_data = output_account.compressed_account.data.as_ref()
        .expect("Should have compressed account data");
    
    let available_data_space = compressed_account_data.data.len();
    println!("Available data space in output account: {} bytes", available_data_space);
    ...
    println!("\n=== BREAKDOWN ===");
    println!("Base mint size (no extensions): {} bytes", base_mint_size_no_ext);
    ...
    println!("Margin: {} bytes", available_data_space as i32 - expected_mint_size as i32);
    ...
    for (i, config) in additional_metadata_configs.iter().enumerate() {
        println!("  item {}: key={}, value={}, total={}", i, config.key, config.value, config.key + config.value);
    }
    println!("  additional metadata total: {} bytes", total_additional_metadata_len);
    ...
    let token_metadata_size = {
        ...
        println!("Total TokenMetadata size: {} + {} = {} bytes", fixed_overhead, total_dynamic_len, size);
        size
    };
    ...
    println!("Dynamic token metadata length: {} bytes", token_metadata_size);
    println!("Expected total size: {} + {} = {} bytes", base_mint_size_no_ext, token_metadata_size, expected_total_size);
    println!("Allocated data space: {} bytes", available_data_space);
    ...
    if manual_vs_actual != 0 {
        println!("📝 Note: {} bytes difference between manual calculation and actual struct size", manual_vs_actual);
...
fn test_allocation_with_various_metadata_sizes() {
    ...
    for (i, (name_len, symbol_len, uri_len, additional_count)) in test_cases.iter().enumerate() {
        println!("\n--- Test case {} ---", i + 1);
        println!("Metadata: name={}, symbol={}, uri={}, additional={}", 
        ...
        println!("Required: {} bytes, Allocated: {} bytes, Margin: {} bytes",
                expected_mint_size, available_space, 
                available_space as i32 - expected_mint_size as i32);
        
        assert!(
            available_space >= expected_mint_size,
            "Test case {}: insufficient allocation", i + 1
        );
