    ...
    if let Some(compressions) = inputs.compressions.as_ref() {
        for compression in compressions {
            ...
            match unsafe { source_or_recipient.owner() } {
                ID => {
                    native::process_native_compressions(compression, source_or_recipient, packed_accounts)?;
                }
                SPL_TOKEN_ID => {
...
pub(crate) fn validate_compression_mode_fields(compression: &ZCompression) -> Result<(), ProgramError> {
