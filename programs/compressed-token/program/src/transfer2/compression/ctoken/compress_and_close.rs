...
pub fn process_compress_and_close(
    ...
    // Verify rent_sponsor_is_signer flag matches actual signer
    // This prevents both:
    // - Bypassing mint checks by falsely claiming rent sponsor is signer
    // - Accidentally skipping mint checks when owner is closing
    if compression_authority_is_signer != close_inputs.rent_sponsor_is_signer_flag {
        msg!(
            "rent_sponsor_is_signer mismatch: actual={}, flag={}",
            compression_authority_is_signer,
            close_inputs.rent_sponsor_is_signer_flag
        );
        return Err(ErrorCode::RentSponsorIsSignerMismatch.into());
    }
...
fn validate_compressed_token_account(
        ...
        compression_only_extension
    {
        ...
        if let Some(withheld_fee) = ctoken_withheld_fee {
            if u64::from(compression_only_extension.withheld_transfer_fee) != withheld_fee {
                msg!(
