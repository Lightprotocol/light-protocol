...
use light_ctoken_types::{
    ...
    state::{
        ...
        TokenDataVersion, ZExtensionStructMut,
    },
};
...
fn set_output_compressed_account_inner<'a, const IS_FROZEN: bool>(
    ...
    // 1. Set token account data
    {
        ...
        token_data.set(mint_pubkey, owner, amount, delegate, state)?;

        // Set TLV extension values after allocation
        if let (Some(tlv_vec), Some(exts)) = (token_data.tlv.as_mut(), tlv_data) {
