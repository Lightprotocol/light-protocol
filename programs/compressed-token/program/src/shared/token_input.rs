...
use crate::{
    extensions::check_mint_extensions, shared::owner_validation::verify_owner_or_delegate_signer,
};
...
fn set_input_compressed_account_inner<'a, const IS_FROZEN: bool>(
    ...
    // Get mint account early to check for permanent delegate
    ...
    // Check mint for permanent delegate extension (hot path = true to allow restricted extensions)
    let mint_checks = check_mint_extensions(mint_account, true)?;
    let permanent_delegate = mint_checks.permanent_delegate.as_ref();

    // Use all_accounts for signer check since permanent delegate may be outside packed_accounts
