...
use crate::{
    sdk::NOOP_PROGRAM_ID,
    utils::{get_cpi_authority_pda, get_forester_epoch_pda_address, get_forester_pda_address},
};
...
pub fn create_nullify_instruction(
    ...
    let (forester_pda, _) = get_forester_pda_address(&inputs.derivation);
    let registered_forester_pda = get_forester_epoch_pda_address(&forester_pda, epoch).0;
...
pub fn create_rollover_instruction(
    ...
    let (forester_pda, _) = get_forester_pda_address(&inputs.authority);
    let registered_forester_pda = get_forester_epoch_pda_address(&forester_pda, epoch).0;
...
pub fn create_update_address_merkle_tree_instruction(
    ...
    let register_program_pda = get_registered_program_pda(&crate::ID);
    let (forester_pda, _) = get_forester_pda_address(&instructions.authority);
    let registered_forester_pda = get_forester_epoch_pda_address(&forester_pda, epoch).0;
