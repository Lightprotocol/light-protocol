...
pub mod verifying_keypublic_psp_2_in_2_out;
pub mod verifying_keypublic_psp_8_in_2_out;
...
pub mod psp_token_compression {
    ...
    pub fn transfer2in2out<'info>(
        ctx: Context<'_, '_, '_, 'info, TransferInstruction<'info>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
        process_2in2out_transfer(ctx, inputs, None)
    }

    pub fn transfer8in2out<'info>(
        ctx: Context<'_, '_, '_, 'info, TransferInstruction<'info>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
        process_8in2out_transfer(ctx, inputs, None)
    }
