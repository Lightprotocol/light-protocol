use crate::{
    ...
    zero_copy::check_account_info_mut,
...
#[repr(C)]
#[derive(BorshSerialize, BorshDeserialize, Debug, PartialEq, Pod, Zeroable, Clone, Copy)]
...
impl BatchedMerkleTreeAccount {
    ...
    pub fn state_tree_from_account_info_mut(
        ...
        check_account_info_mut::<Self>(&ACCOUNT_COMPRESSION_PROGRAM_ID, account_info)?;
        let account_data = &mut account_info.try_borrow_mut_data()?;
        let merkle_tree = BatchedMerkleTreeAccount::state_tree_from_bytes_mut(account_data)?;
        Ok(merkle_tree)
    }
    ...
    pub fn state_tree_from_bytes_mut(
        ...
        let merkle_tree = BatchedMerkleTreeAccount::from_bytes_unchecked_mut(account_data)?;
        if merkle_tree.get_metadata().tree_type != TreeType::BatchedState as u64 {
    ...
    pub fn address_tree_from_account_info_mut(
        ...
        check_account_info_mut::<Self>(&ACCOUNT_COMPRESSION_PROGRAM_ID, account_info)?;
        let account_data = &mut account_info.try_borrow_mut_data()?;
        let merkle_tree = BatchedMerkleTreeAccount::address_tree_from_bytes_mut(account_data)?;
        Ok(merkle_tree)
    }
    ...
    pub fn address_tree_from_bytes_mut(
        ...
        let merkle_tree = BatchedMerkleTreeAccount::from_bytes_unchecked_mut(account_data)?;
        if merkle_tree.get_metadata().tree_type != TreeType::BatchedAddress as u64 {
            return Err(MerkleTreeMetadataError::InvalidTreeType.into());
        }
        Ok(merkle_tree)
    }

    fn from_bytes_unchecked_mut(
        account_data: &mut [u8],
    ) -> Result<BatchedMerkleTreeAccount, BatchedMerkleTreeError> {
        unsafe {
    ...
    pub fn init(
        ...
        unsafe {
            ...
            if account_data.len() != (*account_metadata).get_account_size()? {
                msg!("merkle_tree_account: {:?}", (*account_metadata));
...
pub fn assert_nullify_event(
    ...
    let ref_event = BatchNullifyEvent {
        ...
        root_index: (old_zero_copy_account.get_root_index() + 1)
            % old_zero_copy_account.get_metadata().root_history_capacity,
        sequence_number: old_zero_copy_account.get_metadata().sequence_number + 1,
        batch_size: old_zero_copy_account
            .get_metadata()
            .queue_metadata
            .zkp_batch_size,
    };
...
pub fn assert_batch_append_event_event(
    ...
    let ref_event = BatchAppendEvent {
        ...
        batch_size: old_zero_copy_account
            ...
            .queue_metadata
            .zkp_batch_size,
        old_next_index: old_zero_copy_account.get_metadata().next_index,
        new_next_index: old_zero_copy_account.get_metadata().next_index
