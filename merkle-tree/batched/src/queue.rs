...
use crate::zero_copy::{bytes_to_struct_unchecked, check_account_info_mut, ZeroCopyError};
...
impl BatchedQueueAccount {
    ...
    pub fn output_queue_from_account_info_mut(
        ...
        let queue = Self::from_bytes_unchecked_mut(account_data)?;
        if queue.get_metadata().metadata.queue_type != QueueType::Output as u64 {
            return Err(MerkleTreeMetadataError::InvalidQueueType.into());
        }
        Ok(queue)
    }

    pub fn from_bytes_unchecked_mut(
        account_data: &mut [u8],
    ) -> Result<BatchedQueueAccount, BatchedMerkleTreeError> {
...
pub fn assert_queue_zero_copy_inited(
    ...
    let mut zero_copy_account = BatchedQueueAccount::from_bytes_unchecked_mut(account_data)
        .expect("from_bytes_unchecked_mut failed");
    let num_batches = ref_account.batch_metadata.num_batches as usize;
    let batch_metadata = zero_copy_account.get_metadata().batch_metadata;
    let queue_type = zero_copy_account.get_metadata().metadata.queue_type;
    let next_index = zero_copy_account.get_metadata().next_index;
    assert_eq!(
        zero_copy_account.get_metadata().metadata,
