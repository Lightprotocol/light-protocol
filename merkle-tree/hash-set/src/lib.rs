    ...
    <usize as TryFrom<I>>::Error: fmt::Debug,
{
    ...
    pub unsafe fn from_bytes_copy(bytes: &mut [u8]) -> Result<Self, HashSetError> {
        ...
        println!("from_bytes_copy: values_layout: {values_layout:?}");
        // SAFETY: `I` is always a signed integer. Creating a layout for an
        // array of integers of any size won't cause any panic.
        ...
        println!("from_bytes_copy: values offset: {offset}");
    ...
    <usize as TryFrom<I>>::Error: fmt::Debug,
{
    ...
    pub unsafe fn from_bytes_zero_copy_mut(bytes: &mut [u8]) -> Result<Self, HashSetError> {
        ...
        #[cfg(feature = "solana")]
        solana_program::msg!("from_bytes_zero_copy_mut: indices offset: {}", offset);
        let indices_size_unaligned = mem::size_of::<Option<I>>() * capacity_indices;
        // Make sure that alignment of `indices` matches the alignment of `usize`.
        let indices_size = indices_size_unaligned + mem::align_of::<usize>()
            - (indices_size_unaligned % mem::align_of::<usize>());
        ...
        let offset = offset + indices_size;
        #[cfg(feature = "solana")]
        solana_program::msg!("from_bytes_zero_copy_mut: values offset: {}", offset);
        let values =
...
mod test {
    ...
    fn test_load_from_bytes() {
        ...
        assert_eq!(hs.0.capacity_indices, INDICES);
        assert_eq!(hs.0.capacity_values, VALUES);
        assert_eq!(hs.0.sequence_threshold, SEQUENCE_THRESHOLD);
        assert_eq!(unsafe { *hs.0.next_value_index }, 0);
        let mut iterator = hs.iter();
        assert_eq!(iterator.next(), None);
        for i in 0..INDICES {
            assert!(unsafe { &*hs.0.indices.as_ptr().add(i) }.is_none());
        }
        for i in 0..VALUES {
            assert!(unsafe { &*hs.0.values.as_ptr().add(i) }.is_none());
        }
