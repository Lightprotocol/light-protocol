...
use light_utils::{bigint::bigint_to_le_bytes_array, UtilsError};
...
impl HashSetCell {
    ...
    pub fn value_biguint(&self) -> BigUint {
        BigUint::from_bytes_le(self.value.as_slice())
    }
    ...
    <usize as TryFrom<I>>::Error: fmt::Debug,
{
    ...
    pub fn size_in_account(
        ...
        let indices_size = mem::size_of::<Option<I>>() * capacity_indices;
        let values_size = mem::size_of::<Option<HashSetCell>>() * capacity_values;

        Ok(dyn_fields_size + indices_size + values_size)
    }
    ...
    pub unsafe fn from_bytes_copy(bytes: &mut [u8]) -> Result<Self, HashSetError> {
        ...
        let next_value_index = usize::from_ne_bytes(bytes[16..24].try_into().unwrap());
        let sequence_threshold = usize::from_ne_bytes(bytes[24..32].try_into().unwrap());
        ...
        let offset = offset + indices_layout.size();
    ...
    pub fn insert(
        ...
        for i in 0..self.capacity_values {
            ...
            match index_bucket {
                ...
                Some(value_index) => {
                    let value_bucket = unsafe {
                        &mut *self.values.as_ptr().add(
                    ...
                    match value_bucket {
                        ...
                        Some(value_bucket) => {
                            ...
                            if let Some(element_sequence_number) = value_bucket.sequence_number {
                                if current_sequence_number >= element_sequence_number {
                                    *value_bucket = HashSetCell {
                                        value: bigint_to_le_bytes_array(value)?,
                            ...
                            if &BigUint::from_le_bytes(value_bucket.value.as_slice()) == value {
                ...
                None => {
                    ...
                    *index_bucket = Some(
                        I::try_from(self.next_value_index)
                    ...
                    if let Some(value_bucket) = value_bucket {
                        match current_sequence_number {
                            Some(current_sequence_number) => {
                                ...
                                // search for an element with a lower sequence number.
                                //
                                // If an element has a higher or equal sequence number,
                                // continue the quadratic probe.
                                if let Some(element_sequence_number) = value_bucket.sequence_number
                                {
                                    if current_sequence_number >= element_sequence_number {
                                        continue;
                            ...
                            None => {
                                ...
                                //
                                // If the sequence number is not `None`, continue the
                                // quadratic probe.
                        ...
                        let cell_value = &value_bucket.value_biguint();
                        if cell_value == value {
    ...
    pub fn by_value_index(&self, value_index: usize) -> Option<&mut HashSetCell> {
        let value_bucket = unsafe { &mut *self.values.as_ptr().add(value_index) };
        match value_bucket {
            Some(value_bucket) => Some(value_bucket),
            None => None,
        }
    }
...
{
    fn drop(&mut self) {
        // SAFETY: As long as `capacity_indices` and `capacity_values` are
        // correct, this deallocaion is safe.
        unsafe {
            // SAFETY: `I` is always a signed integer. Creating a layout for an
            // array of integers of any size won't cause any panic.
...
#[derive(Debug)]
pub struct HashSetZeroCopy<I>(pub mem::ManuallyDrop<HashSet<I>>)
where
    I: Bounded
        + CheckedAdd
        + CheckedSub
    ...
    <usize as TryFrom<I>>::Error: fmt::Debug,
{
    ...
    pub unsafe fn from_bytes_zero_copy_mut(bytes: &mut [u8]) -> Result<Self, HashSetError> {
        ...
        let offset = offset + (mem::size_of::<Option<I>>() * capacity_indices);
        let values =
            NonNull::new(bytes.as_mut_ptr().add(offset) as *mut Option<HashSetCell>).unwrap();

        Ok(Self(mem::ManuallyDrop::new(HashSet {
            capacity_indices,
            capacity_values,
            next_value_index,
            sequence_threshold,
    ...
    pub unsafe fn from_bytes_zero_copy_init(
        ...
        bytes[24..32].copy_from_slice(&sequence_threshold.to_ne_bytes());

        let hash_set = Self::from_bytes_zero_copy_mut(bytes)?;

        for i in 0..capacity_indices {
            std::ptr::write(hash_set.0.indices.as_ptr().add(i), None);
        }
        for i in 0..capacity_values {
            std::ptr::write(hash_set.0.values.as_ptr().add(i), None);
        }
    ...
    pub fn insert(&mut self, value: &BigUint, sequence_number: usize) -> Result<(), HashSetError> {
        self.0.insert(value, sequence_number)
    }

    /// Returns a first available element.
    pub fn first(&self, sequence_number: usize) -> Result<Option<&mut HashSetCell>, HashSetError> {
        self.0.first(sequence_number)
    }

    pub fn by_value_index(&self, value_index: usize) -> Option<&mut HashSetCell> {
        self.0.by_value_index(value_index)
    }

    /// Check if the hash set contains a value.
    pub fn contains(&self, value: &BigUint, sequence_number: usize) -> Result<bool, HashSetError> {
        self.0.contains(value, sequence_number)
    }
    ...
    pub fn mark_with_sequence_number(
        ...
        self.0.mark_with_sequence_number(value, sequence_number)
    }
...
impl<I> Drop for HashSetZeroCopy<I>
where
    I: Bounded
        + CheckedAdd
        + CheckedSub
...
where
    ...
    <usize as TryFrom<I>>::Error: fmt::Debug,
{
    type Item = &'a HashSetCell;

    fn next(&mut self) -> Option<Self::Item> {
        if self.current < self.hash_set.capacity_values {
            let element = unsafe { &*self.hash_set.values.as_ptr().add(self.current) };
    ...
    fn test_hash_set_random() {
        ...
        for (seq, nullifier) in nullifiers.iter().enumerate() {
            ...
            assert_eq!(
                ...
                Some(HashSetCell {
                    value: bigint_to_le_bytes_array(&nullifier).unwrap(),
                    sequence_number: None,
                })
            );
        ...
        for (i, element) in hs.iter().enumerate() {
