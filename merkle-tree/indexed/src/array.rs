...
pub struct RawIndexedElement<I, const N: usize>
where
...
impl<I> From<RawIndexedElement<I, 32>> for IndexedElement<I>
where
    I: CheckedAdd + CheckedSub + Copy + Clone + PartialOrd + ToBytes + TryFrom<usize> + Unsigned,
    usize: From<I>,
{
    fn from(element: RawIndexedElement<I, 32>) -> Self {
        let value = BigUint::from_bytes_le(element.value.as_slice());
        Self {
            index: element.index,
            value,
...
impl<I> TryFrom<IndexedElement<I>> for RawIndexedElement<I, 32>
where
    I: CheckedAdd + CheckedSub + Copy + Clone + PartialOrd + ToBytes + TryFrom<usize> + Unsigned,
    usize: From<I>,
{
    ...
    fn try_from(element: IndexedElement<I>) -> Result<Self, Self::Error> {
        let value: [u8; 32] = bigint_to_le_bytes_array(&element.value).map_err(|_| ())?;
        Ok(Self {
            index: element.index,
            value,
            next_index: element.next_index,
    ...
    usize: From<I>,
{
    ...
    {
        let hash = H::hashv(&[
            bigint_to_le_bytes_array::<32>(&self.value)?.as_ref(),
            self.next_index.to_le_bytes().as_ref(),
            next_value.to_bytes_le().as_ref(),
        ])?;
    ...
    usize: From<I>,
{
    ...
    pub fn hash_element(&self, index: I) -> Result<[u8; 32], IndexedMerkleTreeError> {
        ...
        let next_element = self
            ...
            .ok_or(IndexedMerkleTreeError::IndexHigherThanMax)?;
        let hash = H::hashv(&[
            bigint_to_le_bytes_array::<32>(&element.value)?.as_ref(),
            element.next_index.to_le_bytes().as_ref(),
            bigint_to_le_bytes_array::<32>(&next_element.value)?.as_ref(),
        ])?;
        Ok(hash)
    }
