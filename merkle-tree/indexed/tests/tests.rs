    ...
    H: Hasher,
{
    for i in 0..NR_NULLIFIERS {
        let nullifier = BigUint::from_le_bytes(nullifiers[i].as_slice());
    ...
    H: Hasher,
{
    ...
    program_insert::<H>(
        ...
        [
            bigint_to_le_bytes_array(&nullifier1).unwrap(),
            bigint_to_le_bytes_array(&nullifier2).unwrap(),
        ],
    )
    .unwrap();
    ...
    program_insert::<H>(
        onchain_queue.borrow_mut(),
        [
            bigint_to_le_bytes_array(&nullifier3).unwrap(),
            bigint_to_le_bytes_array(&nullifier4).unwrap(),
        ],
    )
    .unwrap();
    ...
    H: Hasher,
{
    ...
    let nullifier1 = 30_u32.to_biguint().unwrap();
    let nullifier1: [u8; 32] = bigint_to_le_bytes_array(&nullifier1).unwrap();
    let nullifier2 = 10_u32.to_biguint().unwrap();
    let nullifier2: [u8; 32] = bigint_to_le_bytes_array(&nullifier2).unwrap();
