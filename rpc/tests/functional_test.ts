...
const server = RPC_URL;

describe("API tests", () => {
  let lightWasm: LightWasm;
  let userKeypair = Keypair.generate();
  let provider: Provider, user: User, anchorProvider: AnchorProvider, rpc: Rpc;
  ...
  before(async () => {
    ...
    const rpc = await Rpc.initFromUrl(RPC_URL);
    ...
    await airdropSol({
      connection: anchorProvider.connection,
      lamports: 9e8,
      recipientPublicKey: provider.rpc.accounts.rpcRecipientSol,
    });
  ...
  it("initFromUrl with rpcInfo", async () => {
    rpc = await Rpc.initFromUrl(RPC_URL);
    console.log("rpc", rpc);
    assert.equal(
      rpc.accounts.rpcRecipientSol.toBase58(),
      getKeyPairFromEnv("RPC_RECIPIENT").publicKey.toBase58(),
    );
    assert.equal(
      rpc.accounts.rpcPubkey.toBase58(),
      getKeyPairFromEnv("KEY_PAIR").publicKey.toBase58(),
    );
    assert.equal(rpc.rpcFee.toString(), RPC_FEE.toString());
