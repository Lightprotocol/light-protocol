...
export async function parseReqParams(reqInstructions: any) {
  ...
  for (const instruction of reqInstructions) {
    ...
    if (
      ...
      throw new Error(
        `Rpc pubkey in instruction != rpc pubkey ${accounts[0].pubkey.toBase58()} ${rpc.accounts.rpcPubkey.toBase58()} not signer ${
          accounts[0].isSigner
  ...
  const rpcRecipientSol = accounts[5];
  console.log(
    "rpcRecipientSol",
    rpcRecipientSol.pubkey.toBase58() ===
  ...
  if (
    ...
    throw new Error(
      `Rpc recipient sol pubkey in instruction != rpc recipient sol pubkey ${rpcRecipientSol.pubkey.toBase58()} ${rpc.accounts.rpcRecipientSol.toBase58()} not signer ${
        rpcRecipientSol.isSigner
      } not writable ${rpcRecipientSol.isWritable}}`,
    );
  return instructions;
}
