...
const buildMerkleTree = function ({ connection, config, merkleTreePubkey, poseidonHash, }) {
    return __awaiter(this, void 0, void 0, function* () {
        const merkleTreeProgram = new anchor_1.Program(merkle_tree_program_1.MerkleTreeProgram, index_1.merkleTreeProgramId);
        const mtFetched = yield merkleTreeProgram.account.merkleTree.fetch(merkleTreePubkey);
        // Fetch all the accounts owned by the specified program id
        const leave_accounts = yield merkleTreeProgram.account.twoLeavesBytesPda.all();
        leave_accounts.sort((a, b) => a.account.leftLeafIndex.toNumber() - b.account.leftLeafIndex.toNumber());
        console.log(leave_accounts);
        const leaves = [];
        if (leave_accounts.length > 0) {
            for (let i = 0; i < leave_accounts.length; i++) {
                if (leave_accounts[i].account.leftLeafIndex.toNumber() <
