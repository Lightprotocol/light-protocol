...
class Keypair {
    constructor({ poseidon, seed = new anchor_1.BN(nacl.randomBytes(32)).toString("hex"), burner = false, privateKey, publicKey, }) {
        ...
        if (burner) {
            ...
            this.pubkey = Keypair.generateShieldedPublicKey(this.privkey, this.poseidon);
        }
        else if (privateKey) {
    ...
    sign(commitment, merklePath) {
        return this.poseidon.F.toString(this.poseidon([
    ...
    static generateShieldedPrivateKey(seed) {
        const privkeySeed = seed + "shielded";
        const privateKey = new anchor_1.BN(blake2b.create(b2params).update(privkeySeed).digest());
        return privateKey;
    }
