    ...
    sendTransaction(insert = true) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                try {
                    this.recipientBalancePriorTx = yield this.provider.connection.getBalance(this.recipient);
                }
                catch (e) {
                }
            }
            this.recipientFeeBalancePriorTx = yield this.provider.connection.getBalance(this.recipientFee);
            ...
            this.senderFeeBalancePriorTx = yield this.provider.connection.getBalance(this.senderFee);
            this.relayerRecipientAccountBalancePriorLastTx = yield this.provider.connection.getBalance(this.relayerRecipient);
            ...
            console.log("senderFee account: ", yield this.provider.connection.getAccountInfo(this.senderFee, "confirmed"));
            ...
            const ix = yield this.verifier.verifierProgram.methods.shieldedTransferInputs(Buffer.from(this.proofBytes), Buffer.from(this.publicInputs.publicAmount), this.publicInputs.nullifiers, this.publicInputs.leaves[0], Buffer.from(this.publicInputs.feeAmount), new anchor.BN(this.rootIndex.toString()), new anchor.BN(this.relayerFee.toString()), Buffer.from(this.encryptedUtxos.slice(0, 190)) // remaining bytes can be used once tx sizes increase
            ).accounts({
            ...
            let recentBlockhash = (yield this.provider.connection.getRecentBlockhash(("confirmed"))).blockhash;
            ...
            let lookupTableAccount = yield this.provider.connection.getAccountInfo(this.lookupTable, "confirmed");
            ...
            while (retries > 0) {
                ...
                recentBlockhash = (yield this.provider.connection.getRecentBlockhash(("confirmed"))).blockhash;
                transaction.message.recentBlockhash = recentBlockhash;
                let serializedTx = transaction.serialize();
                try {
                    console.log("serializedTx: ");
                    res = yield (0, web3_js_1.sendAndConfirmRawTransaction)(this.provider.connection, serializedTx, {
