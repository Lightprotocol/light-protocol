...
class Keypair {
    constructor(poseidon, seed = new anchor_1.BN(nacl.randomBytes(32)).toString(), index) {
        // TODO: change key derivation and write tests
        // privkey should be Sha3([ed25519Sig(),"shielded"].concat())
        if (index) {
            let encSeed = seed + "encryption";
            this.encryptionKey = (0, eth_sig_util_1.getEncryptionPublicKey)(sodium.crypto_generichash(32, sodium.from_string(encSeed)));
