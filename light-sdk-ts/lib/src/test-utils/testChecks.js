...
exports.checkNfInserted =
  exports.checkRentExemption =
  exports.checkMerkleTreeBatchUpdateSuccess =
  exports.checkMerkleTreeUpdateStateCreated =
  exports.assert_eq =
    void 0;
...
function checkMerkleTreeUpdateStateCreated({
  connection,
  ...
  return __awaiter(this, void 0, void 0, function* () {
    var merkleTreeTmpAccountInfo = yield connection.getAccountInfo(
      merkleTreeUpdateState,
      CONFIRMATION,
    );
    (0,
    chai_1.assert)(merkleTreeTmpAccountInfo.owner.toBase58() === merkleTreeProgram.programId.toBase58(), "merkle tree pda owner wrong after initializing");
    ...
    var MerkleTreeAccountInfo =
      yield merkleTreeProgram.account.merkleTree.fetch(MerkleTree);
    ...
    (0,
    chai_1.assert)(merkleTreeUpdateStateData.relayer.toBase58() == relayer.toBase58(), "The incorrect signer has been saved");
    (0,
    chai_1.assert)(merkleTreeUpdateStateData.merkleTreePdaPubkey.toBase58() == MerkleTree.toBase58(), "the incorrect merkle tree pubkey was saved");
    (0,
    chai_1.assert)(merkleTreeUpdateStateData.numberOfLeaves == leavesPdas.length, "The incorrect number of leaves was saved");
    (0,
    chai_1.assert)(merkleTreeUpdateStateData.currentInstructionIndex == current_instruction_index, "The instruction index is wrong");
    (0,
    chai_1.assert)(MerkleTreeAccountInfo.pubkeyLocked.toBase58() == merkleTreeUpdateState.toBase58());
...
function checkMerkleTreeBatchUpdateSuccess({
  ...
  return __awaiter(this, void 0, void 0, function* () {
    var merkleTreeTmpStateAccount = yield connection.getAccountInfo(
      merkleTreeUpdateState,
    );
    (0,
    chai_1.assert)(merkleTreeTmpStateAccount === null, "Shielded transaction failed merkleTreeTmpStateAccount is not closed");
    var merkleTreeAccount = yield merkleTreeProgram.account.merkleTree.fetch(
      merkle_tree_pubkey,
    );
    // Merkle tree is locked by merkleTreeUpdateState
    (0,
    chai_1.assert)(merkleTreeAccount.pubkeyLocked.toBase58() == new solana.PublicKey(new Uint8Array(32).fill(0)).toBase58());
    console.log("merkleTreeAccount.time_locked ", merkleTreeAccount.timeLocked);
    (0,
    chai_1.assert)(merkleTreeAccount.timeLocked == 0, "Lock has not been taken within prior  20 slots");
    ...
    (0,
    chai_1.assert)(merkle_tree_prior_current_root_index.add(new anchor.BN("1")).toString() === current_root_index.toString());
    ...
    console.log(
      `${merkle_tree_prior_leaves_index.add(
        new anchor.BN(numberOfLeaves),
      )} == ${merkleTreeAccount.nextIndex}`,
    );
    (0,
    chai_1.assert)(merkle_tree_prior_leaves_index.add(new anchor.BN(numberOfLeaves)).toString() == merkleTreeAccount.nextIndex.toString()); //U64.readLE(merkleTreeAccount.data.slice(594, 594 + 8), 0).toString())
...
function checkRentExemption({ connection, account }) {
  return __awaiter(this, void 0, void 0, function* () {
    let requiredBalance = yield connection.getMinimumBalanceForRentExemption(
      account.data.length,
    );
...
function checkNfInserted(pubkeys, connection) {
  return __awaiter(this, void 0, void 0, function* () {
    for (var i = 0; i < pubkeys.length; i++) {
      ...
      (0, chai_1.assert)(accountInfo == null);
    }
  });
}
