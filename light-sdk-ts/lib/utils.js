...
function hashAndTruncateToCircuit(data) {
    return new anchor_1.BN(leInt2Buff(unstringifyBigInts(ethers_1.ethers.utils.keccak256(data).toString()), 32).reverse().slice(1, 32), undefined, 'be');
}
...
function getAssetLookUpId({ connection, asset }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let mtConf = new merkleTree_1.MerkleTreeConfig({ connection, merkleTreePubkey: constants_1.MERKLE_TREE_KEY });
        let pubkey = yield mtConf.getSplPoolPda(poolType, asset);
        let registeredAssets = yield constants_1.merkleTreeProgram.account.registeredAssetPool.fetch(pubkey.pda);
