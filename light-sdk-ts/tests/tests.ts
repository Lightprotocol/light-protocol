...
describe("verifier_program", () => {
  ...
  it("Test Utxo encryption", async () => {
    ...
    assert.equal(utxo0.getCommitment()?.toString(), "7790797031264776843808539823930722966306182688678265114219628300976218020196");
    assert.equal(utxo0.getNullifier()?.toString(), "12550499222412009082001099158167607283963476261823222797328102845322566964367");
    ...
    // fromBytes
    const utxo40 = Utxo.fromBytes({poseidon,keypair: utxo4.keypair, bytes: bytes4});
    utxoEqual(utxo4, utxo40);
    // encrypt
    const encBytes4 = utxo4.encrypt();
    const utxo41 = Utxo.decrypt({poseidon, encBytes: encBytes4, keypair: utxo4.keypair});
    if(utxo41) {
