  ...
  async shield({
    ...
    if (!tokenCtx.isSol) {
      if (userTokenAccount) {
        ...
        console.log("inside the spl token");
      ...
      if (amount >= tokenBalance.amount)
        throw new Error(
          `Insufficient token balance! ${amount} bal: ${tokenBalance!.amount!}`,
        );
      ...
      if (this.provider.nodeWallet) {
        ...
        try {
          ...
          const response =
            await this.provider.browserWallet!.sendAndConfirmTransaction(
              transaction,
            );
          console.log({ response });
          // TODO: implement browserWallet support; for UI
        } catch (error) {
          throw new Error(`Error approving token transfer! ${error}`);
        }
      }
