...
describe("Merkle Tree Tests", () => {
  ...
  var provider = anchor.AnchorProvider.local(
    "http://127.0.0.1:8899",
    light_sdk_1.confirmConfig
  );
  anchor.setProvider(provider);
  const merkleTreeProgram = new anchor.Program(
    light_sdk_1.IDL_MERKLE_TREE_PROGRAM,
    light_sdk_1.merkleTreeProgramId
  );
  ...
  it.skip("Build Merkle Tree from account compression", () =>
    __awaiter(void 0, void 0, void 0, function* () {
      ...
      let newTree = yield merkleTreeProgram.account.merkleTree.fetch(
        light_sdk_1.MERKLE_TREE_KEY
      );
      chai_1.assert.equal(
  ...
  it("Initialize Merkle Tree Test", () =>
    __awaiter(void 0, void 0, void 0, function* () {
      const verifierProgramZero = new anchor.Program(
        light_sdk_1.IDL_VERIFIER_PROGRAM_ZERO,
        light_sdk_1.verifierProgramZeroProgramId
      );
      ...
      try {
        yield merkleTreeConfig.updateMerkleTreeAuthority(
          newAuthority.publicKey,
          true
        );
        console.log("Registering AUTHORITY success");
      ...
      yield merkleTreeConfig.updateMerkleTreeAuthority(
        light_sdk_1.ADMIN_AUTH_KEYPAIR.publicKey
      );
      merkleTreeConfig.payer = light_sdk_1.ADMIN_AUTH_KEYPAIR;
      // invalid signer
      merkleTreeConfig.payer = INVALID_SIGNER;
      ...
      let merkleTreeAuthority =
        yield merkleTreeProgram.account.merkleTreeAuthority.fetch(
          merkleTreeConfig.merkleTreeAuthorityPda
        );
      ...
      let registeredPoolTypePdaAccount =
        yield merkleTreeProgram.account.registeredPoolType.fetch(
      ...
      let registeredSolPdaAccount =
        yield merkleTreeProgram.account.registeredAssetPool.fetch(
          light_sdk_1.MerkleTreeConfig.getSolPoolPda(
            light_sdk_1.merkleTreeProgramId
          ).pda
        );
      chai_1.assert.equal(
      ...
      chai_1.assert.equal(
        merkleTreeAuthority1.registeredAssetIndex.toString(),
        "2"
      );
      yield merkleTreeConfig.registerVerifier(verifierProgramZero.programId);
      yield merkleTreeConfig.registerSplPool(
        light_sdk_1.POOL_TYPE,
        light_sdk_1.MINT
      );
  ...
  it("deposit ", () =>
    __awaiter(void 0, void 0, void 0, function* () {
      ...
      yield token.approve(
        provider.connection,
        light_sdk_1.ADMIN_AUTH_KEYPAIR,
        light_sdk_1.userTokenAccount,
        light_sdk_1.Transaction.getSignerAuthorityPda(
          light_sdk_1.merkleTreeProgramId,
          new light_sdk_1.VerifierZero().verifierProgram.programId
        ), //delegate
        light_sdk_1.USER_TOKEN_ACCOUNT, // owner
        depositAmount * 10,
        [light_sdk_1.USER_TOKEN_ACCOUNT]
      );
      let lightInstance = {
  ...
  it("Update Merkle Tree Test", () =>
    __awaiter(void 0, void 0, void 0, function* () {
      ...
      let leavesPdas =
        yield light_sdk_1.SolMerkleTree.getUninsertedLeavesRelayer(
          light_sdk_1.MERKLE_TREE_KEY
        );
      ...
      if (leavesPdas.length > 1) {
        ...
        (0, chai_1.assert)(
          error.error.errorCode.code == "FirstLeavesPdaIncorrectIndex"
        );
        leavesPdas.reverse();
        (0, chai_1.assert)(
          (yield connection.getAccountInfo(merkleTreeUpdateState)) == null
        );
      ...
      (0,
      chai_1.assert)((yield connection.getAccountInfo(merkleTreeUpdateState)) == null);
      error = undefined;
      // correct
      try {
        const tx1 = yield merkleTreeProgram.methods
      ...
      let maliciousSigner = yield (0, light_sdk_1.newAccountWithLamports)(
        provider.connection
      );
      ...
      console.log(error);
      (0,
      chai_1.assert)(error.logs.includes("Program log: AnchorError caused by account: authority. Error Code: InvalidAuthority. Error Number: 6016. Error Message: InvalidAuthority."));
      // Test property: 4
      // try to take lock
      try {
        const tx1 = yield merkleTreeProgram.methods
      ...
      console.log(error);
      (0,
      chai_1.assert)(error.error.errorCode.code == "MerkleTreeUpdateNotInRootInsert");
      ...
      var merkleTreeAccountPrior =
        yield merkleTreeProgram.account.merkleTree.fetch(merkle_tree_pubkey);
      let merkleTree = yield light_sdk_1.SolMerkleTree.build({
        pubkey: light_sdk_1.MERKLE_TREE_KEY,
        poseidon: POSEIDON,
      });
