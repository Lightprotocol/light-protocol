...
describe("verifier_program", () => {
    ...
    const verifierProgram = new anchor.Program(light_sdk_1.IDL_VERIFIER_PROGRAM, light_sdk_1.verifierProgramId);
    const userKeypair = new web3_js_1.Keypair();
    ...
    it("shielded transfer 1 & close", () => __awaiter(void 0, void 0, void 0, function* () {
        let [verifierState] = anchor.web3.PublicKey.findProgramAddressSync([
            light_sdk_1.ADMIN_AUTH_KEYPAIR.publicKey.toBuffer(),
            anchor.utils.bytes.utf8.encode("VERIFIER_STATE"),
        ], verifierProgram.programId);
        let balance = yield provider.connection.getBalance(verifierState, "confirmed");
        if (balance === 0) {
            yield provider.connection.confirmTransaction(yield provider.connection.requestAirdrop(verifierState, 1000000000), "confirmed");
        }
        console.log(verifierState);
        let tx0 = yield verifierProgram.methods
            .shieldedTransferFirst(msg)
            .accounts({
            signingAddress: light_sdk_1.ADMIN_AUTH_KEYPAIR.publicKey,
            systemProgram: solana.SystemProgram.programId,
            verifierState: verifierState,
        })
            .signers([light_sdk_1.ADMIN_AUTH_KEYPAIR])
            .rpc(light_sdk_1.confirmConfig);
        console.log(tx0);
        let verifierAcc = yield verifierProgram.account.verifierState.fetch(verifierState, "confirmed");
        chai_1.assert.equal(verifierAcc.msg.toString(), msg.toString());
        let tx1 = yield verifierProgram.methods
            ...
            .rpc(light_sdk_1.confirmConfig);
        console.log(tx1);
        let accountInfo = yield provider.connection.getAccountInfo(verifierState, "confirmed");
    ...
    it("(user class) shield SOL", () => __awaiter(void 0, void 0, void 0, function* () {
        let amount = 3;
        let token = "SOL";
        const provider = yield light_sdk_1.Provider.native(userKeypair);
        const user = yield light_sdk_1.User.load(provider);
        yield user.shield({ amount, token });
        // TODO: add random amount and amount checks
    }));
    it("(user class) transfer SOL", () => __awaiter(void 0, void 0, void 0, function* () {
        let amount = 1;
        let token = "SOL";
        const shieldedRecipient = "19a20668193c0143dd96983ef457404280741339b95695caddd0ad7919f2d434";
        const encryptionPublicKey = "LPx24bc92eecaf5e3904bc1f4f731a2b1e0a28adf445e800c4cff112eb7a3f5350b";
        const recipient = new anchor.BN(shieldedRecipient, "hex");
        const recipientEncryptionPublicKey = (0, light_sdk_1.strToArr)(encryptionPublicKey);
        const provider = yield light_sdk_1.Provider.native(userKeypair);
        const user = yield light_sdk_1.User.load(provider);
        yield user.transfer({
            amount,
    ...
    it("(user class) unshield SOL", () => __awaiter(void 0, void 0, void 0, function* () {
        let amount = 1;
        let token = "SOL";
        let recipient = new web3_js_1.PublicKey("E7jqevikamCMCda8yCsfNawj57FSotUZuref9MLZpWo1");
        const provider = yield light_sdk_1.Provider.native(userKeypair);
        const user = yield light_sdk_1.User.load(provider);
        yield user.unshield({ amount, token, recipient });
        // TODO: add random amount and amount checks
    }));
});
