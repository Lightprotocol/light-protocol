...
use crate::{LastTransactionWithdrawal, LastTransactionDeposit};


pub fn process_last_transaction_deposit(
    ...
    if ext_amount < 0 {
        msg!("Deposit function called with negative external amount {}", ext_amount);
        return err!(ErrorCode::NotLastTransactionState);
    }
    // Deposit
...
pub fn process_last_transaction_withdrawal(
    ...
    if verifier_state.merkle_tree_index == 0 {
        ...
        let seeds = [&[verifier_state.tx_integrity_hash.as_ref(), &b"storage"[..],  &bump_seed][..]];
        msg!("Starting cpi transfer of {} to recipient: {:?}", pub_amount_checked, ctx.accounts.recipient.to_account_info().key);
        let mut cpi_ctx = CpiContext::new_with_signer(merkle_tree_program_id, accounts, &seeds);
        ...
        let x = merkle_tree_program::cpi::withdraw_sol(cpi_ctx, amount);
    } else {
    ...
    if relayer_fee > 0 {
        ...
        if verifier_state.merkle_tree_index == 0 {
            ...
            let seeds = [&[verifier_state.tx_integrity_hash.as_ref(), &b"storage"[..],  &bump_seed][..]];
            msg!("Starting cpi transfer of {} to relayer {:?}", relayer_fee, ctx.accounts.relayer_recipient.to_account_info());
            let mut cpi_ctx = CpiContext::new_with_signer(merkle_tree_program_id, accounts, &seeds);
            cpi_ctx = cpi_ctx.with_remaining_accounts(
                vec![
