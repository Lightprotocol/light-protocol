...
describe("Tests", () => {
    ...
    it("non-inclusion proof", async () => {
        ...
        const pathEements: BN[] = inputs1utxo.merkleProofHashedIndexedElementLeaf;
        const hexPathElements = pathEements.map((value) => toHex(value.toString()));

        for (let i = 0; i < utxos.length; i++) {
            const inputs = {
                root: new Array(utxos[i]).fill(toHex(inputs1utxo.root.toString())),
                value: new Array(utxos[i]).fill(toHex(inputs1utxo.value.toString())),
                leafLowerRangeValue: new Array(utxos[i]).fill(toHex(inputs1utxo.leafLowerRangeValue.toString())),
                leafHigherRangeValue: new Array(utxos[i]).fill(toHex(inputs1utxo.leafHigherRangeValue.toString())),
                leafIndex: new Array(utxos[i]).fill(inputs1utxo.leafIndex),
                merkleProofHashedIndexedElementLeaf: new Array(utxos[i]).fill(hexPathElements),
                indexHashedIndexedElementLeaf: new Array(utxos[i]).fill(inputs1utxo.indexHashedIndexedElementLeaf),
            }
            const inputsData = JSON.stringify(inputs);
            console.time(`Proof generation for ${merkleHeights[0]} ${utxos[i]}`);
            const response = await axios.post(NON_INCLUSION_PROOF_URL, inputsData);
            console.timeEnd(`Proof generation for ${merkleHeights[0]} ${utxos[i]}`);
            assert.equal(response.status, 200);
            assert.isNotEmpty(response.data.toString());
        }
    });
