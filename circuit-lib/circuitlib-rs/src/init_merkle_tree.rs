...
pub fn non_inclusion_merkle_tree_inputs_26() -> NonInclusionMerkleProofInputs {
    ...
    let mut indexed_tree =
        IndexedMerkleTree::<Poseidon, usize, BigInteger256, HEIGHT, ROOTS>::new().unwrap();
    ...
    let new_element_next_value = bundle3.new_element_next_value;
    ...
    let merkle_proof_hashed_indexed_element_leaf =
        indexed_tree.get_proof_of_leaf(new_low_element.index);
    ...
    NonInclusionMerkleProofInputs {
        ...
        merkle_proof_hashed_indexed_element_leaf: merkle_proof_hashed_indexed_element_leaf
            .iter()
            .map(|el| BigInt::from_bytes_be(Sign::Plus, el))
            .collect(),
