...
class Player {
  ...
  static async init(provider: anchor.AnchorProvider, rpc: TestRpc | Rpc) {
    ...
    // The wallet is used to derive the seed for your compressedion keypair with a signature.
  ...
  async createGame(
    ...
    const serializationObject = {
      ...
      accountCompressionPublicKey: this.game.programUtxo.publicKey,
    };
    const utxoBytes = (
...
describe("Test rock-paper-scissors", () => {
  ...
  it.skip("Test Game Draw", async () => {
    ...
    // compress additional sol to pay for rpc fees
    await player1.user.compress({
      publicAmountSol: 10,
      token: "SOL",
    });
