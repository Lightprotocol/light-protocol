use crate::verifying_key_compressed_account_update::VERIFYINGKEY_COMPRESSED_ACCOUNT_UPDATE;
use crate::verifying_key_inclusion_proof::VERIFYINGKEY_INCLUSION_PROOF;
...
pub fn verify_inclusion_proof<'a, 'b, 'c, 'info>(
    proof: &'a Proof,
    root: [u8; 32],
    reference_value: [u8; 32],
) -> Result<()> {
    let public_inputs = [root, reference_value];
    let mut verifier = AppTransaction::<
        { VERIFYINGKEY_INCLUSION_PROOF.nr_pubinputs },
        TransactionsConfig,
    >::new(&proof, &public_inputs, &VERIFYINGKEY_INCLUSION_PROOF);
    verifier.verify()
}
