...
pub struct VerifierState {
    ...
    pub checked_public_inputs: [[u8; 32]; 6],
}

#[derive(Accounts)]
pub struct InitVoteWeightConfig<'info> {
    #[account(mut)]
    pub signing_address: Signer<'info>,
    // TODO: add further random identifier to seeds
    #[account(init, seeds = [&governance_token_mint.key().to_bytes()], bump, space= 88, payer = signing_address )]
...
pub struct VoteWeightConfig {
    ...
    // TODO: make u265
    pub current_vote_weight_number: u64,
    // add further fields here to store custom vote weight config
}

#[derive(Accounts)]
pub struct ChangeConfig<'info> {
    #[account(mut, address=vote_weight_config.load().unwrap().authority)]
    pub signing_address: Signer<'info>,
    #[account(mut, seeds = [&vote_weight_config.load().unwrap().governance_token_mint.key().to_bytes()], bump )]
    pub vote_weight_config: AccountLoader<'info, VoteWeightConfig>,
}

// TODO: add an extra instruction to claim a vote_weight_number, so that every vote weight number is unique
