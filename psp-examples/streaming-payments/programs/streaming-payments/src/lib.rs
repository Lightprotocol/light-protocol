...
pub mod streaming_payments {
    ...
    pub fn light_instruction_first<'a, 'b, 'c, 'info>(
        ctx: Context<'a, 'b, 'c, 'info, LightInstructionFirst<'info, { VERIFYINGKEY_STREAMING_PAYMENTS.nr_pubinputs } >>,
        inputs: Vec<u8>,
    ) -> Result<()> {
        ...
        let mut checked_public_inputs: [[u8; 32]; VERIFYINGKEY_STREAMING_PAYMENTS.nr_pubinputs] = [[0u8; 32]; VERIFYINGKEY_STREAMING_PAYMENTS.nr_pubinputs];
    ...
    pub fn light_instruction_second<'a, 'b, 'c, 'info>(
        ctx: Context<'a, 'b, 'c, 'info, LightInstructionSecond<'info, { VERIFYINGKEY_STREAMING_PAYMENTS.nr_pubinputs }>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
    ...
    pub fn light_instruction_third<'a, 'b, 'c, 'info>(
        ctx: Context<'a, 'b, 'c, 'info, LightInstructionThird<'info, { VERIFYINGKEY_STREAMING_PAYMENTS.nr_pubinputs }>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
    ...
    pub fn close_verifier_state<'a, 'b, 'c, 'info>(
        _ctx: Context<'a, 'b, 'c, 'info, CloseVerifierState<'info, { VERIFYINGKEY_STREAMING_PAYMENTS.nr_pubinputs }>>,
    ) -> Result<()> {
        Ok(())
    }
}
