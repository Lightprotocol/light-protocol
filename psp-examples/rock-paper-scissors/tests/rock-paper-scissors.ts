...
class Game {
  ...
  static async create(choice: Choice, gameAmount: BN, lightProvider: LightProvider) {
  ...
  getWinner(opponentChoice: Choice): {winner: Winner, isWin: BN[], isDraw: BN, isLoss: BN}  {
    ...
    if (choice === opponentChoice) {
      return {winner: Winner.DRAW, isWin: [BN_0, BN_0, BN_0], isDraw: BN_1, isLoss: BN_0};
    }
    if (choice === Choice.ROCK && opponentChoice === Choice.SCISSORS) {
      return {winner: Winner.PLAYER1, isWin: [BN_1, BN_0, BN_0], isDraw: BN_0, isLoss: BN_0};
    }
    if (choice === Choice.PAPER && opponentChoice === Choice.ROCK) {
      return {winner: Winner.PLAYER1, isWin: [BN_0, BN_1, BN_0], isDraw: BN_0, isLoss: BN_0};
    }
    if (choice === Choice.SCISSORS && opponentChoice === Choice.PAPER) {
      return {winner: Winner.PLAYER1, isWin: [BN_0, BN_0, BN_1], isDraw: BN_0, isLoss: BN_0};
    }
    return{ winner: Winner.PLAYER2, isWin: [BN_0, BN_0, BN_0], isDraw: BN_0, isLoss: BN_1};
  }
}
...
class Player {
  ...
  async execute(testProgramUtxo?: Utxo) {
    ...
    this.game.programUtxo.index = merkleTree.indexOf(this.game.programUtxo.getCommitment(this.user.provider.poseidon));
    player2ProgramUtxo.index = merkleTree.indexOf(player2ProgramUtxo.getCommitment(this.user.provider.poseidon));
    ...
    const player2OutUtxo = new Utxo({
      ...
      encryptionPublicKey: new Uint8Array(gamePdaAccountInfo.game.playerTwoProgramUtxo.accountEncryptionPublicKey),
      amounts: [amounts[1]],
      assetLookupTable: this.user.provider.lookUpTables.assetLookupTable,
      verifierProgramLookupTable: this.user.provider.lookUpTables.verifierProgramLookupTable,
