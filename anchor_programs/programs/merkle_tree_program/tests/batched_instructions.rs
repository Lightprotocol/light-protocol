...
use crate::merkle_tree_program::utils::config::ZERO_BYTES_MERKLE_TREE_18;

pub fn insert_0_double(
    merkle_tree_account: &mut MerkleTree,
    tmp_storage_account: &mut MerkleTreeTmpPda,
) -> Result<(), ProgramError> {
    panic!("todo copy next index to tmp account");
    tmp_storage_account.current_index = tmp_storage_account.next_index / 2;
    msg!(
...
pub fn insert_1_inner_loop(
    ...
    if tmp_storage_account.current_index % 2 == 0 {
        ...
        if tmp_storage_account.insert_leaves_index < tmp_storage_account.insert_leaves_count {
            // reset current_instruction_index
            tmp_storage_account.current_instruction_index = 0;
            // increase insert leaves index to insert the next leaf
            tmp_storage_account.insert_leaves_index +=1;
            // increase tmp index by pair
            tmp_storage_account.next_index += 2;
