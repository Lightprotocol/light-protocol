...
import { getConfig, Network } from './utils/v3/config';
...
describe('verifier_program', () => {
  ...
  const verifierProgram = anchor.workspace
    .VerifierProgram as Program<VerifierProgram>;
  const merkleTreeProgram = anchor.workspace
    .MerkleTreeProgram as Program<MerkleTreeProgram>;
  const attackerProgram = anchor.workspace
    .AttackerProgram as Program<AttackerProgram>;
  ...
  it.only('init pubkeys ', async () => {
    ...
    MERKLE_TREE_PDA_TOKEN = (
      await solana.PublicKey.findProgramAddress(
        [
          MERKLE_TREE_KEY.toBuffer(),
          anchor.utils.bytes.utf8.encode('MERKLE_TREE_PDA_TOKEN'),
        ],
        merkleTreeProgram.programId
      )
    )[0];
  ...
  it.only('Initialize Merkle Tree', async () => {
    ...
    if (merkleTreeAccountInfoInit == null) {
      ...
      console.log(
        'AUTHORITY: ',
        Array.prototype.slice.call(AUTHORITY.toBytes())
      );
      console.log(
        'verifierProgram.programId: ',
        Array.prototype.slice.call(verifierProgram.programId.toBytes())
      );
      console.log('MERKLE_TREE_KEY: ', MERKLE_TREE_KEY.toBase58());
      console.log(
        'MERKLE_TREE_KEY: ',
        Array.prototype.slice.call(MERKLE_TREE_KEY.toBytes())
      );
      console.log('MERKLE_TREE_PDA_TOKEN: ', MERKLE_TREE_PDA_TOKEN.toBase58());
      console.log(
        'MERKLE_TREE_PDA_TOKEN: ',
        Array.prototype.slice.call(MERKLE_TREE_PDA_TOKEN.toBytes())
      );
      try {
        const ix = await merkleTreeProgram.methods
      ...
      if (
        merkleTreeAccountInfo.owner.toBase58() !==
      ...
      assert(
        merkleTreeIndexAccountInfo != null,
        'merkleTreeIndexAccountInfo not initialized'
      );
    }
  ...
  it.only('Initialize Token Merkle tree', async () => {
    ...
    RENT_ESCROW = await provider.connection.getMinimumBalanceForRentExemption(
      256
    );
    RENT_VERIFIER = await provider.connection.getMinimumBalanceForRentExemption(
      5 * 1024
    );
    RENT_TOKEN_ACCOUNT =
      await provider.connection.getMinimumBalanceForRentExemption(
    ...
    if (merkleTreeAccountInfoInit == null) {
      ...
      try {
        ...
        let res = await solana.sendAndConfirmTransaction(
          provider.connection,
          txCreateAccount,
          [ADMIN_AUTH_KEYPAIR, solana.Keypair.fromSecretKey(MINT_PRIVATE_KEY)],
      ...
      console.log(
        'MERKLE_TREE_USDC: ',
        Array.prototype.slice.call(MERKLE_TREE_USDC.toBytes())
      );
      console.log(
        'MERKLE_TREE_PDA_TOKEN_USDC: ',
      ...
      console.log(
        'MERKLE_TREE_PDA_TOKEN_USDC: ',
        Array.prototype.slice.call(MERKLE_TREE_PDA_TOKEN_USDC.toBytes())
      );
      ...
      try {
        const tx = await merkleTreeProgram.methods
      ...
      console.log('MERKLE_TREE_USDC: ', MERKLE_TREE_USDC);
      var merkleTreeAccountInfo = await provider.connection.getAccountInfo(
        MERKLE_TREE_USDC
      );
  ...
  it.only('Init Address Lookup Table', async () => {
    ...
    let ix0 = solana.SystemProgram.transfer({
      fromPubkey: ADMIN_AUTH_KEYPAIR.publicKey,
      toPubkey: AUTHORITY,
      lamports: 1_000_000_0000,
    });
    let ix1 = solana.SystemProgram.transfer({
      fromPubkey: ADMIN_AUTH_KEYPAIR.publicKey,
      toPubkey: MERKLE_TREE_PDA_TOKEN,
      lamports: 1_000_000_0000,
    });
  ...
  it.only('Shield', async () => {
    const userClass = new UserClass({ signature: 'somesignature' });
    const user = await userClass.initUser(POSEIDON);
    console.log('user : ', user);
    const config = await getConfig(Network.LOCAL);
    ...
    /// **** setup inputs ****
    const FEE_ASSET = new anchor.BN(
      anchor.web3.SystemProgram.programId._bn.toString()
    ).mod(FIELD_SIZE);
    const ASSET = new anchor.BN(
      new anchor.web3.Account().publicKey._bn.toString()
    ).mod(FIELD_SIZE);
    let ASSET_1 = new anchor.BN(
      new anchor.web3.Account().publicKey._bn.toString()
    ).mod(FIELD_SIZE);

    const depositFeeAmount = 10_000 + Math.floor(Math.random() * 1_000_000_000);
    const depositAmount = 10_000 + Math.floor(Math.random() * 1_000_000_000);
    ...
    let deposit_utxo1 = new light.Utxo(
      user.POSEIDON,
      [FEE_ASSET, ASSET],
      [new anchor.BN(depositFeeAmount), new anchor.BN(depositAmount)],
      user.shieldedKeypair
    ); // this was KEYPAIR

    let outputUtxos = [deposit_utxo1];

    await SHIELDED_TRANSACTION.shield({
      userTokenAccount,
      outputUtxos,
      AUTHORITY,
      FEE_ASSET,
  ...
  it('Deposit', async () => {
    ...
    console.log('getMerkleTree worked');
    ...
    console.log('prepareTransactionFull worked');
  ...
  it('Withdraw', async () => {
    ...
    for (var i = 0; i < outputUtxos.length; i++) {
      SHIELDED_TRANSACTION.merkleTree.update(
    ...
    let withdrawalAmount = Math.floor(
      Math.random() * inputUtxos[0].amounts[1].toNumber()
    );
    let outUtxoAmount = inputUtxos[0].amounts[1].sub(
      new anchor.BN(withdrawalAmount)
    );
    let outUtxoAmount2 = inputUtxos[0].amounts[2];
    console.log(
      'SHIELDED_TRANSACTION.relayerFee ',
      SHIELDED_TRANSACTION.relayerFee
    );
    outputUtxos = [
      new light.Utxo(
        POSEIDON,
        [FEE_ASSET, inputUtxos[0].assets[1], inputUtxos[0].assets[2]],
        [
          outFeeAmount.sub(
            new anchor.BN(SHIELDED_TRANSACTION.relayerFee.toNumber())
          ),
          outUtxoAmount,
          outUtxoAmount2,
        ]
    ...
    await SHIELDED_TRANSACTION.prepareTransactionFull({
      ...
      assetPubkeys: [
        FEE_ASSET,
        inputUtxos[0].assets[1],
        inputUtxos[0].assets[2],
      ],
