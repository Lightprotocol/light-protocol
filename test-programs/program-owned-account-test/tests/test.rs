...
use psp_compressed_pda::compressed_account::CompressedAccountWithMerkleContext;
...
async fn test_create_pda() {
    ...
    assert_eq!(
        ...
        Err(solana_sdk::transaction::TransactionError::InstructionError(
            0,
            InstructionError::Custom(psp_compressed_pda::ErrorCode::SignerSeedsNotProvided.into())
        ))
    );
    let res = perform_create_pda(
        &mut test_indexer,
    ...
    assert_eq!(
        ...
        Err(solana_sdk::transaction::TransactionError::InstructionError(
            0,
            InstructionError::Custom(psp_compressed_pda::ErrorCode::SignerCheckFailed.into())
        ))
    );
    ...
    let compressed_token_account = test_indexer
        .compressed_accounts
        .iter()
        .find(|x| x.compressed_account.owner == psp_compressed_token::ID)
        .unwrap()
        .clone();
