...
func (gadget TwoInputsHashChain) DefineGadget(api frontend.API) interface{} {
	...
	hashChain := poseidon.Poseidon2{In1: gadget.HashesFirst[0], In2: gadget.HashesSecond[0]}.DefineGadget(api)
	for i := 1; i < len(gadget.HashesFirst); i++ {
		hashChain = poseidon.Poseidon3{In1: hashChain, In2: gadget.HashesFirst[i], In3: gadget.HashesSecond[i]}.DefineGadget(api)
	}
	return hashChain
}
...
func computeHashChain(api frontend.API, initialHash frontend.Variable, hashes []frontend.Variable) frontend.Variable {
	...
	for i := 1; i < len(hashes); i++ {
		hashChain = poseidon.Poseidon2{In1: hashChain, In2: hashes[i]}.DefineGadget(api)
	}
