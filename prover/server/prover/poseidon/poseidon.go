...
func (h halfRound) DefineGadget(api frontend.API) interface{} {
	...
	h.Inp[0] = sbox{h.Inp[0]}.DefineGadget(api).(frontend.Variable)
	return mds{h.Inp}.DefineGadget(api).([]frontend.Variable)
}
...
func (h fullRound) DefineGadget(api frontend.API) interface{} {
	...
	for i := 0; i < len(h.Inp); i += 1 {
		h.Inp[i] = sbox{h.Inp[i]}.DefineGadget(api).(frontend.Variable)
	}
	result := mds{h.Inp}.DefineGadget(api).([]frontend.Variable)
	return result
}
