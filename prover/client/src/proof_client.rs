...
impl ProofClient {
    ...
    pub async fn generate_proof(
        ...
        inputs_json: String,
        circuit_type: &str,
    ) -> Result<CompressedProof, ProverClientError> {
        let start_time = std::time::Instant::now();

        println!("Generating proof for circuit type: {}", circuit_type,);
        ...
        let response = request
            ...
            .map_err(|e| {
                ...
                ProverClientError::RpcError
            })?;

        println!(
            "Request sent to prover server: {}{}",
            self.server_address, PROVE_PATH
        );
        println!("Request body: {}", inputs_json);
        ...
        if !status_code.is_success() {
            error!(
                "[ProofClient] HTTP error: status={}, body={:?}",
        ...
        match status_code {
            reqwest::StatusCode::OK => {
                println!("Received synchronous proof response");
                self.parse_proof_from_json(&response_text)
            }
            reqwest::StatusCode::ACCEPTED => {
                println!("Received asynchronous job response");
    ...
    async fn poll_for_result(
        ...
        loop {
            if start_time.elapsed() > self.max_wait_time {
                error!(
                    "[ProofClient] poll_for_result: Timed out after {:?}",
                    self.max_wait_time
                );
                return Err(ProverClientError::RpcError);
            }
            ...
            if !status_code.is_success() {
                error!(
                    "[ProofClient] poll_for_result HTTP error: status={}, body={:?}",
                    status_code, response_text
                );
                return Err(ProverClientError::RpcError);
            }
            ...
            debug!("JobStatusResponse: {:?}", status_response);
            ...
            match status_response.status.as_str() {
                "completed" => {
                    ...
                    return if let Some(result) = status_response.result {
                        let proof_json = serde_json::to_string(&result)
                            .map_err(|_| ProverClientError::RpcError)?;
                        self.parse_proof_from_json(&proof_json)
                    } else {
                        error!("[ProofClient] poll_for_result: No result in completed job response: {:?}", status_response);
                        Err(ProverClientError::RpcError)
                    };
                }
                "failed" => {
    ...
    fn parse_proof_from_json(&self, json_str: &str) -> Result<CompressedProof, ProverClientError> {
        let proof_json = deserialize_gnark_proof_json(json_str).map_err(|e| {
            error!(
                "Failed to deserialize proof JSON: {:?}, json_str: {}",
                e, json_str
            );
            ProverClientError::RpcError
        })?;
    ...
    pub async fn generate_batch_address_append_proof(
        &self,
        inputs: crate::batch_address_append::BatchAddressAppendInputs,
    ) -> Result<(CompressedProof, [u8; 32]), ProverClientError> {
        let new_root =
            light_hasher::bigint::bigint_to_be_bytes_array::<32>(&inputs.new_root).unwrap();
        let inputs_json = crate::gnark::batch_address_append_json_formatter::to_json(&inputs);
    ...
    pub async fn generate_batch_append_proof(
        &self,
        circuit_inputs: crate::batch_append_with_proofs::BatchAppendWithProofsCircuitInputs,
    ) -> Result<(CompressedProof, [u8; 32]), ProverClientError> {
        let inputs_json = crate::gnark::batch_append_with_proofs_json_formatter::BatchAppendWithProofsInputsJson::from_inputs(&circuit_inputs).to_string();
    ...
    pub async fn generate_batch_update_proof(
        &self,
        inputs: crate::batch_update::BatchUpdateCircuitInputs,
    ) -> Result<(CompressedProof, [u8; 32]), ProverClientError> {
        let new_root = light_hasher::bigint::bigint_to_be_bytes_array::<32>(
        ...
        let json_str = crate::gnark::batch_update_json_formatter::update_inputs_string(&inputs);
