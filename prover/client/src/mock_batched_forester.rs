...
impl<const HEIGHT: usize> MockBatchedForester<HEIGHT> {
    pub async fn get_batched_append_proof(
        ...
        for i in account_next_index..account_next_index + batch_size as usize {
            ...
            match self.merkle_tree.get_proof_of_leaf(i, true) {
                Ok(proof) => merkle_proofs.push(proof.to_vec()),
                Err(e) => {
                    return Err(ProverClientError::GenericError(format!(
                        "get_proof_of_leaf({}) error: {:?}",
                        i, e
                    )));
                }
            }
        ...
        for (i, leaf) in leaves.iter().enumerate() {
            if old_leaves[i] == [0u8; 32] {
                ...
                if let Err(e) = self.merkle_tree.update(leaf, index) {
                    println!(
                        "[get_batched_append_proof] update({:?}, {}) error: {:?}",
                        leaf, index, e
                    );
                    return Err(ProverClientError::GenericError(format!(
                        "update({:?}, {}) error: {:?}",
        ...
        if computed_new_root != self.merkle_tree.root() {
            return Err(ProverClientError::GenericError(
                "computed new_root mismatch".to_string(),
            ));
        }
...
impl<const HEIGHT: usize> MockBatchedAddressForester<HEIGHT> {
    pub async fn get_batched_address_proof(
        ...
        if batch_start_index != self.merkle_tree.merkle_tree.rightmost_index {
            return Err(ProverClientError::GenericError(format!(
                "batch_start_index {} != rightmost_index {}",
                batch_start_index, self.merkle_tree.merkle_tree.rightmost_index
            )));
        }
        if batch_start_index < 1 {
            return Err(ProverClientError::GenericError(
                "start index should be greater than 2 else tree is not inited".to_string(),
            ));
        }
        ...
        for new_element_value in &new_element_values {
            let non_inclusion_proof = match self
                .merkle_tree
