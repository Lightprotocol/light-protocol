    ...
    if let Fields::Named(ref mut fields) = anchor_accounts_strct.fields {
        ...
        for field in fields.named.iter() {
            ...
            if light_account {
                ...
                existing_field_names.push(field.ident.as_ref().unwrap().to_string());
            }
        }
    ...
    for (i, field) in fields.named.iter().enumerate() {
        ...
        let try_from_slice_call = match account_args.action {
            LightAccountAction::Init => quote! {
                let #field_ident: #type_path = #type_path_without_args::new_init(
                    &#seeds,
                    &crate::ID,
                    &merkle_context,
            ...
            LightAccountAction::Mut => quote! {
                let #field_ident: #type_path = #type_path_without_args::try_from_slice_mut(
                    inputs[#i].as_slice(),
                    &#seeds,
                    &crate::ID,
                    &merkle_context,
            ...
            LightAccountAction::Close => quote! {
                let #field_ident: #type_path = #type_path_without_args::try_from_slice_close(
                    inputs[#i].as_slice(),
                    &#seeds,
                    &crate::ID,
                    &merkle_context,
