...
impl<const INDEXED_ARRAY_SIZE: usize, R: RpcConnection + Send + Sync + 'static>
    Indexer<INDEXED_ARRAY_SIZE, R> for TestIndexer<INDEXED_ARRAY_SIZE, R>
{
    ...
    async fn get_multiple_new_address_proofs(
        ...
        debug!("merkle_tree_pubkey: {:?}", merkle_tree_pubkey);
        debug!("address: {:?}", address);
        debug!("low_address_index: {:?}", low_address_index);
        debug!("low_address_value: {:?}", low_address_value);
        debug!("low_address_next_index: {:?}", low_address_next_index);
        debug!("low_address_next_value: {:?}", low_address_next_value);
        debug!("low_address_proof: {:?}", low_address_proof);

        info!(
            "root_seq: {:?}",
            address_tree_bundle.merkle_tree.merkle_tree.sequence_number as u64
        );
    ...
    fn get_address_merkle_trees(&self) -> &Vec<AddressMerkleTreeBundle<INDEXED_ARRAY_SIZE>> {
        &self.address_merkle_trees
    }

    fn get_address_merkle_trees_mut(
        &mut self,
    ) -> &mut Vec<AddressMerkleTreeBundle<INDEXED_ARRAY_SIZE>> {
...
impl<const INDEXED_ARRAY_SIZE: usize, R: RpcConnection> TestIndexer<INDEXED_ARRAY_SIZE, R> {
    ...
    pub async fn new(
        ...
        let nullified_compressed_accounts = Vec::new();
