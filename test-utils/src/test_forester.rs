...
use light_registry::utils::{get_forester_epoch_pda_address, get_forester_pda_address};
...
pub async fn nullify_compressed_accounts<R: RpcConnection>(
    ...
    assert_forester_counter(
        rpc,
        &get_forester_epoch_pda_address(&forester_pda, epoch).0,
    ...
    loop {
        ...
        if update_successful {
            if !signer_is_owner {
                let forester_pda = get_forester_pda_address(&forester.pubkey()).0;
                let forester_epoch_pda = get_forester_epoch_pda_address(&forester_pda, epoch).0;
                assert_forester_counter(rpc, &forester_epoch_pda, pre_forester_counter, 1)
                    .await
                    .unwrap();
            }
            let merkle_tree =
