...
pub fn hash_64_to_vec(input_bytes: Vec<u8>) -> [u8; 32] {
    ...
    if input_bytes.len() == 64 {
        let input_for_sponge_1 =  <Fp256::<ark_ed_on_bls12_381::FqParameters> as FromBytes>::read(&input_bytes[..32]).unwrap();
        let input_for_sponge_2 =  <Fp256::<ark_ed_on_bls12_381::FqParameters> as FromBytes>::read(&input_bytes[32..]).unwrap();
        sponge.absorb(&input_for_sponge_1);
        sponge.absorb(&input_for_sponge_2);
    ...
    <Fp256::<ark_ed_on_bls12_381::FqParameters> as ToBytes>::write(&hash[0], &mut hash_bytes[..]);
...
impl Path {
    ...
    pub fn convert_path_to_circuit_ready_merkle_proof(
        ...
        root_hash: &Vec<u8>,
        claimed_leaf_hash: &Vec<u8>,
    ) -> Result<Vec<(bool, ark_ed_on_bls12_381::Fq)>, Error> {
        // calculate leaf hash assumed to be calculated already
        //let claimed_leaf_hash = P::LeafHash::evaluate(&leaf_hash_params, &ark_ff::to_bytes!(&leaf)?)?;
        // check hash along the path from bottom to root
        //println!("Self: {:?}", self.clone())
        let mut res: Vec<(bool, ark_ed_on_bls12_381::Fq)> = Vec::new();
        ...
        //println!("left: {:?}", left_bytes);
        //println!("right: {:?}", right_bytes);
        res.push((sibling_is_left_bool, <Fp256::<ark_ed_on_bls12_381::FqParameters>  as FromBytes>::read(&self.leaf_sibling_hash[..]).unwrap()));
        let mut curr_path_node = hash_64_to_vec([&left_bytes[..], &right_bytes[..]].concat()).to_vec();
        //println!("curr_path_node: {:?}", curr_path_node);
        ...
        for level in (0..self.auth_path.len()).rev() {
            ...
            let mut tmp = [0u8;32];
            for i in 0..32 {
                tmp[i] = self.auth_path[level][i];
            }
            res.push((sibling_is_left_bool, <Fp256::<ark_ed_on_bls12_381::FqParameters>  as FromBytes>::read(&tmp[..]).unwrap()));
            //println!("curr_path_node: {:?}", curr_path_node);
        }
