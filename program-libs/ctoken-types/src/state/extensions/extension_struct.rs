...
pub enum ZExtensionStructMut<'a> {
    ...
    /// Marker extension indicating the account belongs to a mint with permanent delegate
    PermanentDelegateAccount(ZPermanentDelegateAccountExtensionMut),
    /// Transfer fee extension storing withheld fees from transfers
    ...
    fn byte_len(
        ...
        Ok(match config {
            ...
            ExtensionStructConfig::CompressedOnly(config) => {
                // TODO: throw error CompressedOnly is an extension for compressed token accounts not ctoken accounts.
                // 1 byte for discriminant + 8 bytes for u64 delegated_amount
                1 + CompressedOnlyExtension::byte_len(config)?
            }
            _ => {
