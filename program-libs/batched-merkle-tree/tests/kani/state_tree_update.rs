...
fn verify_state_tree_mixed_random() {
    ...
    for i in (0..10u8) {
        ...
        kani::cover!(
            result.is_ok(),
            "update_tree_from_output_queue_account succeeded 0"
        );
        ...
        kani::cover!(result.is_ok(), "update_tree_from_input_queue succeeded 0");
        ...
        kani::cover!(
            result.is_ok(),
            "update_tree_from_output_queue_account succeeded 1"
        );
    }
    ...
    //     let new_root: [u8; 32] = [i + 13; 32];
    //     let selector: bool = kani::any();
    //     if selector {
    //         setup_output_queue_zkp_batches(&mut queue, 1);
    //         kani::cover!(
    //             selector,
    //             "pre update_tree_from_output_queue_account succeeded 1"
    //         );
    //         let result = tree.update_tree_from_output_queue_account(
    //             &mut queue,
    //             InstructionDataBatchAppendInputs {
    ...
    //         kani::cover!(result.is_ok(), "update_tree_from_input_queue succeeded 1");
    //     }
    // }
}
