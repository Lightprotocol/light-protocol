...
pub fn assert_input_queue_insert(
    ...
    for (i, insert_value) in bloom_filter_insert_values.iter().enumerate() {
        ...
        let pre_hashchain = pre_hash_chains.get_mut(inserted_batch_index).unwrap();
...
async fn test_simulate_transactions() {
    ...
    for tx in 0..num_tx {
        ...
        println!("num_output_updates: {}", num_output_updates);
        {
            ...
            let pre_output_hashchains = pre_output_account.hash_chain_stores;
            ...
            let pre_roots = pre_merkle_tree_account
                ...
                .cloned()
                .collect();
            let pre_mt_hashchains = pre_merkle_tree_account.hash_chain_stores;
        ...
        if in_ready_for_update && rng.gen_bool(1.0) {
            ...
            let (input_res, new_root) = {
                ...
                let next_full_batch = account.get_metadata().queue_metadata.pending_batch_index;
                let batch = account
                    .queue_metadata
                    .batches
                ...
                println!("hashchain store len {:?}", account.hash_chain_stores.len());
                println!(
...
fn assert_merkle_tree_update(
    ...
    if let Some(mut old_queue_account) = old_queue_account {
        ...
        // Input queue update
        let old_full_batch_index = old_account.queue_metadata.pending_batch_index;
        ...
        if old_full_batch.get_state() == BatchState::Inserted {
            old_account.queue_metadata.pending_batch_index += 1;
            old_account.queue_metadata.pending_batch_index %= 2;
        }
        let old_full_batch_index = old_account.queue_metadata.pending_batch_index;
