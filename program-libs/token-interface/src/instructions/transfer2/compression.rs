...
impl ZCompression<'_> {
    pub fn get_rent_sponsor_index(&self) -> Result<u8, TokenError> {
        match self.mode {
            ZCompressionMode::CompressAndClose => Ok(self.pool_account_index),
            _ => Err(TokenError::InvalidCompressionMode),
        }
    }
    pub fn get_compressed_token_account_index(&self) -> Result<u8, TokenError> {
        match self.mode {
            ZCompressionMode::CompressAndClose => Ok(self.pool_index),
            _ => Err(TokenError::InvalidCompressionMode),
        }
    }
    pub fn get_destination_index(&self) -> Result<u8, TokenError> {
        match self.mode {
...
impl Compression {
    ...
    pub fn compress_light_token(amount: u64, mint: u8, source: u8, authority: u8) -> Self {
        Compression {
            amount,
            mode: CompressionMode::Compress,
            mint,
    ...
    pub fn decompress_light_token(amount: u64, mint: u8, recipient: u8) -> Self {
        Compression {
            amount,
            mode: CompressionMode::Decompress,
            mint,
...
impl ZCompression<'_> {
    pub fn new_balance_compressed_account(&self, current_balance: u64) -> Result<u64, TokenError> {
        let new_balance = match self.mode {
            ...
            ZCompressionMode::Decompress => {
                ...
                current_balance
                    ...
                    .ok_or(TokenError::CompressInsufficientFunds)
            }
        }?;
        Ok(new_balance)
    }
