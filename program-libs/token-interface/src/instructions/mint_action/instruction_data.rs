...
use super::{
    CpiContext, CreateSplMintAction, MintToAction, MintToCompressedAction, RemoveMetadataKeyAction,
...
pub enum Action {
    ...
    /// Mint tokens from a cmint to a token solana account
            ...
            Some(exts) => {
                let converted_exts: Result<Vec<_>, Self::Error> = exts
                    ...
                    .map(|ext| match ext {
                        ...
                        _ => {
                            Err(TokenError::UnsupportedExtension)
                        }
                    })
                    .collect();
                Some(converted_exts?)
