...
use crate::{
    state::{
        CompressionInfoConfig, ExtensionStruct, ExtensionStructConfig, Token, ZExtensionStruct,
        ZExtensionStructMut,
    },
...
impl<'a> ZeroCopyAt<'a> for TokenMeta {
    type ZeroCopyAt = ZTokenMeta<'a>;
...
impl<'a> ZeroCopyAtMut<'a> for TokenMeta {
    type ZeroCopyAtMut = ZTokenMetaMut<'a>;
...
impl<'a> Deref for ZToken<'a> {
    type Target = <TokenMeta as ZeroCopyAt<'a>>::ZeroCopyAt;
...
pub struct ZTokenMut<'a> {
    __meta: <TokenMeta as ZeroCopyAtMut<'a>>::ZeroCopyAtMut,
    /// Extensions for the token account (including compressible config)
    pub extensions: Option<Vec<ZExtensionStructMut<'a>>>,
}
impl<'a> Deref for ZTokenMut<'a> {
    type Target = <TokenMeta as ZeroCopyAtMut<'a>>::ZeroCopyAtMut;
...
impl<'a> ZeroCopyAt<'a> for Token {
    ...
    #[profile]
    fn zero_copy_at(bytes: &'a [u8]) -> Result<(Self::ZeroCopyAt, &'a [u8]), ZeroCopyError> {
        let (__meta, bytes) = <TokenMeta as ZeroCopyAt<'a>>::zero_copy_at(bytes)?;
        ...
        Ok((ZToken { __meta, extensions }, bytes))
    }
}
...
impl Token {
    ...
    pub fn zero_copy_at_mut_checked(
        ...
        Ok(Token::zero_copy_at_mut(bytes)?)
    }
}
...
impl<'a> ZeroCopyAtMut<'a> for Token {
    ...
    fn zero_copy_at_mut(
        ...
        let (__meta, bytes) = <TokenMeta as ZeroCopyAtMut<'a>>::zero_copy_at_mut(bytes)?;
        let (extensions, bytes) = if !bytes.is_empty() {
            // Check if first byte is AccountType::Account (value 2) for SPL Token 2022 compatibility
            let extension_start = if bytes.first() == Some(&2) {
                // Skip AccountType::Account byte at position 165
        ...
        Ok((ZTokenMut { __meta, extensions }, bytes))
    }
}

impl ZTokenMetaMut<'_> {
...
impl<'a> ZeroCopyNew<'a> for Token {
    ...
    fn new_zero_copy(
        ...
        if bytes.len() < Self::byte_len(&config)? {
            msg!("Token new_zero_copy Insufficient buffer size");
