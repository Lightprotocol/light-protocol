...
use light_token_interface::state::token::{Token, TokenConfig};
...
fn test_compressed_token_new_zero_copy() {
    ...
    let (compressed_token, remaining_bytes) =
        Token::new_zero_copy(&mut buffer, config).expect("Failed to initialize compressed token");
...
fn test_compressed_token_new_zero_copy_with_delegate() {
    ...
    let (compressed_token, _) = Token::new_zero_copy(&mut buffer, config)
