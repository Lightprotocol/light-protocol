...
use light_token_interface::state::{
    token::{Token, TokenConfig, ZToken},
...
fn compare_compressed_token_mut_with_pod_account(
    compressed_token: &light_token_interface::state::token::ZTokenMut,
    pod_account: &PodAccount,
) -> bool {
    // Extensions should be None for basic SPL Token accounts
    if compressed_token.extensions.is_some() {
...
fn test_compressed_token_equivalent_to_pod_account() {
    ...
    for _ in 0..10000 {
        ...
        ));
        {
            ...
            let (mut compressed_token_mut, _) = Token::zero_copy_at_mut(&mut account_data).unwrap();
            ...
            let (modified_compressed_token, _) =
                Token::zero_copy_at(&modified_account_data).unwrap();
...
fn test_compressed_token_with_compressible_extension() {
    ...
    {
        let (compressed_token, remaining_bytes) = Token::new_zero_copy(&mut buffer, config)
    ...
    let (deserialized_token, _) = Token::zero_copy_at(&buffer)
...
fn test_account_type_compatibility_with_spl_parsing() {
    ...
    let (_compressed_token, _) =
        Token::new_zero_copy(&mut buffer, config).expect("Failed to create token with extension");
...
/// Test PartialEq between ZToken and Token with Compressible extension.
...
fn test_compressible_extension_partial_eq() {
    ...
    use light_token_interface::state::{
        extensions::ExtensionStruct, token::AccountState as TokenAccountState,
    };
    ...
    let mut buffer = vec![0u8; Token::byte_len(&config).unwrap()];
    {
        let (mut zctoken_mut, _) = Token::new_zero_copy(&mut buffer, config).unwrap();

        // Set extension fields
        if let Some(ref mut exts) = zctoken_mut.extensions {
            for ext in exts.iter_mut() {
                if let light_token_interface::state::extensions::ZExtensionStructMut::Compressible(
                    ref mut comp,
                ) = ext
                {
    ...
    // Create owned Token with matching values (rent_config is zeroed in the buffer)
    ...
    let ctoken_diff_compress = Token {
