...
use light_token_interface::state::{
    ...
    mint_top_up_lamports_from_slice, BaseMint, Mint, MintConfig, MintMetadata, ACCOUNT_TYPE_MINT,
};
...
/// Generate a random Mint for testing
fn generate_random_compressed_mint(rng: &mut impl Rng, with_extensions: bool) -> Mint {
...
/// Test that Mint borsh serialization and zero-copy representations are compatible
#[test]
fn test_compressed_mint_borsh_zerocopy_compatibility() {
    ...
    for i in 0..100 {
        ...
        let config = MintConfig { extensions: None };
        let byte_len = Mint::byte_len(&config).unwrap();
        let mut zero_copy_bytes = vec![0u8; byte_len];
        let (mut zc_mint, _) = Mint::new_zero_copy(&mut zero_copy_bytes, config)
            .unwrap_or_else(|_| panic!("Failed to create zero-copy Mint at iteration {}", i));
        ...
        zc_mint.base.metadata.mint_decompressed = if original_mint.metadata.mint_decompressed {
            1
        } else {
            0
        };
        ...
        let zc_as_borsh = Mint::deserialize(&mut zero_copy_bytes.as_slice()).unwrap_or_else(|_| {
            panic!(
        ...
        // Test zero-copy read
        let (zc_read, _) = Mint::zero_copy_at(&zero_copy_bytes)
            .unwrap_or_else(|_| panic!("Failed to read zero-copy Mint at iteration {}", i));
        ...
        assert_eq!(
            original_mint.metadata.mint_decompressed,
            zc_read.base.metadata.mint_decompressed != 0,
...
/// Test edge cases for Mint serialization
#[test]
fn test_compressed_mint_edge_cases() {
    // Test with no authorities
    let mint_no_auth = Mint {
        base: BaseMint {
            mint_authority: None,
            supply: u64::MAX,
            decimals: 0,
    ...
    let (mut zc_mint, _) = Mint::new_zero_copy(&mut zc_bytes, config).unwrap();
    ...
    zc_mint.base.metadata.mint_decompressed = 0;
    ...
    let zc_as_borsh = Mint::deserialize(&mut zc_bytes.as_slice()).unwrap();
...
/// Test that BaseMint within Mint maintains SPL compatibility format
#[test]
fn test_base_mint_in_compressed_mint_spl_format() {
    ...
    assert!(
        full_bytes.len() >= 82,
        "Serialized Mint should be at least 82 bytes"
    );
...
fn test_compressed_mint_new_zero_copy_fails_if_already_initialized() {
    ...
    let _ = Mint::new_zero_copy(&mut buffer, config.clone()).expect("First init should succeed");

    // Second initialization should fail because account is already initialized
    let result = Mint::new_zero_copy(&mut buffer, config);
    assert!(
        result.is_err(),
...
/// Test that mint_top_up_lamports_from_slice produces identical results to full deserialization.
#[test]
fn test_mint_top_up_lamports_matches_full_deserialization() {
    // Create a Mint using zero-copy
    let config = MintConfig { extensions: None };
    let byte_len = Mint::byte_len(&config).unwrap();
    let mut buffer = vec![0u8; byte_len];
    let (mut mint, _) = Mint::new_zero_copy(&mut buffer, config).unwrap();

    // Set known values in CompressionInfo
    mint.base.compression.lamports_per_write = 1000.into();
    mint.base.compression.last_claimed_slot = 13500.into(); // Epoch 1
    mint.base.compression.rent_exemption_paid = 50_000.into();
    mint.base.compression.rent_config.base_rent = 128.into();
    mint.base.compression.rent_config.compression_cost = 11000.into();
    mint.base
        .compression
        .rent_config
        .lamports_per_byte_per_epoch = 1;
    mint.base.compression.rent_config.max_funded_epochs = 2;
    ...
    let optimized_result = mint_top_up_lamports_from_slice(&buffer, current_lamports, current_slot)
        .expect("Should return Some");

    // Calculate using full deserialization
    let (mint_read, _) = Mint::zero_copy_at(&buffer).unwrap();
    let full_deser_result = mint_read
        .base
        .compression
