...
impl<'a, T: ZeroCopyStructInner + Deserialize<'a>> ZeroCopyStructInner for Vec<T> {
    type ZeroCopyInner = Vec<T::ZeroCopyInner>;
}

impl<T: ZeroCopyStructInner> ZeroCopyStructInner for Option<T> {
...
mod test {
    ...
    // Tests for derived structures (with ZeroCopy proc macro)

    #[repr(C)]
    #[derive(Debug, PartialEq, BorshSerialize, BorshDeserialize, zero_copy_derive::ZeroCopy)]
    ...
    fn test_struct_1_derived() {
        let bytes = Struct1Derived { a: 1, b: 2 }.try_to_vec().unwrap();
