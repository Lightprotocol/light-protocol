...
//! - CToken base struct is 165 bytes (SPL-compatible)
...
/// Create a test CToken with Compressible extension
fn create_test_ctoken_with_extension() -> CToken {
    CToken {
...
/// Create a simple CToken without extensions (SPL-compatible 165 bytes)
fn create_test_ctoken_simple() -> CToken {
    CToken {
        mint: Pubkey::new_from_array([1; 32]),
        owner: Pubkey::new_from_array([2; 32]),
        amount: 1000,
...
fn test_account_type_byte_position() {
    ...
    // CToken with extensions has account_type byte at position 165
    let ctoken = create_test_ctoken_with_extension();
    let ctoken_bytes = ctoken.try_to_vec().unwrap();
    assert_eq!(
        ctoken_bytes[ACCOUNT_TYPE_OFFSET], 2,
        "CToken with extensions account_type should be 2"
    );
}

#[test]
fn test_ctoken_without_extensions_size() {
    // CToken without extensions should be exactly 165 bytes (SPL Token Account size)
    let ctoken = create_test_ctoken_simple();
    let ctoken_bytes = ctoken.try_to_vec().unwrap();
    assert_eq!(
        ctoken_bytes.len(),
        165,
        "CToken without extensions should be 165 bytes"
    );
}
