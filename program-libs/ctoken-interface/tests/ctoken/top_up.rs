...
use light_ctoken_interface::state::{
    top_up_lamports_from_slice, CToken, CompressedTokenConfig, CompressibleExtensionConfig,
...
fn test_top_up_lamports_matches_full_deserialization() {
    // Create a CToken with Compressible extension
    ...
    let (mut ctoken, _) = CToken::new_zero_copy(&mut buffer, config).unwrap();

    // Set known values in CompressionInfo via zero-copy
    let ext = ctoken.extensions.as_mut().unwrap();
    let compressible = ext
        .iter_mut()
        .find_map(|e| match e {
            light_ctoken_interface::state::ZExtensionStructMut::Compressible(c) => Some(c),
            _ => None,
        })
        .unwrap();
    ...
    let (ctoken_read, _) = CToken::zero_copy_at(&buffer).unwrap();
    let compressible_read = ctoken_read
        ...
        .iter()
        .find_map(|e| match e {
            light_ctoken_interface::state::ZExtensionStruct::Compressible(c) => Some(c),
            _ => None,
        })
        .unwrap();
