...
fn test_byte_len_calculation() {
    ...
    println!("SimpleVecStruct byte_len: calculated={}, expected={}", calculated_size, expected_size);
    ...
    println!("SimpleOptionStruct (Some) byte_len: calculated={}, expected={}", calculated_size_some, expected_size_some);
    
    let config_none = SimpleOptionStructConfig {
        option: false, // Option<u64> disabled
    };
    
    let expected_size_none = 
        1 +  // a: u8 (meta field)
    ...
    println!("MixedStruct byte_len: calculated={}, expected={}", calculated_size_mixed, expected_size_mixed);
...
fn test_dynamic_buffer_allocation_with_byte_len() {
    ...
    assert_eq!(remaining.len(), 0, "Should have used exactly the calculated number of bytes");
