...
fn main() {
    ...
    let (mut struct_copy_mut, _remaining) = VeryLongFieldNames::new_zero_copy(&mut new_bytes, config).unwrap();
    // set field values
    struct_copy_mut.this_is_an_extremely_long_field_name_that_tests_the_limits_of_identifier_processing = 12345.into();
    struct_copy_mut.another_very_long_field_name_with_many_underscores_and_words_combined_together[0] = 1;
    struct_copy_mut.another_very_long_field_name_with_many_underscores_and_words_combined_together[1] = 2;
    struct_copy_mut.another_very_long_field_name_with_many_underscores_and_words_combined_together[2] = 3;
    if let Some(ref mut val) = struct_copy_mut.yet_another_ridiculously_long_field_name_to_ensure_macro_handles_long_identifiers {
        **val = 98765.into();
    }
