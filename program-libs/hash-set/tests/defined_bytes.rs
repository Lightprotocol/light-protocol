...
fn zero_copy_rejects_buffer_without_reserved_gap() {
    ...
    let mut bytes = vec![0xAA; expected_size - 8];
    let before = bytes.clone();
    let error = unsafe { HashSetZeroCopy::from_bytes_zero_copy_init(&mut bytes, CAPACITY, 10) }
        .unwrap_err();
    assert!(
        matches!(error, HashSetError::BufferSize(expected, actual) if expected == expected_size && actual == expected_size - 8)
    );
    assert_eq!(bytes, before);

    bytes[..8].copy_from_slice(&CAPACITY.to_le_bytes());
    let error = unsafe { HashSetZeroCopy::from_bytes_zero_copy_mut(&mut bytes) }.unwrap_err();
    assert!(
        matches!(error, HashSetError::BufferSize(expected, actual) if expected == expected_size && actual == expected_size - 8)
    );
}
