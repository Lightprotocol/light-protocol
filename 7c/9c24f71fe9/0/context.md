...
use a subagent to check for test coverage of the tests that we just added is there anyhting else we should  test for the generic compression and decompress functions? any other high risk paths which are neither covered explicitly nor with the randomized tests?
