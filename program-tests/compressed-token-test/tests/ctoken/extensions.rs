...
async fn test_mint_and_compress_with_extensions() {
    ...
    let (token_pool_pda, token_pool_pda_bump) = find_token_pool_pda_with_index(&mint_pubkey, 0);
...
async fn test_compress_with_restricted_extensions_fails() {
    ...
    let result = context
        ...
        .await;
    // Mint has restricted extensions - hot path required (error code 124)
    assert_rpc_error(result, 0, 124).unwrap();
...
async fn test_compress_and_close_ctoken_with_extensions() {
    ...
    context.rpc.warp_epoch_forward(1).await.unwrap();
...
async fn run_compress_and_close_extension_test(
    ...
    context.rpc.warp_epoch_forward(1).await?;
