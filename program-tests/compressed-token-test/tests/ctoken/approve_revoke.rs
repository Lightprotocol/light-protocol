...
// ============================================================================
// Approve Checked Tests
// ============================================================================

use light_ctoken_sdk::ctoken::ApproveCTokenChecked;
use light_program_test::utils::assert::assert_rpc_error;

use super::shared::setup_account_test_with_spl_mint;

/// Test approve checked with correct decimals succeeds
#[tokio::test]
#[serial]
async fn test_approve_checked_success() {
    let mut context = setup_account_test_with_spl_mint(9).await.unwrap();
    let payer_pubkey = context.payer.pubkey();
    let mint = context.mint_pubkey;
    let delegate = Keypair::new();
    let token_account_keypair = Keypair::new();

    // Create a token account directly (without assertion that expects specific structure)
    ...
    println!("test_approve_checked_success: passed");
}

/// Test approve checked with wrong decimals fails
#[tokio::test]
#[serial]
async fn test_approve_checked_wrong_decimals() {
    let mut context = setup_account_test_with_spl_mint(9).await.unwrap();
    let payer_pubkey = context.payer.pubkey();
    let mint = context.mint_pubkey;
    let delegate = Keypair::new();
    ...
    // Try to approve with wrong decimals (8 instead of 9)
    let approve_ix = ApproveCTokenChecked {
        token_account: token_account_keypair.pubkey(),
        mint,
        delegate: delegate.pubkey(),
    ...
    // Should fail because cached decimals (9) mismatch instruction decimals (8)
    // When CToken has cached decimals, we return InvalidInstructionData (code 2)
    assert_rpc_error(result, 0, 2).unwrap();
    println!("test_approve_checked_wrong_decimals: passed");
}
