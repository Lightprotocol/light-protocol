...
async fn functional_and_failing_tests() {
    ...
    // 10. SUCCEED - MintToCToken with valid mint authority
    {
        ...
        let recipient_ata =
            light_token::instruction::get_associated_token_address(&recipient2.pubkey(), &spl_mint_pda);
...
async fn test_mint_to_ctoken_max_top_up_exceeded() {
    ...
    let ctoken_ata = light_token::instruction::get_associated_token_address(&recipient.pubkey(), &spl_mint_pda);
