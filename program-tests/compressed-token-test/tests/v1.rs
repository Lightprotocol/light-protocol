...
async fn test_wrapped_sol() {
    ...
    for is_token_22 in [false] {
        ...
        .await;
        let input_compressed_accounts: Vec<CompressedTokenAccount> = test_indexer
...
async fn test_mint_to_and_burn_from_all_token_pools() {
    ...
    for is_token_22 in [false, true] {
        ...
        for i in iterator {
            let accounts: Vec<CompressedTokenAccount> = test_indexer
                .get_compressed_token_accounts_by_owner(&payer.pubkey(), None, None)
                .await
                .unwrap()
                .value
            ...
            let change_account_merkle_tree = input_compressed_account.account.tree_info.tree;
            burn_test(
                &payer,
                &mut rpc,
                &mut test_indexer,
...
async fn test_multiple_decompression() {
    ...
    for is_token_22 in [false, true] {
        ...
        {
            let all_accounts: Vec<CompressedTokenAccount> = test_indexer
                .get_compressed_token_accounts_by_owner(&sender.pubkey(), None, None)
                .await
                .unwrap()
                .value
            ...
            let input_compressed_accounts = all_accounts
                ...
                .collect::<Vec<_>>()[0]
                .clone();
            let amount = input_compressed_accounts.token.amount;
            decompress_test(
                &sender,
                &mut context,
                &mut test_indexer,
...
async fn test_delegation(
    ...
    // 1. Delegate tokens
    {
        ...
        let delegated_compressed_account_merkle_tree =
            input_compressed_accounts[0].account.tree_info.tree;
        approve_test(
            &sender,
            &mut rpc,
            &mut test_indexer,
    ...
    // 2. Transfer partial delegated amount
    {
        ...
        let input_compressed_accounts = input_compressed_accounts
            ...
            .cloned()
            .collect::<Vec<CompressedTokenAccount>>();
...
async fn test_delegation_mixed() {
    ...
    // 2. Transfer partial delegated amount with delegate change account
    {
        ...
        let mut input_compressed_accounts = input_compressed_accounts
            ...
            .cloned()
            .collect::<Vec<CompressedTokenAccount>>();
        let delegate_input_compressed_accounts: Vec<CompressedTokenAccount> = test_indexer
        ...
        let delegate_lamports = delegate_input_compressed_accounts[0].account.lamports;
...
async fn test_approve_failing() {
    ...
    let mint = input_compressed_accounts[0].token.mint;
    ...
    // 1. Invalid delegated compressed account Merkle tree.
    {
        ...
        let inputs = CreateApproveInstructionInputs {
            ...
            input_merkle_contexts: input_compressed_accounts
                ...
                .map(|x| x.account
                .tree_info
                .to_light_merkle_context(x.account.leaf_index, x.account.prove_by_index))
                .collect(),
...
async fn test_burn() {
    ...
    for is_token_22 in [false, true] {
        ...
        // 1. Burn tokens
        {
            ...
            let change_account_merkle_tree = input_compressed_accounts[0].account.tree_info.tree;
            burn_test(
                &sender,
                &mut rpc,
                &mut test_indexer,
        ...
        // 5. Burn tokens from multiple token pools
        {
            ...
            let burn_amount = input_compressed_accounts
                ...
                .map(|x| x.token.amount)
                .sum();
            let invalid_change_account_merkle_tree =
                input_compressed_accounts[0].account.tree_info.queue;
    ...
    for is_token_22 in [false, true] {
        ...
        // 1. Freeze tokens
        {
            let input_compressed_accounts: Vec<CompressedTokenAccount> = test_indexer
                ...
                .value
                .items;
            let output_merkle_tree = input_compressed_accounts[0].account.tree_info.tree;
...
async fn test_failing_freeze() {
    ...
    for is_token_22 in [false, true] {
        ...
        let input_compressed_accounts: Vec<CompressedTokenAccount> = vec![test_indexer
            .get_compressed_token_accounts_by_owner(&sender.pubkey(), None, None)
            .await
            .unwrap()
            .value
            .items[0]
            .clone()];
        let outputs_merkle_tree = input_compressed_accounts[0].account.tree_info.tree;
        ...
        // 4. Freeze frozen compressed account
        {
            ...
            let accounts = test_indexer
                ...
                .cloned()
                .collect::<Vec<_>>();
            let input_compressed_accounts: Vec<CompressedTokenAccount> = vec![accounts[0].clone()];
            let outputs_merkle_tree = input_compressed_accounts[0].account.tree_info.tree;
...
async fn test_failing_thaw() {
    ...
    for is_token_22 in [false, true] {
        ...
        // 4. thaw compressed account which is not frozen
        {
            ...
            let input_compressed_accounts = input_compressed_accounts
                .iter()
                .filter(|x| x.token.state == AccountState::Initialized)
                .cloned()
                .collect::<Vec<CompressedTokenAccount>>();
            let outputs_merkle_tree = input_compressed_accounts[0].account.tree_info.tree;
...
async fn test_failing_decompression() {
    ...
    for is_token_22 in [false, true] {
        ...
        .await
        .unwrap();
        let input_compressed_account: Vec<CompressedTokenAccount> = test_indexer
            .get_compressed_token_accounts_by_owner(&sender.pubkey(), None, None)
            .await
            .unwrap()
            .value
...
pub async fn failing_compress_decompress<R: Rpc + Indexer>(
    payer: &Keypair,
    rpc: &mut R,
    input_compressed_accounts: Vec<CompressedTokenAccount>,
    amount: u64,
...
async fn test_invalid_inputs() {
    ...
    let input_compressed_account_token_data =
        test_indexer.token_compressed_accounts[0].token.clone();
    let input_compressed_accounts = vec![test_indexer.token_compressed_accounts[0].account.clone()];
    let input_compressed_accounts_program: Vec<CompressedAccountWithMerkleContext> =
        input_compressed_accounts
            .iter()
            .cloned()
            .map(Into::into)
            .collect();
    let proof_rpc_result = test_indexer
        .get_validity_proof(vec![input_compressed_accounts[0].hash], Vec::new(), None)
        .await
        .unwrap();
    ...
    };
    {
        ...
        let res = perform_transfer_failing_test(
            ...
            &input_compressed_accounts_program,
            false,
        )
        .await;
    ...
    // Test 5: invalid input token data amount (0)
    {
        ...
        let mut input_compressed_accounts =
            vec![test_indexer.token_compressed_accounts[0].account.clone()];
        let mut vec = Vec::new();
        ...
        .unwrap();
        input_compressed_accounts[0].data.as_mut().unwrap().data = vec;
        let change_out_compressed_account_0 = TokenTransferOutputData {
            amount: input_compressed_account_token_data.amount - 1000,
            owner: recipient_keypair.pubkey(),
            lamports: None,
    ...
    // Test 6: invalid delegate
    {
        ...
        input_compressed_accounts[0].data.as_mut().unwrap().data = vec;
        let input_for_test: Vec<CompressedAccountWithMerkleContext> = input_compressed_accounts
            .iter()
            .cloned()
            .map(Into::into)
            .collect();
...
async fn batch_compress_with_batched_tree() {
    ...
    for num_recipients in 1..=8 {
        ...
        for i in 0..num_recipients {
            let recipient_compressed_token_accounts: Vec<CompressedTokenAccount> = test_indexer
            ...
            let expected_token_data = light_token::compat::TokenData {
                mint,
            ...
            assert_eq!(
                recipient_compressed_token_account.token,
                expected_token_data
            );
        }
        let token_pool_account = rpc.get_account(token_pool_pda).await.unwrap().unwrap();
