...
pub fn process_compress_full_and_close<'info>(
    ...
    let token_account_info =
        &ctx.remaining_accounts[(packed_accounts_offset + source_index) as usize];
    ...
    // Close token account (find close recipient in remaining accounts)
    let close_recipient_info =
        &ctx.remaining_accounts[(packed_accounts_offset + close_recipient_index) as usize];
