...
use changelog_test::{
    ...
    create_changelog::CreateChangelogInstructionData,
};
use solana_sdk::{
...
async fn test_sdk_test() {
    let config = ProgramTestConfig::new_v2(true, Some(vec![("changelog_test", changelog_test::ID)]));
...
#[tokio::test]
async fn test_create_changelog() {
    let config = ProgramTestConfig::new_v2(true, Some(vec![("changelog_test", changelog_test::ID)]));
    let mut rpc = LightProgramTest::new(config).await.unwrap();
    let payer = rpc.get_payer().insecure_clone();

    create_changelog_account(&payer, &mut rpc, 1000)
        .await
        .unwrap();
}
...
pub async fn create_pda(
    ...
    let system_account_meta_config = SystemAccountMetaConfig::new(changelog_test::ID);
