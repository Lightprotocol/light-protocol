...
async fn invoke_failing_test() {
    ...
    let mut test_indexer = TestIndexer::init_from_env(&payer, &env, None, 0).await;
...
pub async fn failing_transaction_inputs<R: RpcConnection, I: Indexer + TestIndexerExtensions>(
    context: &mut R,
    test_indexer: &mut I,
    payer: &Keypair,
    env: &EnvAccounts,
