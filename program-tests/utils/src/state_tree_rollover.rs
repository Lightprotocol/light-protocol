...
pub async fn assert_rolled_over_pair<R: Rpc>(
    ...
    let old_address_queue = get_hash_set::<QueueAccount, R>(rpc, *old_nullifier_queue_pubkey)
        .await
        .unwrap();
    let new_address_queue = get_hash_set::<QueueAccount, R>(rpc, *new_nullifier_queue_pubkey)
        .await
        .unwrap();
