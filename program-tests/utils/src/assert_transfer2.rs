    ...
    for action in actions.iter() {
        match action {
            Transfer2InstructionType::Transfer(transfer_input) => {
                ...
                let expected_recipient_token_data = light_token::compat::TokenData {
                    mint: source_mint,
                ...
                assert!(
                    recipient_accounts
                        .iter()
                        .any(|account| account.account.owner.to_bytes() == LIGHT_TOKEN_PROGRAM_ID),
                ...
                if change_amount > 0 {
                    ...
                    let expected_change_token = light_token::compat::TokenData {
                        mint: source_mint,
                        owner: source_owner,
            ...
            Transfer2InstructionType::CompressAndClose(compress_and_close_input) => {
                ...
                let compress_to_pubkey = if pre_account_data.data.len() > 165 {
                    ...
                    let (ctoken, _) = Token::zero_copy_at(&pre_account_data.data)
                ...
                let expected_state = if is_frozen {
                    light_token::compat::AccountState::Frozen
                } else {
                ...
                let expected_tlv = if needs_tlv {
                    Some(vec![
                        light_token_interface::state::ExtensionStruct::CompressedOnly(
                            light_token_interface::state::CompressedOnlyExtension {
                ...
                let expected_token = light_token::compat::TokenData {
                    mint: expected_mint,
