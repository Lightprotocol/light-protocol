...
mod test_instruction_builders {
    ...
    #[test]
    fn test_decompress_multiple_accounts_idempotent_instruction_builder() {
        ...
        let compressed_accounts = vec![
            (compressed_account1.clone(), account_variant1),
            (compressed_account2.clone(), account_variant2),
        ];
    ...
    #[test]
    fn test_decompress_multiple_accounts_idempotent_validation_accounts_mismatch() {
        let program_id = Pubkey::new_unique();
        let fee_payer = Pubkey::new_unique();
        let rent_payer = Pubkey::new_unique();
        ...
        let compressed_accounts = vec![
            (compressed_account1, vec![1u8, 2, 3, 4]),
            (compressed_account2, vec![5u8, 6, 7, 8]),
        ];
    ...
    #[test]
    fn test_decompress_multiple_accounts_idempotent_validation_bumps_mismatch() {
        let program_id = Pubkey::new_unique();
        let fee_payer = Pubkey::new_unique();
        let rent_payer = Pubkey::new_unique();
        let pda_accounts = vec![Pubkey::new_unique()]; // 1 PDA
        ...
        let compressed_accounts = vec![(compressed_account, vec![1u8, 2, 3, 4])];
