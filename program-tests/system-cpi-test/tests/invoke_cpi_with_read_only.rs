...
use light_compressed_account::{
    ...
    compressed_account::{PackedMerkleContext, ReadOnlyCompressedAccount},
...
#[tokio::test]
async fn addresses_with_read_only() {
    spawn_prover(
        true,
        ProverConfig {
            run_mode: Some(ProverMode::Rpc),
    ...
    for batched in [true, false] {
        ...
        // 1 in 1 out
        {
            ...
            //     .collect::<Vec<_>>();
            // let output_accounts =
            //     vec![get_compressed_output_account(true, env.batched_output_queue,); 1];
            ...
            // Use unassigned address in account_info.
            {
            ...
            // address assigned to another account, that accounts address is None.
            {
                let mut new_address_params = new_address_params.clone();
                new_address_params.assigned_account_index = Some(1);
                let mut account_info1 = account_info1.clone();
                account_info1.address = None;
                let result = local_sdk::perform_test_transaction(
                    &mut rpc,
                    &mut test_indexer,
                    &payer,
            ...
            // address assigned to another account, that accounts address is Some but another.
            {
                let new_address_params1 = new_address_params1.clone();
                // new_address_params.assigned_account_index = Some(1);
                let result = local_sdk::perform_test_transaction(
                    &mut rpc,
                    &mut test_indexer,
                ...
                .await;
                assert_rpc_error(result, 0, SystemProgramError::AddressDoesNotMatch.into())
                    .unwrap();
            }
            // two addresses assigned to same account
            {
            ...
            // Functional create two addresses.
            {
                println!("new_address_params {:?}", new_address_params);
                println!("new_address_params1 {:?}", new_address_params1);
                local_sdk::perform_test_transaction(
                    &mut rpc,
                    &mut test_indexer,
                    &payer,
