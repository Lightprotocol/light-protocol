...
mod tests {
	...
	use light_protocol_core::poseidon_merkle_tree::mt_state::{HashBytes, MerkleTree as MerkleTreeOnchain};

	use ark_std::{One};

	use light_protocol_core::utils::init_bytes18;
	use light_protocol_core::poseidon_merkle_tree::{mt_processor, mt_instructions};
  ...
  243, 171, 172, 211,  96,  76, 229,  47
    ...
    fn merkle_tree_offchain_test () {
        ...
        let leaves: Vec<Vec<u8>> = vec![initial_zero_hash.clone(); 2_usize.pow(tree_height.try_into().unwrap())];
        println!("starting to init arkworks_fork tree");
        let mut tree = MerkleTree::new(
            &leaves,
        ).unwrap();
