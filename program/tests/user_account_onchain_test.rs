...
async fn user_account_onchain_test() {
    ...
    let mut program_context = create_and_start_program(&user_account_pubkey, &program_id, &signer_pubkey).await;
    ...
    let user_account_data_init = program_context.banks_client
        .get_account(user_account_pubkey)
        .await
        .expect("get_account").unwrap();
    assert_eq!(1u8, user_account_data_init.data[0]);

    assert_eq!(signer_keypair.pubkey(), Pubkey::new(&user_account_data_init.data[2..34]));
    ...
    let user_account_data_modified = program_context.banks_client
        .get_account(user_account_pubkey)
        .await
        .expect("get_account").unwrap();
