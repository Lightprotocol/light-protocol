...
use crate::parsers::*;
use crate::state_miller_loop::{MillerLoopBytes, complete_instruction_order_verify_one};
...
pub fn _pre_process_instruction_miller_loop(_instruction_data: &[u8], accounts: &[AccountInfo]) -> Result<(), ProgramError>{ // init f::OK
    ...
    assert!(account_main_data.current_instruction_index < 1821, "Miller loop finished");
    ...
    if complete_instruction_order_verify_one[account_main_data.current_instruction_index] == 251 {
        ...
        let mut account_prepare_inputs_data = PrepareInputsBytes::unpack(&account_prepare_inputs.data.borrow())?;


        //signer which completed prepared inputs is the same as of this tx
        assert_eq!(*account_prepare_inputs.owner, solana_program::pubkey::Pubkey::new(&state_prep_inputs::PREPARED_INPUTS_PUBKEY[..]));
        //signer is equal
        assert_eq!(*signing_account.key, solana_program::pubkey::Pubkey::new(&account_prepare_inputs_data.signing_address), "Invalid sender");
        //root hash been found in prepared inputs
        assert_eq!(account_prepare_inputs_data.found_root, 1, "No root was found in prior algorithm");
        //prepared inputs has been completed
        assert_eq!(account_prepare_inputs_data.current_instruction_index, 1086,"prepare inputs is not completed yet");



        let g_ic_affine = parse_x_group_affine_from_bytes(&account_prepare_inputs_data.x_1_range); // 10k
        let p2 : ark_ec::bls12::G1Prepared<ark_bls12_381::Parameters> =  ark_ec::bls12::g1::G1Prepared::from(g_ic_affine);
        msg!("prepared inputs bytes:{:?}", account_prepare_inputs_data.x_1_range);
        ...
        MillerLoopTransferBytes::pack_into_slice(&account_main_data, &mut account_main.data.borrow_mut());
        return Ok(());
    }
    ...
    // // init f
    else if complete_instruction_order_verify_one[account_main_data.current_instruction_index] == 0 {
    ...
    // // init p1,p2:
    else if complete_instruction_order_verify_one[account_main_data.current_instruction_index] == 230 {
        ...
        let p1 : ark_ec::bls12::G1Prepared<ark_bls12_381::Parameters> =  ark_ec::bls12::g1::G1Prepared::from(proof_a);
        let p3 : ark_ec::bls12::G1Prepared<ark_bls12_381::Parameters> =  ark_ec::bls12::g1::G1Prepared::from(proof_c);
    ...
    // pass final f to account_verif2
    else if complete_instruction_order_verify_one[account_main_data.current_instruction_index] == 255 { // account_main is actually a separate account
        ...
        _process_instruction_bridge_to_final_exp(complete_instruction_order_verify_one[account_main_data.current_instruction_index], &mut account_main_data, &mut account_verify_part_2_data);
        FinalExpBytes::pack_into_slice(&account_verify_part_2_data, &mut account_main.data.borrow_mut());
    ...
    // NEW INSTRUCTION:
        ...
        _process_instruction( complete_instruction_order_verify_one[account_main_data.current_instruction_index], &mut account_main_data, &current_coeff_quad_0, &current_coeff_quad_1, &current_coeff_quad_2, &proof_b_bytes); // updated: will always pass coeff quads as empty. Acc gets them from prior instructions.
    ...
    msg!("Instruction {}", complete_instruction_order_verify_one[account_main_data.current_instruction_index]);

    //checks signer
    assert_eq!(*signing_account.key, solana_program::pubkey::Pubkey::new(&account_main_data.signing_address), "Invalid sender");
