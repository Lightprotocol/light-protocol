...
impl Pack for MillerLoopBytes {
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Self, ProgramError>{
        ...
        let (
            ...
            unused_remainder,
        ) = array_refs![input, 1, 3, 32, 176, 8,  576, 288, 288, 288, 288, 96, 96, 288, 96, 288, 48, 48, 48, 48, 48, 48, 96, 96, 96, 96, 96, 288, 192, 1,1,910];
        Ok(//216 - 32 - 8
            MillerLoopBytes {
                is_initialized: true,
    ...
    fn pack_into_slice(&self, dst: &mut [u8]) {
        ...
        let (
            ...
            unused_remainder,
        ) = mut_array_refs![dst, 1, 3, 32, 176, 8, 576, 288, 288, 288, 288, 96, 96, 288, 96, 288, 48, 48, 48, 48, 48, 48, 96, 96, 96, 96, 96, 288, 192, 1,1, 910];
        ...
        for (i, var_has_changed) in self.changed_variables.iter().enumerate(){
            if *var_has_changed {
                ...
                //additions ell_coeffs
                    ...
                    *current_coeff_2_range_dst = self.current_coeff_2_range.clone().try_into().unwrap();
                } else if i == 24 {
                    *current_coeff_3_range_dst = self.current_coeff_3_range.clone().try_into().unwrap();
                }
            }
            else {
                if i == 0 {
