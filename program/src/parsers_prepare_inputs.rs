// Dead-code: This file is just here for legacy compile reasons. In the audit version It'll be different and merged with program_prepare_inputs anyway.
...
pub fn parse_fp256_to_bytes(
    fp256: ark_ff::Fp256<ark_ed_on_bls12_381::FqParameters>,
    account: &mut Vec<u8>,
    range: [usize; 2],
) {
    let start = range[0];
...
pub fn parse_fp256_from_bytes(
    account: &Vec<u8>,
    range: [usize; 2],
) -> ark_ff::Fp256<ark_ed_on_bls12_381::FqParameters> {
    let fp256: ark_ff::Fp256<ark_ed_on_bls12_381::FqParameters>;
    let start = range[0];
    let end = range[1];
    fp256 = <Fp256<ark_ed_on_bls12_381::FqParameters> as FromBytes>::read(&account[start..end])
        .unwrap();
...
fn parse_fp384_to_bytes(
    fp384: ark_ff::Fp384<ark_bls12_381::FqParameters>,
    account: &mut Vec<u8>,
    range: [usize; 2],
) {
    let start = range[0];
