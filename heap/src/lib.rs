...
//         if pos == 0 {
//             // First time, set starting position
//             pos = (*self.start.get()) as usize + self.len;
//         }
//         pos = pos.saturating_sub(layout.size());
//         pos &= !(layout.align().wrapping_sub(1));
//         if pos < *self.start.get() + BumpAllocator::RESERVED_MEM {
...
mod test {
    ...
    fn test_unsafe_cell() {
        ...
        let cell = UnsafeCell::new(heap.as_ptr());
        // unsafe {
    ...
    //         let heap = [0u8; 128];
    //         let allocator = BumpAllocator {
    //             start: UnsafeCell::new(heap.as_ptr()),
    //             len: heap.len(),
    //         };
    //         println!("heap: {:?}", heap.as_ptr() as *const _ as usize);
    //         for i in 0..128 - size_of::<*mut u8>() {
