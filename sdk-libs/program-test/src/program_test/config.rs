...
impl ProgramTestConfig {
    ...
    /// Register custom instruction decoders for enhanced logging
    ///
    /// This allows registering decoders generated by `#[derive(InstructionDecoder)]`
    /// or `#[instruction_decoder]` macros from `light-instruction-decoder-derive`.
    ///
    /// ## Example
    ///
    /// ```rust,ignore
    ...
    /// }
    ///
    /// let config = ProgramTestConfig::new_v2(true, Some(vec![("my_program", program_id)]))
    ///     .with_decoders(vec![Box::new(MyProgramInstructionDecoder)]);
    /// ```
    pub fn with_decoders(
        mut self,
        decoders: Vec<Box<dyn crate::logging::InstructionDecoder>>,
    ) -> Self {
        self.enhanced_logging = self.enhanced_logging.with_decoders(decoders);
        self
    }
}
