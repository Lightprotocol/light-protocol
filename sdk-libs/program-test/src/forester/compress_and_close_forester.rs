...
use super::{
    instructions::create_compress_and_close_instruction,
    types::{
        get_forester_epoch_pda_from_authority, CompressAndCloseIndices,
        COMPRESSED_TOKEN_PROGRAM_ID, REGISTRY_PROGRAM_ID,
    },
};
...
pub async fn compress_and_close_forester<R: Rpc + Indexer>(
    ...
    // Create the instruction using our local instruction builder
    let compress_and_close_ix = create_compress_and_close_instruction(
        &authority.pubkey(),
