...
/// Compress and close a token account via the registry program
pub async fn compress_and_close_forester<R: Rpc + Indexer>(
    ...
    let ctoken_solana_account = rpc
        ...
        .await
        .map_err(|e| RpcError::CustomError(format!("Failed to get ctoken account: {}", e)))?
        .ok_or_else(|| RpcError::CustomError("CToken account not found".to_string()))?;

    // Prepare accounts using PackedAccounts
    let mut packed_accounts = PackedAccounts::default();

    // Manually pack accounts for the registry flow where rent_authority is a PDA
    ...
    let (ctoken_account, _) = CompressedToken::zero_copy_at(ctoken_solana_account.data.as_slice())
        .map_err(|e| RpcError::CustomError(format!("Failed to parse ctoken account: {:?}", e)))?;
