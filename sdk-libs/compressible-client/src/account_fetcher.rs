//! Utilities for fetching compressible accounts from either compressed or on-chain storage.
//!
//! This module provides functionality to transparently fetch accounts regardless of
//! whether they are compressed or on-chain.
...
/// Fetch account from either compressed or on-chain storage.
///
...
// ) -> Result<T> {
//     // First check if account exists onchain
//     if let Ok(onchain_account) = rpc_client.get_account(address) {
//         return deserialize_anchor_account(&onchain_account);
//     }

//     // If not onchain, check compressed storage
...
//     let compressed_account = light_client
//         .get_compressed_account(compressed_address, None)
//         .await?
//         .value;
...
//         .data
//         .as_ref()
//         .ok_or_else(|| anyhow::anyhow!("Compressed account has no data"))?;

//     let mut data_slice = Vec::with_capacity(8 + account_data.data.len());
//     data_slice.extend_from_slice(&account_data.discriminator);
//     data_slice.extend_from_slice(&account_data.data);

//     T::try_deserialize(&mut data_slice.as_slice()).map_err(Into::into)
// }
