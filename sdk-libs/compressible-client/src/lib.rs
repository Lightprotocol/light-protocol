...
impl CompressibleInstruction {
        ...
        T: Pack + Clone + std::fmt::Debug,
    {
        ...
        for (compressed_account, _) in compressed_accounts.iter() {
            if compressed_account.owner == COMPRESSED_TOKEN_PROGRAM_ID.into() {
                has_tokens = true;
            } else {
                has_pdas = true;
            }
    ...
    /// * `discriminator` - The instruction discriminator bytes (flexible length)
    /// * `fee_payer` - Fee payer signer
    /// * `rent_recipient` - Rent recipient account
    /// * `accounts_to_compress` - Accounts to compress (PDAs and token accounts)
    /// * `compressed_account_metas` - Metadata for where to store compressed data
    ...
    pub fn compress_accounts_idempotent(
        ...
        for (i, account) in accounts_pubkeys.iter().enumerate() {
        ...
        for account in accounts_to_compress.iter() {
            if account.owner == COMPRESSED_TOKEN_PROGRAM_ID.into() {
        ...
        for account in accounts_pubkeys {
            accounts.push(AccountMeta::new(*account, false));
        }
