...
impl<R: RpcConnection> Indexer<R> for PhotonIndexer<R> {
    async fn get_queue_elements(
        ...
        let result =
            photon_api::apis::default_api::get_queue_elements_post(&self.configuration, request)
                .await;
                ...
                Some(result) => {
                    ...
                    let proofs = response
                        ...
                        .map(|x| {
                            ...
                            let tx_hash = x.tx_hash.as_ref().map(|x| Hash::from_base58(x).unwrap());
    ...
    async fn get_compressed_accounts_by_owner_v2(
        ...
        for acc in accs.items {
            ...
            let nullifier_queue_pubkey =
                Pubkey::from(Hash::from_base58(&acc.merkle_context.queue)?);

            let merkle_context = MerkleContext {
                merkle_tree_pubkey: Pubkey::from(Hash::from_base58(&acc.merkle_context.tree)?),
            ...
            let token_data_with_merkle_context = TokenDataWithMerkleContext {
                ...
                compressed_account: CompressedAccountWithMerkleContext {
                    ...
                    merkle_context: MerkleContext {
                        merkle_tree_pubkey: Pubkey::from_str(&account.account.merkle_context.tree)
                            .unwrap(),
    ...
    async fn get_compressed_token_account_balance(
        ...
        let response = Self::extract_result("get_compressed_token_account_balance", result.result)?;
    ...
    async fn get_multiple_new_address_proofs(
        ...
        for photon_proof in photon_proofs {
            ...
            let low_address_value =
                Hash::from_base58(&photon_proof.lower_range_address).map_err(|e| {
                    IndexerError::Base58DecodeError {
    ...
    async fn get_validity_proof(
        ...
        let result =
            photon_api::apis::default_api::get_validity_proof_post(&self.configuration, request)
                .await?;
        debug!("get_validity_proof result: {:?}", result);
    ...
    async fn get_validity_proof_v2(
        ...
        let result =
            photon_api::apis::default_api::get_validity_proof_v2_post(&self.configuration, request)
                .await?;
