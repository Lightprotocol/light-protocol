...
#[repr(C)]
#[derive(ZeroCopy, LightHasher, ZeroCopyEq, Debug, PartialEq, BorshDeserialize, BorshSerialize)]
#[poseidon]
...
fn test_struct_1_derived() {
    ...
    assert_eq!(struct_1.a, 2);
    assert_eq!(struct_1.b, 3);
    assert_eq!(struct_1, borsh);
    let hash = borsh.hash::<Poseidon>();
    println!("z hash");
    let z_hash = struct_1.hash::<Poseidon>();
...
#[repr(C)]
#[derive(
    LightHasher, Debug, PartialEq, Clone, BorshSerialize, BorshDeserialize, ZeroCopy, ZeroCopyEq,
)]
#[poseidon]
...
fn test_struct_2_derived() {
    ...
    let hash = ref_struct.hash::<Poseidon>().unwrap();
    let zero_copy_hash = struct2.hash::<Poseidon>().unwrap();
    assert_eq!(hash, zero_copy_hash);
}

// Struct3 equivalent: fields should match Struct3
#[repr(C)]
#[derive(LightHasher, Debug, PartialEq, BorshSerialize, BorshDeserialize, ZeroCopy, ZeroCopyEq)]
#[poseidon]
...
fn test_struct_3_derived() {
    ...
    assert_eq!(remaining, &[]);
    let hash = ref_struct.hash::<Poseidon>().unwrap();
    let zero_copy_hash = zero_copy.hash::<Poseidon>().unwrap();
    assert_eq!(hash, zero_copy_hash);
}

#[repr(C)]
#[derive(
    LightHasher, Debug, PartialEq, BorshSerialize, BorshDeserialize, Clone, ZeroCopy, ZeroCopyEq,
)]
#[poseidon]
...
// TODO: fix LightHasher for vec of vec
// #[repr(C)]
// #[derive(
//     LightHasher, Debug, Clone, PartialEq, BorshSerialize, BorshDeserialize, ZeroCopy, ZeroCopyEq,
// )]
...
#[repr(C)]
#[derive(LightHasher, Debug, PartialEq, Clone, BorshSerialize, BorshDeserialize, ZeroCopy)]
#[poseidon]
...
fn test_struct_7_derived() {
    {
        let test_struct = Struct7Derived {
            a: 1,
            b: 2,
            option: Some(3),
        };
...
#[repr(C)]
#[derive(LightHasher, ZeroCopy, ZeroCopyEq, BorshSerialize, BorshDeserialize, PartialEq, Debug)]
#[poseidon]
...
fn test_array_struct() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(remaining, &[]);
    let hash = array_struct.hash::<Poseidon>().unwrap();
    let zero_copy_hash = zero_copy.hash::<Poseidon>().unwrap();
    assert_eq!(hash, zero_copy_hash);
    Ok(())
}
...
fn test_compressed_account_data() {
    ...
    assert_eq!(remaining, &[]);
    let hash = compressed_account_data.hash::<Poseidon>().unwrap();
    let zero_copy_hash = zero_copy.hash::<Poseidon>().unwrap();
    assert_eq!(hash, zero_copy_hash);
}
