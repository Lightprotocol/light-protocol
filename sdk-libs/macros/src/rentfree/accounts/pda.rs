...
//! marked with `#[rentfree]`. Each PDA field generates code for:
...
pub(super) struct PdaBlockBuilder<'a> {
    field: &'a RentFreeField,
    idents: PdaIdents,
}

impl<'a> PdaBlockBuilder<'a> {
    pub fn new(field: &'a RentFreeField, idx: usize) -> Self {
        Self {
            field,
