//! Orchestration layer for RentFree derive macro.
//!
//! This module coordinates code generation by combining:
//! - PDA block generation from `pda.rs`
//! - Mint action invocation from `light_mint.rs`
...
use super::builder::RentFreeBuilder;

/// Main orchestration - shows the high-level flow clearly.
pub(super) fn derive_rentfree(input: &DeriveInput) -> Result<TokenStream, syn::Error> {
    let builder = RentFreeBuilder::parse(input)?;
    builder.validate()?;
