...
//! allowing `#[rentfree_program]` to discover all `#[derive(RentFree)]` structs
...
fn has_derive_attribute(attrs: &[syn::Attribute], derive_name: &str) -> bool {
    for attr in attrs {
        ...
        if let Ok(nested) = attr.parse_args_with(
            ...
            for path in nested {
                // Check simple ident: #[derive(RentFree)]
                if let Some(ident) = path.get_ident() {
                    if ident == derive_name {
                        return true;
                    }
                }
                // Check path: #[derive(light_sdk::RentFree)]
...
mod tests {
    ...
    fn test_has_derive_attribute() {
        ...
        assert!(has_derive_attribute(&item.attrs, "RentFree"));
