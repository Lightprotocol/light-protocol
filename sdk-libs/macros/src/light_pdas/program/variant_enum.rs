...
impl<'a> LightVariantBuilder<'a> {
    ...
    /// Generate variant structs for each PDA type.
    ///
    /// This generates the `{Variant}Variant` and `Packed{Variant}Variant` structs
    /// along with their `LightAccountVariant` and `PackedLightAccountVariant` impls.
    fn generate_variant_structs(&self) -> TokenStream {
        let variants: Vec<_> = self
            .pda_ctx_seeds
            .iter()
            .map(|info| generate_single_variant_struct(info))
            .collect();
    ...
    /// Generate `impl DecompressibleAccount for LightAccountVariant`.
    ///
    /// This provides the `is_token()` method needed by HasTokenVariant.
    fn generate_decompressible_account_impl(&self) -> TokenStream {
        // All PDA variants return false for is_token()
        // CToken variants (if included) return true
        ...
        let ctoken_arm = if self.include_ctoken {
            quote! {
                Self::PackedCTokenData(_) | Self::CTokenData(_) => true,
            }
        } else {
            quote! {}
        };
        ...
        quote! {
            impl light_sdk::interface::DecompressibleAccount for LightAccountVariant {
                ...
                fn prepare<'a, 'info>(
                    ...
                    _solana_account: &anchor_lang::prelude::AccountInfo<'info>,
                    ...
                    // This is typically implemented via DecompressVariant trait on PackedLightAccountVariant
                    // For LightAccountVariant (unpacked), this shouldn't normally be called
                    Err(solana_program_error::ProgramError::InvalidAccountData)
                }
            }
        }
...
// =============================================================================
// SINGLE VARIANT STRUCT GENERATION
// =============================================================================

/// Generate a single variant struct pair (unpacked and packed) for a PDA type.
///
/// This generates:
/// - `{Variant}Variant` struct with Pubkey fields from ctx seeds + params-only seeds + data bytes
/// - `Packed{Variant}Variant` struct with u8 indices + params-only seed bytes + data bytes
fn generate_single_variant_struct(info: &PdaCtxSeedInfo) -> TokenStream {
    let variant_name = &info.variant_name;
    let variant_struct_name = format_ident!("{}Variant", variant_name);
    let packed_variant_struct_name = format_ident!("Packed{}Variant", variant_name);

    // Generate unpacked struct fields
    let ctx_fields: Vec<_> = info
        .ctx_seed_fields
        .iter()
        .map(|field| quote! { pub #field: solana_pubkey::Pubkey })
        .collect();

    let params_fields: Vec<_> = info
        .params_only_seed_fields
        .iter()
        .map(|(name, ty, _)| quote! { pub #name: #ty })
        .collect();

    // Generate packed struct fields
    let packed_ctx_fields: Vec<_> = info
        .ctx_seed_fields
        .iter()
        .map(|field| {
    ...
    let packed_params_fields: Vec<_> = info
        .params_only_seed_fields
        .iter()
        .map(|(name, ty, has_conversion)| {
            if *has_conversion {
                // For types like u64 that use to_le_bytes(), store as byte array
                let bytes_field = format_ident!("{}_bytes", name);
                quote! { pub #bytes_field: [u8; 8] }
            } else {
                // For Pubkey types, store as u8 index
                let idx_field = format_ident!("{}_idx", name);
                quote! { pub #idx_field: u8 }
            }
        })
        .collect();
    ...
    quote! {
        /// Unpacked variant struct for #variant_name.
        #[derive(anchor_lang::AnchorSerialize, anchor_lang::AnchorDeserialize, Clone, Debug)]
        ...
        /// Packed variant struct for #variant_name.
        #[derive(anchor_lang::AnchorSerialize, anchor_lang::AnchorDeserialize, Clone, Debug)]
        pub struct #packed_variant_struct_name {
            #(#packed_ctx_fields,)*
            #(#packed_params_fields,)*
            pub data: Vec<u8>,
        }
    }
}
