...
pub(crate) fn hasher(input: ItemStruct) -> Result<TokenStream> {
    ...
    let mut field_count = fields.named.len();
    if field_count >= 13 {
    ...
    fields.named.iter().enumerate().for_each(|(i, field)| {
        ...
        let flatten = field
            ...
            .iter()
            .any(|attr| attr.path().is_ident("flatten"));
        println!("field_name: {:?}", field_name.to_token_stream().to_string());
        ...
        if hash_to_field_size {
            ...
            if field.ty.to_token_stream().to_string() == "Vec < u8 >"{
                ...
                if flatten_field_exists {
                    code.push(quote! {
                        ...
                        // slices[num_flattned_fields ] = field_array[ num_flattned_fields].as_slice();
            ...
            println!(" field_type {}", org_field_type.to_token_stream().to_string());
            // let field_type = org_field_type.to_token_stream().to_string()
            //     .split(" & '")
            //     .enumerate()
            //     .map(|(i, part)| {
            //         if i == 0 {
            ...
            code.push(quote! {
                {
                    ...
                    println!("flattened_arrays: {:?}", flattened_arrays);
    ...
    println!("total field count {}", total_field_count.to_string());
    ...
    // if flatten_field_exists {
    //     println!("hasher_impl {}", hasher_impl.to_string());

    //     code.iter()
    //         .for_each(|x| println!("code: {}", x.to_string()));
    // }
    ...
    // println!("post field assignment1 {}", struct_name.to_string());

    // // Calculate the total number of fields, accounting for flattened fields

    // println!("post field assignment2 {}", struct_name.to_string());
    ...
    // let hasher_impl = quote! {  impl #impl_gen ::light_hasher::DataHasher for #struct_name #type_gen #where_clause {
    //         fn hash<H>(&self) -> ::std::result::Result<[u8; 32], ::light_hasher::HasherError>
    //         where
    //             H: ::light_hasher::Hasher
    //         {
    ...
    //         // #(#truncate_code)*
    //         // #(#code)*
    //         Ok([[0u8;32];NUM_FIELDS])
    //     }
    // };
    // let to_byte_array = quote! {
    //     Ok([0u8;32])
    // };
    // let total_field_count = 1usize;
    println!("post field assignment3 {}", struct_name.to_string());
    println!("post field 3 to_byte_array {}", to_byte_array.to_string());
    println!(
        "post field 3 to_byte_arrays_code {}",
        to_byte_arrays_code.to_string()
    );
    println!("post field 3 hasher_impl {}", hasher_impl.to_string());
    Ok(quote! {
