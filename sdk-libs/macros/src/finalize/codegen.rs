...
fn generate_pre_init_pdas_and_mints(
    ...
    let pda_count = parsed.compressible_fields.len();

    // Get the first PDA's output tree index (for the state tree output queue)
    let first_pda_output_tree = &parsed.compressible_fields[0].output_tree;
    ...
    quote! {
        ...
        // Collect compressed infos for all compressible PDA accounts
