            ...
            match first_ident.to_string().as_str() {
                ...
                _ => {
                    ...
                    if content.peek(Token![,]) {
                        let _comma: Token![,] = content.parse()?;
                        let rest: Punctuated<SeedElement, Token![,]> =
...
pub fn add_compressible_instructions(
    ...
    // Generate helper functions per account type to avoid large locals in the caller's frame
    let helper_unpacked_fns: Result<Vec<_>> = account_types.iter().map(|name| {
        ...
        let func_name = format_ident!("handle_unpacked_{}", name);
        Ok(quote! {
            #[inline(never)]
            fn #func_name<'a, 'b, 'info>(
                ...
                if derived_pda != *solana_accounts[i].key {
                    msg!(
                        "(handle_unpacked) Derived PDA does not match account at index {}: expected {:?}, got {:?}, seeds: {:?}",
                        i,
    ...
    let helper_unpacked_fns = helper_unpacked_fns?;
    ...
    // Generate match arms that call out to helpers (small caller frame)
    let call_unpacked_arms: Vec<_> = account_types.iter().map(|name| {
        let func_name = format_ident!("handle_unpacked_{}", name);
        quote! {
            CompressedAccountVariant::#name(data) => {
                #func_name(accounts, &cpi_accounts, address_space, solana_accounts, i, &data, &meta, &mut compressed_pda_infos)?;
            }
        }
    }).collect();
    ...
    let helpers_module: syn::ItemMod = {
        let helper_unpacked_fns = helper_unpacked_fns.clone();
        ...
        syn::parse_quote! {
            mod __macro_helpers {
                ...
                pub fn collect_pda_and_token<'a, 'b, 'info>(
                    ...
                    msg!("compressed_pda_infos: {:?}", compressed_pda_infos);
    ...
    let decompress_instruction: ItemFn = match instruction_variant {
        ...
        pub fn decompress_accounts_idempotent<'info>(
            ...
            fn process_tokens<'a, 'b, 'c, 'info>(
                ...
                let packed_accounts = cpi_accounts.post_system_accounts().unwrap(); // local borrow
                ...
                for (token_data, meta) in compressed_token_accounts.into_iter() {
                    ...
                    // Create compressible token account with scoped seed_slices to free stack immediately
                    {
                        ...
                        light_compressed_token_sdk::create_compressible_token_account(
                            ...
                            &seed_slices,
                            compressed_token_rent_payer.as_ref(), // rent_auth
                            compressed_token_rent_recipient.as_ref(), // rent_recipient
                ...
                // Scope large collections to free stack immediately after use
                {
                    ...
                    // all_account_infos.extend(rent_payer.to_account_infos());
            ...
            msg!("MIXED! solana accs {:?}", solana_accounts);
            ...
            msg!("has_pdas {:?} has_tokens {:?}", has_pdas, has_tokens);
            ...
            msg!("fee_payer: {:?}",ctx.accounts.fee_payer.as_ref());
            let fee_payer = ctx.accounts.fee_payer.as_ref();
            msg!("authority: {:?}",cpi_accounts.authority());
            let authority = cpi_accounts.authority().unwrap();
            msg!("cpi_context: {:?}",cpi_accounts.cpi_context());
            let cpi_context = cpi_accounts.cpi_context().unwrap();
            ...
            if has_tokens {
                process_tokens(
                    ...
                    &ctx.accounts.compressed_token_rent_payer.to_account_info(),  // TODO: token_rent_recipient
    ...
    let compress_instruction: syn::ItemFn = match instruction_variant {
        ...
        pub fn compress_accounts_idempotent<'info>(
            ...
            #[inline(never)]
            fn detect_pda_and_token_presence<'info>(
                solana_accounts: &[anchor_lang::prelude::AccountInfo<'info>],
            ) -> (bool, bool) {
                let (mut has_tokens, mut has_pdas) = (false, false);
            ...
            let (has_tokens, has_pdas) = detect_pda_and_token_presence(solana_accounts);
            if !has_tokens && !has_pdas {
                return Ok(());
            }
            ...
            let mut token_accounts_to_compress = Vec::with_capacity(0);
            ...
            fn collect_accounts_to_compress<'b, 'info>(
                ...
                token_accounts_to_compress: &mut Vec<light_compressed_token_sdk::TokenAccountToCompress<'info>>,
                compressed_pda_infos: &mut Vec<light_compressed_account::instruction_data::with_account_info::CompressedAccountInfo>,
            ...
            // 1) Compress PDAs first (if any)
            // if has_pdas {
            //     let cpi_inputs = light_sdk::cpi::CpiInputs::new(proof, compressed_pda_infos);
            //     cpi_inputs.invoke_light_system_program_small(cpi_accounts.clone())?;
            // }

            // msg!("token_accounts_to_compress {:?}", token_accounts_to_compress);

            // 2) Compress and close token accounts (tokens must always come at the end)
            if has_tokens {
                light_compressed_token_sdk::compress_and_close_token_accounts(
                    ...
                    &ctx.accounts.config,
                    &ctx.accounts.token_rent_recipient,
                    ctx.remaining_accounts,
            ...
            // if has_pdas {
            //     // Close all PDA accounts using a native close to reduce stack usage
            //     for idx in pda_indices_to_close.into_iter() {
            //         // Clone to obtain a mutable AccountInfo handle without holding large typed structs
...
fn generate_decompress_accounts_struct(
    ...
    match variant {
        ...
        InstructionVariant::TokenOnly => {
            // Token-only: only need compressed_token_rent_payer for tokens
            account_fields.push(quote! {
