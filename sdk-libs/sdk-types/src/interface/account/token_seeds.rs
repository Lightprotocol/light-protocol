...
fn unpack_token_data_from_packed<AI: AccountInfoTrait>(
    ...
    accounts: &[AI],
) -> Result<Token, LightSdkTypesError> {
    ...
    fn pack(
        ...
        let token_data = PackedTokenData {
            ...
            delegate: self
                .token_data
                .delegate
                .map(|d| remaining_accounts.insert_or_get(AM::pubkey_from_bytes(d.to_bytes())))
                .unwrap_or(0),
    ...
    S::Unpacked: UnpackedTokenSeeds<N, Packed = S>,
{
    ...
    fn into_in_tlv(&self) -> Result<Option<Vec<ExtensionInstructionData>>, LightSdkTypesError> {
        Ok(self
            .extension
            .as_ref()
