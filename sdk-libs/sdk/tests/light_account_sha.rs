//! Tests for SHA256-based LightAccount (HASH_FLAT = true).
//!
...
/// Test struct for SHA256 hashing tests.
/// Uses AnchorDiscriminator for Anchor compatibility (SHA256("account:TestShaAccount")[0..8]).
#[derive(
    Clone, Debug, Default, AnchorDiscriminator, BorshSerialize, BorshDeserialize, PartialEq,
)]
...
// Hardcoded discriminator for TestShaAccount (derived from AnchorDiscriminator)
// SHA256("account:TestShaAccount")[0..8]
const TEST_SHA_DISCRIMINATOR: [u8; 8] = [133, 206, 241, 201, 32, 17, 165, 67];

// Hardcoded SHA256 data hash for TestShaAccount { owner: [1u8; 32], counter: 42 }
// SHA256(borsh_serialize(account)) with byte[0] = 0 for BN254 compatibility
const TEST_SHA_DATA_HASH: [u8; 32] = [
    0, 9, 197, 147, 16, 241, 193, 207, 83, 232, 26, 162, 175, 208, 179, 15, 121, 149, 42, 178, 122,
    153, 187, 124, 40, 143, 206, 74, 247, 35, 196, 181,
];
...
/// Regression test ensuring SHA256 hashing remains stable.
#[test]
fn test_sha_hash_regression() {
    ...
    let account_data = TestShaAccount { owner, counter };

    let account = LightAccount::<TestShaAccount>::new_mut(&program_id, &account_meta, account_data)
    ...
    assert_eq!(
        input_info.data_hash, TEST_SHA_DATA_HASH,
        "SHA256 data hash must match hardcoded value"
    );
}
...
fn test_new_init() {
    ...
    let mut account =
        LightAccount::<TestShaAccount>::new_init(&program_id, Some(address), output_tree_index);
    ...
    let expected_out = OutAccountInfo {
        discriminator: TEST_SHA_DISCRIMINATOR,
...
fn test_new_mut() {
    ...
    let mut account =
        LightAccount::<TestShaAccount>::new_mut(&program_id, &account_meta, account_data.clone())
            .expect("Failed to create LightAccount");

    // Expected InAccountInfo
    let expected_in = InAccountInfo {
        discriminator: TEST_SHA_DISCRIMINATOR,
        data_hash: TEST_SHA_DATA_HASH,
...
fn test_new_empty() {
    ...
    let mut account = LightAccount::<TestShaAccount>::new_empty(&program_id, &account_meta)
    ...
    let expected_out = OutAccountInfo {
        discriminator: TEST_SHA_DISCRIMINATOR, // Output has discriminator set
        data_hash: [0u8; 32],
...
fn test_new_close() {
    ...
    let account =
        LightAccount::<TestShaAccount>::new_close(&program_id, &account_meta, account_data)
...
fn test_new_burn() {
    ...
    let mut account =
        LightAccount::<TestShaAccount>::new_burn(&program_id, &account_meta, account_data)
...
fn test_to_account_info_mut() {
    ...
    let expected = CompressedAccountInfo {
        ...
        output: Some(OutAccountInfo {
            discriminator: TEST_SHA_DISCRIMINATOR,
            data_hash: TEST_SHA_DATA_HASH, // Same hash for unchanged data
            output_merkle_tree_index: 5,
            lamports: 0,
            data: expected_data,
        }),
...
fn test_discriminator_method() {
    let program_id = Pubkey::new_from_array([2u8; 32]);
    let account = LightAccount::<TestShaAccount>::new_init(&program_id, None, 0);

    assert_eq!(
        *account.discriminator(),
        TEST_SHA_DISCRIMINATOR,
...
fn test_lamports_methods() {
    ...
    let mut account = LightAccount::<TestShaAccount>::new_mut(
        &program_id,
        &account_meta,
        TestShaAccount::default(),
    )
    .expect("Failed to create LightAccount");
