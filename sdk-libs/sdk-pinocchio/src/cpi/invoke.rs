...
pub fn light_system_progam_instruction_invoke_cpi(
    ...
    msg!(format!("account_metas len: {}", account_metas.len()).as_str());

    // Create instruction with owned data and immediately invoke it
    use pinocchio::instruction::{Instruction, Seed, Signer};

    let (_authority, bump) = find_cpi_signer_macro!(cpi_accounts.invoking_program().key());
    let bump_seed = [bump];
    let seed_array = [
    ...
    msg!("pre account infos");
    let account_infos = cpi_accounts.to_account_infos();
    msg!(format!("account_infos len: {}", account_infos.len()).as_str());

    // Debug: print account order
    msg!("Account metas:");
    for (i, meta) in account_metas.iter().enumerate() {
        msg!(format!("  [{}] {:?}", i, meta.pubkey).as_str());
    }
    msg!("Account infos:");
    for (i, info) in account_infos.iter().enumerate() {
        msg!(format!("  [{}] {:?}", i, info.key()).as_str());
    }

    msg!("calling slice_invoke_signed");
    match slice_invoke_signed(&instruction, &account_infos, &[signer]) {
        Ok(()) => {
            msg!("slice_invoke_signed completed successfully");
        }
        Err(e) => {
