...
use light_sdk_types::{
    cpi_accounts::v2::CpiAccounts, instruction::PackedStateTreeInfo, CpiSigner,
};
...
use light_sdk_types::{
    ...
    constants::{
        ACCOUNT_COMPRESSION_AUTHORITY_PDA, ACCOUNT_COMPRESSION_PROGRAM_ID,
...
use light_token_interface::{
    instructions::{
        ...
        transfer2::{
            Compression, CompressedTokenInstructionDataTransfer2, MultiInputTokenDataWithContext,
        },
    },
    ...
    V: DecompressVariant<AI>,
{
    ...
    if has_token_accounts {
        ...
        if token_seeds.is_empty() {
            ...
            let signer_seed_refs: Vec<&[u8]> =
                token_seeds.iter().map(|s| s.as_slice()).collect();
            AI::invoke_cpi(
                &LIGHT_TOKEN_PROGRAM_ID,
                &transfer2_data,
                &account_metas,
