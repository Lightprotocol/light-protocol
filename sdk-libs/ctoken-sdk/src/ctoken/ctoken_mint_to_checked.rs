...
/// ```
pub struct CTokenMintToChecked {
...
/// # Ok::<(), solana_program_error::ProgramError>(())
/// ```
pub struct CTokenMintToCheckedCpi<'info> {
...
impl<'info> CTokenMintToCheckedCpi<'info> {
    pub fn instruction(&self) -> Result<Instruction, ProgramError> {
        CTokenMintToChecked::from(self).instruction()
    }

    pub fn invoke(self) -> Result<(), ProgramError> {
        let instruction = CTokenMintToChecked::from(&self).instruction()?;
...
impl<'info> From<&CTokenMintToCheckedCpi<'info>> for CTokenMintToChecked {
    fn from(cpi: &CTokenMintToCheckedCpi<'info>) -> Self {
        Self {
            cmint: *cpi.cmint.key,
...
impl CTokenMintToChecked {
    pub fn instruction(self) -> Result<Instruction, ProgramError> {
        Ok(Instruction {
            ...
            data: {
                let mut data = vec![14u8]; // CTokenMintToChecked discriminator
