...
/// ```rust
/// # use solana_pubkey::Pubkey;
/// # use light_token_sdk::token::TransferSplToLightToken;
...
/// let instruction = TransferSplToLightToken {
///     amount: 100,
...
/// ```
pub struct TransferSplToLightToken {
    pub amount: u64,
...
/// ```rust,no_run
/// # use light_token_sdk::token::TransferSplToLightTokenCpi;
/// # use solana_account_info::AccountInfo;
...
/// # Ok::<(), solana_program_error::ProgramError>(())
/// ```
pub struct TransferSplToLightTokenCpi<'info> {
    pub amount: u64,
    pub spl_interface_pda_bump: u8,
...
impl<'info> TransferSplToLightTokenCpi<'info> {
    pub fn instruction(&self) -> Result<Instruction, ProgramError> {
        TransferSplToLightToken::from(self).instruction()
    }

    pub fn invoke(self) -> Result<(), ProgramError> {
        let instruction = TransferSplToLightToken::from(&self).instruction()?;
...
impl<'info> From<&TransferSplToLightTokenCpi<'info>> for TransferSplToLightToken {
    fn from(account_infos: &TransferSplToLightTokenCpi<'info>) -> Self {
        Self {
...
impl TransferSplToLightToken {
    pub fn instruction(self) -> Result<Instruction, ProgramError> {
        ...
        let ctoken_account = CTokenAccount2 {
            inputs: vec![],
            output: MultiTokenTransferOutputData::default(),
            compression: Some(Compression::decompress_light_token(self.amount, 0, 1)),
            delegate_is_set: false,
            method_used: true,
        };
