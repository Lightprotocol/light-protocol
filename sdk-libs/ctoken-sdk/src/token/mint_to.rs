...
use light_token_interface::instructions::mint_action::{
    CompressedMintWithContext, CpiContext, MintToAction,
};
...
pub struct MintToParams {
    ...
    pub mint_to_actions: Vec<MintToAction>,
...
impl MintToParams {
    pub fn new(
        ...
        Self {
            ...
            mint_to_actions: vec![MintToAction {
    ...
    pub fn add_mint_to_action(mut self, account_index: u8, amount: u64) -> Self {
        self.mint_to_actions.push(MintToAction {
            account_index,
            amount,
        });
        self
...
/// # Create a mint to Light Token instruction:
/// ```rust,no_run
/// # use solana_pubkey::Pubkey;
/// use light_token_sdk::token::{MintTo, MintToParams, CompressedMintWithContext};
...
impl MintTo {
    ...
    pub fn instruction(self) -> Result<Instruction, ProgramError> {
        ...
        for action in self.params.mint_to_actions {
            instruction_data = instruction_data.with_mint_to(action);
        }
        ...
        let meta_config = if let Some(cpi_context_pubkey) = self.cpi_context_pubkey {
            ...
            .with_ctoken_accounts(self.token_accounts)
        };
...
// ============================================================================
// Params Struct: MintToCpiWriteParams
// ============================================================================

#[derive(Debug, Clone)]
pub struct MintToCpiWriteParams {
    pub compressed_mint_inputs: CompressedMintWithContext,
    pub mint_to_actions: Vec<MintToAction>,
    pub mint_authority: Pubkey,
...
// ============================================================================
// Builder Struct: MintToCpiWrite
// ============================================================================

#[derive(Debug, Clone)]
pub struct MintToCpiWrite {
    pub payer: Pubkey,
    pub cpi_context_pubkey: Pubkey,
    pub token_accounts: Vec<Pubkey>,
    pub params: MintToCpiWriteParams,
}

impl MintToCpiWrite {
    pub fn new(
        params: MintToCpiWriteParams,
        payer: Pubkey,
...
// ============================================================================
// AccountInfos Struct: MintToCpi (for CPI usage)
// ============================================================================

/// # Mint to Light Token account via CPI:
/// ```rust,no_run
...
/// # let token_accounts: Vec<AccountInfo> = todo!();
/// # let system_accounts: SystemAccountInfos = todo!();
/// # let params: MintToParams = todo!();
/// MintToCpi {
///     authority,
///     payer,
///     state_tree,
...
impl<'info> MintToCpi<'info> {
    pub fn instruction(&self) -> Result<Instruction, ProgramError> {
        MintTo::try_from(self)?.instruction()
    }
    ...
    pub fn invoke(self) -> Result<(), ProgramError> {
        ...
        account_infos.extend(self.token_accounts);
...
impl<'info> TryFrom<&MintToCpi<'info>> for MintTo {
    type Error = ProgramError;

    fn try_from(account_infos: &MintToCpi<'info>) -> Result<Self, Self::Error> {
        if account_infos.params.mint_authority != *account_infos.authority.key {
            solana_msg::msg!(
                "MintToCpi: params.mint_authority ({}) does not match authority account ({})",
...
// ============================================================================
// AccountInfos Struct: MintToCpiWriteCpi
// ============================================================================

pub struct MintToCpiWriteCpi<'info> {
    pub payer: AccountInfo<'info>,
...
impl<'info> MintToCpiWriteCpi<'info> {
    pub fn instruction(&self) -> Result<Instruction, ProgramError> {
        MintToCpiWrite::from(self).instruction()
    }
...
impl<'info> From<&MintToCpiWriteCpi<'info>> for MintToCpiWrite {
    fn from(account_infos: &MintToCpiWriteCpi<'info>) -> Self {
        Self {
            payer: *account_infos.payer.key,
