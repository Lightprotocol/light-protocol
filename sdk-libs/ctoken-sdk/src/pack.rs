...
/// Simple trait for checking if a token variant represents a user-owned ATA.
///
/// This is used during packing to ensure the ATA address is included in the
/// packed accounts, enabling unified decompression of ATAs alongside PDAs.
///
/// Implementations are auto-generated by the `#[compressible]` macro for
/// variants marked with `is_ata`.
pub trait IsAta {
...
pub mod compat {
    ...
    where
        V: AnchorSerialize + Clone + std::fmt::Debug + super::IsAta,
    {
        type Packed = PackedCTokenDataWithVariant<V>;

        fn pack(&self, remaining_accounts: &mut PackedAccounts) -> Self::Packed {
            // For ATAs, the token_data.owner is the ATA ADDRESS (not the wallet).
            // The runtime will find the wallet by searching for a signer that derives to this ATA.
            // We don't need special handling here - just pack the owner (ATA address) normally.
            // The ATA address will be derived at runtime from (wallet, mint).
        ...
        V: AnchorSerialize + Clone + std::fmt::Debug + super::IsAta,
    {
        ...
        fn pack(&self, remaining_accounts: &mut PackedAccounts) -> Self::Packed {
            // For ATAs, insert the ATA address into packed_accounts FIRST.
            // See CTokenDataWithVariant::pack for details.
            if self.variant.is_ata() {
                let (ata_address, _bump) = crate::ctoken::get_associated_ctoken_address_and_bump(
                    &self.token_data.owner,
                    &self.token_data.mint,
                );
