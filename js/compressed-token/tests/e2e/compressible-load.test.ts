...
describe('compressible-load', () => {
    ...
    describe('buildLoadParams', () => {
        ...
        describe('ATA loading via atas parameter', () => {
            ...
            it('should return empty ataInstructions for hot ATAs', async () => {
                ...
                const loadIxs = await buildAtaLoadInstructions(
                    rpc,
                    payer.publicKey,
                    coldAta,
                    { tokenPoolInfos },
    ...
    describe('buildAtaLoadInstructions', () => {
        it('should throw if AccountInterface not from getAtaInterface', async () => {
            ...
            await expect(
                buildAtaLoadInstructions(rpc, payer.publicKey, fakeInterface),
        ...
        it('should build instructions for cold ATA', async () => {
            ...
            const ixs = await buildAtaLoadInstructions(
                rpc,
                payer.publicKey,
                ata,
                { tokenPoolInfos },
    ...
    describe('loadAtaInstructions', () => {
        ...
        it('should return empty when nothing to load (hot ATA)', async () => {
            ...
            // This is tested via buildAtaLoadInstructions since loadAtaInstructions
            // fetches internally
        });
    });

    describe('loadAtaInstructions', () => {
        it('should be an alias for buildAtaLoadInstructions', async () => {
            const owner = await newAccountWithLamports(rpc, 1e9);
            ...
            const ixs1 = await loadAtaInstructions(rpc, payer.publicKey, ata, {
                tokenPoolInfos,
            });

            const ixs2 = await buildAtaLoadInstructions(
                rpc,
                payer.publicKey,
                ata,
                { tokenPoolInfos },
            ...
            expect(ixs1.length).toBe(ixs2.length);
            expect(ixs1.length).toBeGreaterThan(0);
        });
    });
