...
describe('transfer', () => {
  ...
  it('should transfer from bob -> charlie', async () => {
    ...
    const charliePreCompressedTokenAccounts3 =
      await getCompressedTokenAccountsForTest(rpc, charlie.publicKey, mint);
    await transfer(rpc, payer, mint, bn(5), charlie, bob.publicKey, merkleTree);
    ...
    await expect(
      transfer(rpc, payer, mint, 10000, bob, charlie.publicKey, merkleTree),
