...
describe('mintTo (MintToLightToken)', () => {
    let rpc: Rpc;
    let payer: Signer;
    ...
    let recipientLightToken: PublicKey;
    ...
    beforeAll(async () => {
        ...
        recipientLightToken = getAssociatedLightTokenAddress(
            recipient.publicKey,
            mint,
        );
    });

    it('should mint tokens to onchain light-token account', async () => {
        const amount = 1000;
        ...
        const accountInfo = await rpc.getAccountInfo(recipientLightToken);
    ...
    it('should fail with wrong authority', async () => {
        ...
        await expect(
            mintTo(rpc, payer, mint, recipientLightToken, wrongAuthority, 100),
        ).rejects.toThrow();
    });
