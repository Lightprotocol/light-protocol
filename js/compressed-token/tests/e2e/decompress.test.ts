...
describe.each([TreeType.StateV2])('decompress with state tree %s', treeType => {
    let rpc: Rpc;
    let payer: Signer;
    let bob: Signer;
    let charlie: Signer;
    ...
    beforeAll(async () => {
        ...
        bob = payer; // await newAccountWithLamports(rpc, 1e9);
        charlie = payer; // await newAccountWithLamports(rpc, 1e9);
