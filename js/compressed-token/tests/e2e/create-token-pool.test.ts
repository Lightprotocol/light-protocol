...
describe('createTokenPool', () => {
    ...
    it('should allow adding multiple token pools for the same mint (idempotent)', async () => {
        ...
        const poolAccount2 = CompressedTokenProgram.deriveTokenPoolPdaWithBump(
            newMint,
            1,
        );
        const poolAccount3 = CompressedTokenProgram.deriveTokenPoolPdaWithBump(
            newMint,
            2,
        );
        const poolAccount4 = CompressedTokenProgram.deriveTokenPoolPdaWithBump(
            newMint,
            3,
        );
        const poolAccount5 = CompressedTokenProgram.deriveTokenPoolPdaWithBump(
            newMint,
            4,
        );
