...
describe.each([TreeType.StateV1, TreeType.StateV2])(
    'rpc-interop token with state tree %s',
    treeType => {
        let rpc: Rpc;
        let testRpc: Rpc;
        let payer: Signer;
        ...
        it('getCompressedTokenAccountsByOwner should match', async () => {
            ...
            const receiverAccountsTest = (
                await testRpc.getCompressedTokenAccountsByOwner(
                    charlie.publicKey,
                    {
                        mint,
                    },
        ...
        it('getCompressedTokenBalancesByOwner should match', async () => {
            const balances = (
                await rpc.getCompressedTokenBalancesByOwner(bob.publicKey, {
                    mint,
                })
            ).items;
            const balancesTest = (
            ...
            const balancesReceiverTest = (
                await testRpc.getCompressedTokenBalancesByOwner(
                    charlie.publicKey,
                    {
                        mint,
                    },
        ...
        it('getCompressedTokenBalancesByOwnerV2 should match', async () => {
            ...
            const balancesReceiver = (
                await rpc.getCompressedTokenBalancesByOwnerV2(
                    charlie.publicKey,
                    {
                        mint,
                    },
        ...
        it('[test-rpc missing] getSignaturesForTokenOwner should match', async () => {
            ...
            const signaturesReceiver = (
                await rpc.getCompressionSignaturesForTokenOwner(
                    charlie.publicKey,
                )
            ).items;
        ...
        it('[rpc] getCompressedTokenAccountsByOwner with 2 mints should return both mints', async () => {
            ...
            assert.isTrue(
                senderAccounts.items.some(
                    account =>
                        account.parsed.mint.toBase58() === mint.toBase58(),
                ),
            );
            assert.isTrue(
                senderAccounts.items.some(
                    account =>
                        account.parsed.mint.toBase58() === mint2.toBase58(),
                ),
            );
        });
        ...
        it('getCompressedMintTokenHolders should return correct holders', async () => {
            ...
            const charlieHolder = holders.value.items.find(
                holder =>
                    holder.owner.toBase58() === charlie.publicKey.toBase58(),
            );
