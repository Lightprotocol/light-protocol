...
import { SplInterface } from '../../spl-interface';
...
export function createDecompressInstruction({
    ...
    for (const account of inputCompressedTokenAccounts) {
        if (!account.parsed.mint.equals(mint)) {
            throw new Error(
                'All input light-token accounts must have the same mint for decompress.',
            );
        }
        if (!account.parsed.owner.equals(owner)) {
            throw new Error(
                'All input light-token accounts must have the same owner for decompress.',
            );
        }
    }
    ...
    // Keep token program index materialized to preserve packed account ordering
    // and index side effects for instruction construction.
    void tokenProgramIndex;
    ...
    if (amount > totalInputAmount) {
        throw new Error(
            `Decompress amount ${amount.toString()} exceeds total input amount ${totalInputAmount.toString()}.`,
        );
    }
    ...
    const keys = [
        ...
        ...packedAccounts.map((pubkey, i) => {
            ...
            const isPool =
                splInterface !== undefined &&
                pubkey.equals(splInterface.poolPda);
            return {
                pubkey,
