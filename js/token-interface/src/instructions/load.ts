...
async function _buildLoadInstructions(
  ...
  const needsToken2022Program = ataType === "token2022" || t22Balance > BigInt(0);
  ...
  if (needsSplProgram || needsToken2022Program) {
    ...
    splProgramInterface = splInterfaces.find(
      (info) =>
        info.isInitialized && info.tokenProgramId.equals(TOKEN_PROGRAM_ID),
    );
    token2022ProgramInterface = splInterfaces.find(
      (info) =>
        info.isInitialized && info.tokenProgramId.equals(TOKEN_2022_PROGRAM_ID),
    );
