...
import { getSplPoolInfos } from '../spl-interface';
...
export function createTransferCheckedInstruction({
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            { pubkey: authority, isSigner: true, isWritable: payer.equals(authority) },
...
export async function buildTransferInstructions({
    ...
    if (recipientTokenProgramId.equals(LIGHT_TOKEN_PROGRAM_ID)) {
        ...
        const splPoolInfos = await getSplPoolInfos(rpc, mint);
        const splPoolInfo = splPoolInfos.find(
            info =>
                info.isInitialized &&
