...
describe("rpc-multi-trees", () => {
  ...
  beforeAll(async () => {
    ...
    } else stateTreeInfo2 = selectStateTreeInfo(await rpc.getStateTreeInfos());
  ...
  it("must create account with random output tree (selectStateTreeInfo)", async () => {
    ...
    const addressSeed = deriveAddressSeed([seed], LightSystemProgram.programId);
  ...
  it("getMultipleCompressedAccountProofs in transfer loop should match", async () => {
    for (let round = 0; round < numberOfTransfers; round++) {
      ...
      proofs.forEach((proof, index) => {
        const expectedTree = prePayerAccounts.items[index].treeInfo.tree;
        const actualTree = proof.treeInfo.tree;
        const expectedQueue = prePayerAccounts.items[index].treeInfo.queue;
  ...
  it("getMultipleCompressedAccounts should match", async () => {
    ...
    compressedAccounts.forEach((account, index) => {
      assert.isTrue(
        account.treeInfo.tree.equals(senderAccounts.items[index].treeInfo.tree),
