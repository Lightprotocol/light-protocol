...
describe("rpc-interop", () => {
  ...
  it("getValidityProof [noforester] (inclusion) should match", async () => {
    ...
    validityProof.treeInfos.forEach((elem, index) => {
      assert.isTrue(elem.tree.equals(validityProofTest.treeInfos[index].tree));
    });
  ...
  it("getValidityProof [noforester] (new-addresses) should match", async () => {
    ...
    const validityProofTest = await testRpc.getValidityProof([], [newAddress]);
  ...
  it("getValidityProof [noforester] (combined) should match", async () => {
    ...
    compressedAccountProof.merkleProof.forEach((proof, index) => {
      assert.isTrue(proof.eq(compressedAccountProofTest.merkleProof[index]));
    });
    ...
    assert.isTrue(newAddressProof.nextIndex.eq(newAddressProofTest.nextIndex));
    assert.isTrue(
    ...
    assert.isTrue(
      newAddressProof.treeInfo.tree.equals(newAddressProofTest.treeInfo.tree),
    );
    assert.isTrue(
      newAddressProof.treeInfo.queue.equals(newAddressProofTest.treeInfo.queue),
    );
  ...
  it("getMultipleNewAddressProofs [noforester] should match", async () => {
    ...
    newAddressProof.merkleProofHashedIndexedElementLeaf.forEach(
      (elem, index) => {
        const expected =
          newAddressProofTest.merkleProofHashedIndexedElementLeaf[index];
        assert.isTrue(
          elem.eq(expected),
  ...
  it("getCompressedAccountsByOwner should match", async () => {
    ...
    assert.equal(senderAccounts.items.length, senderAccountsTest.items.length);
  ...
  it("getMultipleCompressedAccounts should match", async () => {
    ...
    const compressedAccountsTest = await testRpc.getMultipleCompressedAccounts(
      senderAccounts.items.map((account) => bn(account.hash)),
    );
  ...
  it("[test-rpc missing] getLatestCompressionSignatures should match", async () => {
    const { items: signatures } = (await rpc.getLatestCompressionSignatures())
      .value;
