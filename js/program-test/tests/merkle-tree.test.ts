...
describe("MerkleTree", () => {
  ...
  const refIndexedMerkleTreeRootWithOneAppend = [
    31, 159, 196, 171, 68, 16, 213, 28, 158, 200, 223, 91, 244, 193, 188, 162,
    50, 68, 54, 244, 116, 44, 153, 65, 209, 9, 47, 98, 126, 89, 131, 158,
  ];

  const refIndexedMerkleTreeRootWithTwoAppends = [
    1, 185, 99, 233, 59, 202, 51, 222, 224, 31, 119, 180, 76, 104, 72, 27, 152,
    12, 236, 78, 81, 60, 87, 158, 237, 1, 176, 9, 155, 166, 108, 89,
  ];
  const refIndexedMerkleTreeRootWithThreeAppends = [
    41, 143, 181, 2, 66, 117, 37, 226, 134, 212, 45, 95, 114, 60, 189, 18, 44,
    155, 132, 148, 41, 54, 131, 106, 61, 120, 237, 168, 118, 198, 63, 116,
  ];
  ...
  describe("IndexedArray", () => {
    ...
    it("should findLowElement", () => {
      ...
      const [lowElement2, nextValue2] = indexedArray.findLowElement(bn(31));
    ...
    it("should appendWithLowElementIndex", () => {
      ...
      const newElement = indexedArray.appendWithLowElementIndex(0, bn(30));
      expect(newElement.newElement).toEqual(refIndexedArrayElem2);
      expect(newElement.newLowElement).toEqual(refIndexedArrayElem0);
      expect(newElement.newElementNextValue).toEqual(HIGHEST_ADDRESS_PLUS_ONE);
    });
    ...
    it("should append 3 times and match merkle trees", async () => {
      ...
      expect(newElement2.newElementNextValue).toEqual(HIGHEST_ADDRESS_PLUS_ONE);
      ...
      leaves = [hash0, hash1, hash2, hash3].map((leaf) => bn(leaf!).toString());
      tree = new MerkleTree(26, lightWasm, leaves);
