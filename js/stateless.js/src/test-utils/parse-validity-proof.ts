...
export function proofFromJsonStruct(json: GnarkProofJson): ProofABC {
  ...
  const proofAX = new BN(json.ar[0].slice(2), 'hex').toArray('be', 32);
  const proofAY = new BN(json.ar[1].slice(2), 'hex').toArray('be', 32);
  const proofA: Uint8Array = new Uint8Array([...proofAX, ...proofAY]);

  const proofBX0 = new BN(json.bs[0][0].slice(2), 'hex').toArray('be', 32);
  const proofBX1 = new BN(json.bs[0][1].slice(2), 'hex').toArray('be', 32);
  const proofBY0 = new BN(json.bs[1][0].slice(2), 'hex').toArray('be', 32);
  const proofBY1 = new BN(json.bs[1][1].slice(2), 'hex').toArray('be', 32);
  ...
  const proofCX = new BN(json.krs[0].slice(2), 'hex').toArray('be', 32);
  const proofCY = new BN(json.krs[1].slice(2), 'hex').toArray('be', 32);
...
export function negateAndCompressProof(
  ...
  // gnark input is be, we need le to apply bitmask
