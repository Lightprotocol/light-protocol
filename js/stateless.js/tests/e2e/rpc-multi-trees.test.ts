...
describe('rpc-multi-trees', () => {
    ...
    it.skipIf(featureFlags.isV2())(
        'must create account with random output tree (selectStateTreeInfo)',
        async () => {
    ...
    it.skipIf(featureFlags.isV2())(
        'getValidityProof [noforester] (inclusion) should return correct trees and queues',
        async () => {
            const acc = await rpc.getCompressedAccount(bn(address.toBuffer()));
    ...
    it.skipIf(featureFlags.isV2())(
        'getValidityProof [noforester] (combined) should return correct trees and queues',
        async () => {
            ...
            compressedAccountProof.merkleProof.forEach((proof, index) => {
                assert.isTrue(
                    proof.eq(compressedAccountProof.merkleProof[index]),
                );
            });
