...
describe.each([TreeType.BatchedState])(
    ...
    treeType => {
        ...
        it('getValidityProof [noforester] (inclusion) should match', async () => {
            ...
            console.log('hash', senderAccounts.items[0]);
            console.log('hashTest', senderAccountsTest.items[0]);
            ...
            console.log('validityProofTest', validityProofTest);
        ...
        it('getValidityProof [noforester] (combined) should match', async () => {
            ...
            console.time('confirmTransactionIndexed');
            const slot = await rpc.getSlot();
            await rpc.confirmTransactionIndexed(slot);
            console.timeEnd('confirmTransactionIndexed');
            // await sleep(3000);
            const senderAccounts = await rpc.getCompressedAccountsByOwner(
                payer.publicKey,
            );
        ...
        it('[test-rpc missing] getCompressionSignaturesForAddress should work', async () => {
            ...
            console.log('rpc accounts len:', accounts.items.length);
            const latestAccount = accounts.items[0];

            const testRpcAccounts = await testRpc.getCompressedAccountsByOwner(
                payer.publicKey,
            );
            console.log('testRpc accounts len:', testRpcAccounts.items.length);
