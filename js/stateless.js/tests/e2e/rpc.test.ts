...
describe('rpc / photon', () => {
    ...
    /// 0th leaf merkle proof for refPayer
    ...
    it('rpc should return refAccountHash', async () => {
        ...
        console.log('(returned) ref hash', compressedAccounts[0].hash);
        console.log(
            '(returned) ref hash len',
            compressedAccounts[0].hash.length,
        );

        expect(bn(compressedAccounts[0].hash).eq(bn(refHash))).toBeTruthy();

        console.log('refHash 58 via bn', encodeBN254toBase58(bn(refHash)));
        console.log('refHash 58 via pubkey', new PublicKey(refHash).toBase58());
        console.log('hash bn from array', bn(refHash));
        ...
        console.log('arrFromB58', arrFromB58);
    ...
    it('getCompressedAccountsByOwner', async () => {
        ...
        console.log('cAccount payer:', compressedAccounts);
        assert.equal(compressedAccounts.length, 1);
        assert.equal(
    ...
    it('getCompressedAccountProof for refPayer', async () => {
        ...
        console.log(
            ...
            JSON.stringify(compressedAccountProof),
        );
        ...
        console.log('rootIndex', compressedAccountProof.rootIndex);
    ...
    it('getCompressedAccountProof: many valid proofs', async () => {
        for (let lamports = 1; lamports <= 30; lamports++) {
            await decompress(rpc, payer, lamports, payer.publicKey, merkleTree);
            console.log(`Decompressed ${lamports} lamports`);
        }
    });
    it('getHealth', async () => {
        /// getHealth
