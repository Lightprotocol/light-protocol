...
class TestTransaction {
    constructor({ txParams, provider, appParams, }) {
        this.merkleTreeProgram = new anchor_1.Program(index_1.IDL_MERKLE_TREE_PROGRAM, index_1.merkleTreeProgramId, provider.provider);
    ...
    async getTestValues() {
        if (!this.provider)
            throw new errors_1.TransactionError(errors_1.ProviderErrorCode.PROVIDER_UNDEFINED, "getTestValues", "");
        if (!this.provider.provider)
            throw new errors_1.TransactionError(errors_1.ProviderErrorCode.ANCHOR_PROVIDER_UNDEFINED, "getTestValues", "Provider.provider undefined");
        if (!this.params)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.TX_PARAMETERS_UNDEFINED, "getTestValues", "");
        if (!this.params.relayer)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.RELAYER_UNDEFINED, "getTestValues", "");
        if (!this.params.accounts.recipientSol)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.SOL_RECIPIENT_UNDEFINED, "getTestValues", "");
        if (!this.params.accounts.senderSol)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.SOL_SENDER_UNDEFINED, "getTestValues", "");
        if (!this.testValues)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.TRANSACTION_INPUTS_UNDEFINED, "getTestValues", "");
        if (this.params.accounts.recipientSpl) {
            try {
                this.testValues.recipientBalancePriorTx = new anchor_1.BN((await (0, spl_token_1.getAccount)(this.provider.provider.connection, this.params.accounts.recipientSpl)).amount.toString());
            }
            catch (e) {
                // covers the case of the recipient being a native sol address not a spl token address
                try {
                    this.testValues.recipientBalancePriorTx = new anchor_1.BN(await this.provider.provider.connection.getBalance(this.params.accounts.recipientSpl));
                }
                catch (e) { }
            }
        }
        try {
            this.testValues.recipientFeeBalancePriorTx = new anchor_1.BN(await this.provider.provider.connection.getBalance(this.params.accounts.recipientSol));
        }
        catch (error) {
            throw error;
        }
        if (this.params.action === "SHIELD") {
            this.testValues.senderFeeBalancePriorTx = new anchor_1.BN(await this.provider.provider.connection.getBalance(this.params.relayer.accounts.relayerPubkey));
        }
        else {
            this.testValues.senderFeeBalancePriorTx = new anchor_1.BN(await this.provider.provider.connection.getBalance(this.params.accounts.senderSol));
        }
        this.testValues.relayerRecipientAccountBalancePriorLastTx = new anchor_1.BN(await this.provider.provider.connection.getBalance(this.params.relayer.accounts.relayerRecipientSol));
    }
    async checkBalances(transactionInputs, remainingAccounts, proofInput, account) {
        var _a, _b, _c, _d, _e, _f;
        if (!this.params)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.TX_PARAMETERS_UNDEFINED, "getPdaAddresses", "");
        if (!transactionInputs.publicInputs)
            throw new errors_1.TransactionError(errors_1.TransactionErrorCode.PUBLIC_INPUTS_UNDEFINED, "getPdaAddresses", "");
        if (!this.params.accounts.senderSol) {
        ...
        if (new anchor_1.BN(proofInput.publicAmountSpl).toString() === "0") {
        ...
        for (var i = 0; i < ((_a = remainingAccounts.nullifierPdaPubkeys) === null || _a === void 0 ? void 0 : _a.length); i++) {
            ...
            await (0, index_1.checkRentExemption)({
        ...
        for (var i = 0; i < remainingAccounts.leavesPdaPubkeys.length; i += 2) {
            ...
            for (var j = 0; j < this.params.encryptedUtxos.length / 256; j++) {
                let decryptedUtxo1 = await index_1.Utxo.decrypt({
                ...
                if (decryptedUtxo1 !== null) {
                    index_1.Utxo.equal(this.provider.poseidon, decryptedUtxo1, this.params.outputUtxos[0], true);
                }
            }
        }
        try {
            const merkleTreeAfterUpdate = await this.merkleTreeProgram.account.transactionMerkleTree.fetch(index_1.MerkleTreeConfig.getTransactionMerkleTreePda(), "confirmed");
            leavesAccountData =
        ...
        if (this.params.action == "SHIELD" && this.testValues.is_token == false) {
            ...
            assert.equal(new anchor_1.BN(this.testValues.senderFeeBalancePriorTx)
                .sub(this.params.publicAmountSol)
                .sub(new anchor_1.BN(5000 * nrInstructions))
                .toString(), senderFeeAccountBalance.toString());
        }
        else if (this.params.action == "SHIELD" &&
            this.testValues.is_token == true) {
            var recipientAccount = await (0, spl_token_1.getAccount)(this.provider.provider.connection, this.params.accounts.recipientSpl);
            ...
            assert.equal(new anchor_1.BN(recipientFeeAccount)
                .add(new anchor_1.BN(this.params.relayer
                .getRelayerFee(this.params.ataCreationFee)
                .toString()))
                .toString(), new anchor_1.BN(this.testValues.recipientFeeBalancePriorTx)
                .sub((_b = this.params.publicAmountSol) === null || _b === void 0 ? void 0 : _b.sub(index_1.FIELD_SIZE).mod(index_1.FIELD_SIZE))
                .toString());
            assert.equal(new anchor_1.BN(relayerAccount)
            ...
            this.testValues.is_token == true) {
            await (0, spl_token_1.getAccount)(this.provider.provider.connection, this.params.accounts.senderSpl);
            var recipientAccount = await (0, spl_token_1.getAccount)(this.provider.provider.connection, this.params.accounts.recipientSpl);
            assert.equal(recipientAccount.amount.toString(), new anchor_1.BN(this.testValues.recipientBalancePriorTx)
                .sub((_d = this.params.publicAmountSpl) === null || _d === void 0 ? void 0 : _d.sub(index_1.FIELD_SIZE).mod(index_1.FIELD_SIZE))
            ...
            assert.equal(new anchor_1.BN(recipientFeeAccount)
                ...
                .toString()))
                .toString(), new anchor_1.BN(this.testValues.recipientFeeBalancePriorTx)
                .sub((_e = this.params.publicAmountSol) === null || _e === void 0 ? void 0 : _e.sub(index_1.FIELD_SIZE).mod(index_1.FIELD_SIZE))
                .toString());
            assert.equal(new anchor_1.BN(relayerAccount)
                .sub(this.params.relayer.getRelayerFee(this.params.ataCreationFee))
                // .add(new BN("5000"))
        ...
        else if (this.params.action === index_1.Action.TRANSFER) {
        ...
        if (this.params.message) {
            const indexedTransactions = await (0, index_1.fetchRecentTransactions)({
                connection: this.provider.provider.connection,
                batchOptions: {
                    limit: 5000,
                },
