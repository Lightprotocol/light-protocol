...
class User {
    ...
    async approve() {
        ...
        if (this.recentTransactionParameters?.publicAmountSpl.gt(index_1.BN_0) &&
            this.recentTransactionParameters?.action === index_1.Action.SHIELD) {
            let tokenAccountInfo = await this.provider.provider?.connection.getAccountInfo(this.recentTransactionParameters.accounts.senderSpl);
            if (!tokenAccountInfo)
            ...
            let tokenBalance = await splToken.getAccount(this.provider.provider?.connection, this.recentTransactionParameters.accounts.senderSpl);
            if (this.recentTransactionParameters?.publicAmountSpl.gt(new anchor_1.BN(tokenBalance.amount.toString())))
                throw new index_1.UserError(index_1.UserErrorCode.INSUFFICIENT_BAlANCE, "shield", `Insufficient token balance! ${this.recentTransactionParameters?.publicAmountSpl.toString()} bal: ${tokenBalance
                    .amount}`);
            try {
                const transaction = new web3_js_1.Transaction().add(splToken.createApproveInstruction(this.recentTransactionParameters.accounts.senderSpl, index_1.AUTHORITY, this.provider.wallet.publicKey, this.recentTransactionParameters?.publicAmountSpl.toNumber()));
                transaction.recentBlockhash = (await this.provider.provider?.connection.getLatestBlockhash("confirmed"))?.blockhash;
    ...
    async sendTransaction() {
        ...
        if (this.recentTransactionParameters?.action === index_1.Action.SHIELD &&
            !this.approved)
    ...
    async createUnshieldTransactionParameters({ token, publicAmountSpl, publicAmountSol, recipient = index_1.AUTHORITY, minimumLamports = true, }) {
        ...
        if (publicAmountSpl) {
            ...
            const tokenAccountInfo = await this.provider.provider.connection?.getAccountInfo(recipientSpl);
            if (!tokenAccountInfo) {
                ataCreationFee = true;
            }
        }
        ...
        let solUtxos = this.balance.tokenBalances
            .get(web3_js_1.SystemProgram.programId.toBase58())
            ?.utxos.values();
    ...
    async mergeAllUtxos(asset, confirmOptions = ConfirmOptions.spendable, latest = true) {
        ...
        let _solUtxos = this.balance.tokenBalances
            .get(new web3_js_1.PublicKey(0).toBase58())
            ?.utxos.values();
                ...
                for (var [, leaf] of data.leaves.entries()) {
                    try {
                        ...
                        if (decryptedUtxo !== null) {
                            ...
                            const nfExists = await (0, index_1.fetchNullifierAccountInfo)(decryptedUtxo.getNullifier(this.provider.poseidon), this.provider.provider?.connection);
                            if (!nfExists) {
