...
function selectInUtxos({ utxos, publicMint, publicAmountSpl, publicAmountSol, poseidon, relayerFee, inUtxos, outUtxos = [], action, numberMaxInUtxos, numberMaxOutUtxos, }) {
    ...
    for (var utxo of outUtxos) {
        if (!mint && utxo.amounts[1]?.gt(index_1.BN_0))
            mint = utxo.assets[1];
    ...
    if (mint) {
        filteredUtxos = utxos.filter((utxo) => utxo.assets.find((asset) => asset.toBase58() === mint?.toBase58()));
    ...
    if (mint && mint != index_1.TOKEN_REGISTRY.get("SOL")?.mint) {
