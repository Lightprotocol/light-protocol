...
import { merkleTreeProgramId, MERKLE_TREE_HEIGHT, sleep, } from "../index";
...
export class SolMerkleTree {
    constructor({ pubkey, poseidon, merkleTree = new MerkleTree(MERKLE_TREE_HEIGHT, poseidon), }) {
        this.pubkey = pubkey;
        this.merkleTree = merkleTree;
    }
    static async getLeaves(merkleTreePubkey, provider) {
        const merkleTreeProgram = new Program(IDL_MERKLE_TREE_PROGRAM, merkleTreeProgramId, provider);
    ...
    static async build({ pubkey, poseidon, indexedTransactions, provider, }) {
        ...
        indexedTransactions.sort((a, b) => new BN(a.firstLeafIndex).toNumber() -
            new BN(b.firstLeafIndex).toNumber());
        const merkleTreeIndex = mtFetched.nextIndex;
        const leaves = [];
        if (indexedTransactions.length > 0) {
            for (let i = 0; i < indexedTransactions.length; i++) {
                if (new BN(indexedTransactions[i].firstLeafIndex).toNumber() <
        ...
        let fetchedMerkleTree = new MerkleTree(MERKLE_TREE_HEIGHT, poseidon, leaves);
