...
import { UtxoError, UtxoErrorCode, CONSTANT_SECRET_AUTHKEY, fetchAssetByIdLookUp, getAssetIndex, hashAndTruncateToCircuit, Account, IDL_VERIFIER_PROGRAM_ZERO, CreateUtxoErrorCode, createAccountObject, COMPRESSED_UTXO_BYTES_LENGTH, UNCOMPRESSED_UTXO_BYTES_LENGTH, ENCRYPTED_COMPRESSED_UTXO_BYTES_LENGTH, NACL_ENCRYPTED_COMPRESSED_UTXO_BYTES_LENGTH, fetchVerifierByIdLookUp, setEnvironment, FIELD_SIZE, BN_0, } from "./index";
...
export class Utxo {
    ...
    assets = [SystemProgram.programId], amounts = [BN_0], account, blinding = new BN(randomBN(), 31, "be"), poolType = BN_0, verifierAddress = SystemProgram.programId, index, appDataHash, appData, appDataIdl, includeAppData = true, assetLookupTable, verifierProgramLookupTable, }) {
        var _a, _b;
        this.assetsCircuit = [];
        if (!blinding.eq(blinding.mod(FIELD_SIZE))) {
            throw new UtxoError(UtxoErrorCode.BLINDING_EXCEEDS_FIELD_SIZE, "constructor", `Blinding ${blinding}, exceeds field size.`);
        }
        if (assets.length != amounts.length) {
            throw new UtxoError(UtxoErrorCode.INVALID_ASSET_OR_AMOUNTS_LENGTH, "constructor", `Length mismatch assets: ${assets.length} != amounts: ${amounts.length}`);
        }
        if (assets.length > N_ASSETS) {
            throw new UtxoError(UtxoErrorCode.EXCEEDED_MAX_ASSETS, "constructor", `assets.length ${assets.length} > N_ASSETS ${N_ASSETS}`);
        }
        if (assets.findIndex((asset) => !asset) !== -1) {
            throw new UtxoError(UtxoErrorCode.ASSET_UNDEFINED, "constructor", `asset in index ${index} is undefined. All assets: ${assets}`);
        }
        if (assets.findIndex((asset) => !asset) !== -1) {
            throw new UtxoError(UtxoErrorCode.ASSET_UNDEFINED, "constructor", `asset in index ${index} is undefined. All assets: ${assets}`);
        }
        ...
        while (i < N_ASSETS) {
            ...
            if ((_a = amount === null || amount === void 0 ? void 0 : amount.lt) === null || _a === void 0 ? void 0 : _a.call(amount, BN_0)) {
                throw new UtxoError(UtxoErrorCode.NEGATIVE_AMOUNT, "constructor", `amount cannot be negative, amounts[${i}] = ${amount !== null && amount !== void 0 ? amount : "undefined"}`);
            }
            i++;
        }
        ...
        this.amounts = amounts.map((x) => {
            try {
                ...
                throw new UtxoError(UtxoErrorCode.NOT_U64, "constructor", `amount ${x} not a u64`);
            }
        ...
        if (assets[1].toBase58() === SystemProgram.programId.toBase58() &&
            !amounts[1].isZero()) {
            throw new UtxoError(UtxoErrorCode.POSITIVE_AMOUNT, "constructor", `spl amount cannot be positive, amounts[1] = ${(_b = amounts[1]) !== null && _b !== void 0 ? _b : "undefined"}`);
        }
        ...
        if (verifierAddress.toBase58() == SystemProgram.programId.toBase58()) {
            ...
            this.verifierProgramIndex = new BN(verifierProgramLookupTable.findIndex((verifierAddress) => {
                return verifierAddress === this.verifierAddress.toBase58();
            }));
            if (this.verifierProgramIndex.isNeg())
                throw new UtxoError(UtxoErrorCode.VERIFIER_INDEX_NOT_FOUND, "constructor", `verifier pubkey ${this.verifierAddress}, not found in lookup table`);
        }
        this.splAssetIndex = getAssetIndex(this.assets[1], assetLookupTable);
        if (this.splAssetIndex.isNeg())
            throw new UtxoError(UtxoErrorCode.ASSET_NOT_FOUND, "constructor", `asset pubkey ${this.assets[1]}, not found in lookup table`);
        // if appDataBytes parse appData from bytes
        if (appData) {
            if (!appDataIdl)
                throw new UtxoError(UtxoErrorCode.APP_DATA_IDL_UNDEFINED, "constructor", "");
            if (!appDataIdl.accounts)
                throw new UtxoError(UtxoErrorCode.APP_DATA_IDL_DOES_NOT_HAVE_ACCOUNTS, "APP_DATA_IDL_DOES_NOT_HAVE_ACCOUNTS");
            let i = appDataIdl.accounts.findIndex((acc) => {
                return acc.name === "utxo";
            });
            if (i === -1)
                throw new UtxoError(UtxoErrorCode.UTXO_APP_DATA_NOT_FOUND_IN_IDL, "constructor");
            ...
            if (hashArray.length > 16) {
                throw new UtxoError(UtxoErrorCode.INVALID_APP_DATA, "constructor", "appData length exceeds 16");
            }
            this.appDataHash = new BN(leInt2Buff(unstringifyBigInts(poseidon.F.toString(poseidon(hashArray))), 32), undefined, "le");
            if (appDataHash && appDataHash.toString() !== this.appDataHash.toString())
                throw new UtxoError(UtxoErrorCode.INVALID_APP_DATA, "constructor", "appDataHash and appData are inconsistent, appData produced a different hash than appDataHash");
            this.appData = appData;
    ...
    async toBytes(compressed = false) {
        ...
        if (!this.appDataIdl || !this.includeAppData) {
            ...
            throw new UtxoError(UtxoErrorCode.APP_DATA_IDL_UNDEFINED, "constructor", "Should include app data but no appDataIdl provided");
        }
    ...
    static fromBytes({ poseidon, bytes, account, includeAppData = true, index, appDataIdl, verifierAddress, assetLookupTable, verifierProgramLookupTable, }) {
        ...
        if (bytes.length === COMPRESSED_UTXO_BYTES_LENGTH) {
            ...
            bytes = Buffer.from([
                ...tmp,
                ...new Uint8Array(UNCOMPRESSED_UTXO_BYTES_LENGTH - COMPRESSED_UTXO_BYTES_LENGTH).fill(0),
            ]);
            includeAppData = false;
            if (!account)
                throw new UtxoError(CreateUtxoErrorCode.ACCOUNT_UNDEFINED, "fromBytes", "For deserializing a compressed utxo an account is required.");
        }
        let decodedUtxoData;
        let assets;
        ...
        if (!appDataIdl) {
            ...
            if (!appDataIdl.accounts)
                throw new UtxoError(UtxoErrorCode.APP_DATA_IDL_DOES_NOT_HAVE_ACCOUNTS, "fromBytes");
            let coder = new BorshAccountsCoder(appDataIdl);
            decodedUtxoData = coder.decode("utxo", bytes);
            appData = createAccountObject(decodedUtxoData, appDataIdl.accounts, "utxoAppData");
        }
        assets = [
            SystemProgram.programId,
            fetchAssetByIdLookUp(decodedUtxoData.splAssetIndex, assetLookupTable),
        ];
        verifierAddress = fetchVerifierByIdLookUp(decodedUtxoData.verifierAddressIndex, verifierProgramLookupTable);
        if (!account) {
            let concatPublicKey = bs58.encode(new Uint8Array([
    ...
    getCommitment(poseidon) {
        if (!this._commitment) {
            ...
            if (this.account) {
                ...
                throw new UtxoError(CreateUtxoErrorCode.ACCOUNT_UNDEFINED, "getCommitment", "Neither Account nor shieldedPublicKey was provided");
            }
    ...
    getNullifier(poseidon, index) {
        if (this.index === undefined) {
            if (index) {
                ...
                throw new UtxoError(UtxoErrorCode.INDEX_NOT_PROVIDED, "getNullifier", "The index of a UTXO in the Merkle tree is required to compute the nullifier hash.");
            }
        }
        if ((!this.amounts[0].eq(BN_0) || !this.amounts[1].eq(BN_0)) &&
            this.account.privkey.toString() === "0") {
            throw new UtxoError(UtxoErrorCode.ACCOUNT_HAS_NO_PRIVKEY, "getNullifier", "The index of an utxo in the merkle tree is required to compute the nullifier hash.");
        }
        if (!this._nullifier) {
    ...
    async encrypt(poseidon, merkleTreePdaPublicKey, compressed = true) {
        ...
        if (!this.account.aesSecret) {
            ...
            const ciphertext = box(bytes_message, nonce, this.account.encryptionKeypair.publicKey, CONSTANT_SECRET_AUTHKEY);
            return Uint8Array.from([...ciphertext]);
        }
        else {
            if (!merkleTreePdaPublicKey)
                throw new UtxoError(UtxoErrorCode.MERKLE_TREE_PDA_PUBLICKEY_UNDEFINED, "encrypt", "For aes encryption the merkle tree pda publickey is necessary to derive the viewingkey");
            setEnvironment();
            const iv16 = nonce.slice(0, 16);
            const ciphertext = await encrypt(bytes_message, this.account.getAesUtxoViewingKey(merkleTreePdaPublicKey, bs58.encode(commitment)), iv16, "aes-256-cbc", true);
            if (!compressed)
                return ciphertext;
    ...
    static async decrypt({ poseidon, encBytes, account, index, merkleTreePdaPublicKey, aes = true, commitment, appDataIdl, compressed = true, assetLookupTable, verifierProgramLookupTable, }) {
        if (aes) {
            ...
            if (!merkleTreePdaPublicKey)
                throw new UtxoError(UtxoErrorCode.MERKLE_TREE_PDA_PUBLICKEY_UNDEFINED, "encrypt", "For aes decryption the merkle tree pda publickey is necessary to derive the viewingkey");
            if (compressed) {
            ...
            try {
                const cleartext = await decrypt(encBytes, account.getAesUtxoViewingKey(merkleTreePdaPublicKey, bs58.encode(commitment)), iv16, "aes-256-cbc", true);
            ...
            if (account.encryptionKeypair.secretKey) {
                const cleartext = box.open(encBytes, nonce, nacl.box.keyPair.fromSecretKey(CONSTANT_SECRET_AUTHKEY).publicKey, account.encryptionKeypair.secretKey);
                if (!cleartext) {
                    return null;
                }
                const bytes = Buffer.from(cleartext);
    ...
    static equal(poseidon, utxo0, utxo1, skipNullifier = false) {
        var _a, _b, _c, _d;
        assert.equal(utxo0.amounts[0].toString(), utxo1.amounts[0].toString(), "solAmount");
        assert.equal(utxo0.amounts[1].toString(), utxo1.amounts[1].toString(), "splAmount");
        assert.equal(utxo0.assets[0].toBase58(), utxo1.assets[0].toBase58(), "solAsset");
        assert.equal(utxo0.assets[1].toBase58(), utxo1.assets[1].toBase58()),
            "splAsset";
        assert.equal(utxo0.assetsCircuit[0].toString(), utxo1.assetsCircuit[0].toString(), "solAsset circuit");
        assert.equal(utxo0.assetsCircuit[1].toString(), utxo1.assetsCircuit[1].toString(), "splAsset circuit");
        assert.equal(utxo0.appDataHash.toString(), utxo1.appDataHash.toString(), "appDataHash");
        assert.equal(utxo0.poolType.toString(), utxo1.poolType.toString(), "poolType");
        assert.equal(utxo0.verifierAddress.toString(), utxo1.verifierAddress.toString(), "verifierAddress");
        assert.equal(utxo0.verifierAddressCircuit.toString(), utxo1.verifierAddressCircuit.toString(), "verifierAddressCircuit");
        assert.equal((_a = utxo0.getCommitment(poseidon)) === null || _a === void 0 ? void 0 : _a.toString(), (_b = utxo1.getCommitment(poseidon)) === null || _b === void 0 ? void 0 : _b.toString(), "commitment");
        if (!skipNullifier) {
            if (utxo0.index || utxo1.index) {
                if (utxo0.account.privkey || utxo1.account.privkey) {
                    assert.equal((_c = utxo0.getNullifier(poseidon)) === null || _c === void 0 ? void 0 : _c.toString(), (_d = utxo1.getNullifier(poseidon)) === null || _d === void 0 ? void 0 : _d.toString(), "nullifier");
                }
            }
        }
    }
