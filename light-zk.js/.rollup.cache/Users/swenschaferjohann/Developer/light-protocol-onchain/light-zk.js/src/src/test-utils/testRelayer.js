...
import { fetchRecentTransactions, sendVersionedTransactions, } from "../transaction";
...
export class TestRelayer extends Relayer {
    constructor({ relayerPubkey, relayerRecipientSol, relayerFee = BN_0, highRelayerFee, payer, }) {
    ...
    async updateMerkleTree(provider) {
        ...
        const balance = await ((_a = provider.provider.connection) === null || _a === void 0 ? void 0 : _a.getBalance(this.relayerKeypair.publicKey));
        ...
        try {
            const response = await updateMerkleTreeForTest(this.relayerKeypair, provider.url);
            return response;
        }
        catch (e) {
            console.log(e);
    ...
    async sendTransactions(instructions, provider) {
        var res = await sendVersionedTransactions(instructions, provider.provider.connection, provider.lookUpTables.versionedTransactionLookupTable, useWallet(this.relayerKeypair));
        if (res.error)
    ...
    async getIndexedTransactions(connection) {
        const merkleTreeAccountInfo = await connection.getAccountInfo(MerkleTreeConfig.getTransactionMerkleTreePda(), "confirmed");
