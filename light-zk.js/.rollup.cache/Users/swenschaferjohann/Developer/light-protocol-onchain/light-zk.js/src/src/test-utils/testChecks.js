 ...
 */
export async function checkMerkleTreeUpdateStateCreated({ connection, merkleTreeUpdateState, relayer, transactionMerkleTree, leavesPdas, current_instruction_index, merkleTreeProgram, }) {
    let x = console.log;
    console.log = () => { };
    ...
    assert.equal(merkleTreeTmpAccountInfo.owner.toBase58(), merkleTreeProgram.programId.toBase58(), "merkle tree pda owner wrong after initializing");
    ...
    assert.equal(merkleTreeUpdateStateData.relayer.toBase58(), relayer.toBase58(), "The incorrect signer has been saved");
    assert.equal(merkleTreeUpdateStateData.merkleTreePdaPubkey.toBase58(), transactionMerkleTree.toBase58(), "the incorrect merkle tree pubkey was saved");
    assert.equal(merkleTreeUpdateStateData.numberOfLeaves, leavesPdas.length, "The incorrect number of leaves was saved");
    assert(merkleTreeUpdateStateData.currentInstructionIndex.eq(new anchor.BN(current_instruction_index)), "The instruction index is wrong");
    assert.equal(MerkleTreeAccountInfo.pubkeyLocked.toBase58(), merkleTreeUpdateState.toBase58());
    console.log("checkMerkleTreeUpdateStateCreated: success");
    console.log = x;
}
export async function checkMerkleTreeBatchUpdateSuccess({ connection, merkleTreeUpdateState, merkleTreeAccountPrior, numberOfLeaves, transactionMerkleTree, merkleTreeProgram, }) {
    var merkleTreeTmpStateAccount = await connection.getAccountInfo(merkleTreeUpdateState, "confirmed");
    assert.equal(merkleTreeTmpStateAccount, null, "Shielded transaction failed merkleTreeTmpStateAccount is not closed");
    ...
    assert.equal(merkleTreeAccount.pubkeyLocked.toBase58(), new solana.PublicKey(new Uint8Array(32).fill(0)).toBase58());
    assert.equal(merkleTreeAccount.timeLocked.toNumber(), 0, "Lock has not been taken within prior  20 slots");
    ...
    assert(merkle_tree_prior_current_root_index
        .add(new anchor.BN("1"))
        .mod(new anchor.BN(256))
        .eq(current_root_index));
    assert(merkle_tree_prior_leaves_index
...
export async function checkNfInserted(pubkeys, connection, returnValue = false) {
