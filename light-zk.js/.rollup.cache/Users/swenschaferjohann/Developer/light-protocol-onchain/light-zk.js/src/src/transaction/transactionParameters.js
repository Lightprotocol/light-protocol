...
import { FIELD_SIZE, hashAndTruncateToCircuit, merkleTreeProgramId, Relayer, TransactionErrorCode, TransactionError, TransactionParametersErrorCode, TransactionParametersError, UserErrorCode, RelayerErrorCode, CreateUtxoErrorCode, selectInUtxos, Transaction, Action, IDL_VERIFIER_PROGRAM_ZERO, createOutUtxos, BN_0, } from "../index";
...
export class TransactionParameters {
    constructor({ message, eventMerkleTreePubkey, transactionMerkleTreePubkey, senderSpl, recipientSpl, senderSol, recipientSol, inputUtxos, outputUtxos, relayer, encryptedUtxos, poseidon, action, ataCreationFee, verifierIdl, }) {
        if (!outputUtxos && !inputUtxos) {
            throw new TransactionParametersError(TransactionErrorCode.NO_UTXOS_PROVIDED, "constructor", "");
        }
        if (!verifierIdl) {
            throw new TransactionParametersError(TransactionParametersErrorCode.NO_VERIFIER_IDL_PROVIDED, "constructor", "");
        }
        if (!poseidon) {
            throw new TransactionParametersError(TransactionParametersErrorCode.NO_POSEIDON_HASHER_PROVIDED, "constructor", "");
        }
        if (!action) {
            throw new TransactionParametersError(TransactionParametersErrorCode.NO_ACTION_PROVIDED, "constructor", "Define an action either Action.TRANSFER, Action.SHIELD,Action.UNSHIELD");
        }
        ...
        if (action === Action.SHIELD && senderSol) {
            ...
            throw new TransactionParametersError(TransactionErrorCode.SOL_SENDER_UNDEFINED, "constructor", "Sender sol always needs to be defined because we use it as the signer to instantiate the relayer object.");
        }
        if (action !== Action.SHIELD) {
            if (relayer) {
                ...
                throw new TransactionParametersError(TransactionErrorCode.RELAYER_UNDEFINED, "constructor", "For a transfer or withdrawal a relayer needs to be provided.");
            }
        }
        ...
        if (!this.publicAmountSol.gte(BN_0))
            throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_NEGATIVE, "constructor", "Public sol amount cannot be negative.");
        if (!this.publicAmountSpl.gte(BN_0))
            throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_NEGATIVE, "constructor", "Public spl amount cannot be negative.");
        ...
        if (this.action === Action.SHIELD) {
             ...
             */
            if (relayer)
                throw new TransactionParametersError(TransactionParametersErrorCode.RELAYER_DEFINED, "constructor", "For a deposit no relayer should to be provided, the user send the transaction herself.");
            try {
                this.publicAmountSol.toArray("be", 8);
            }
            catch (error) {
                throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_NOT_U64, "constructor", `Public amount sol ${this.publicAmountSol} needs to be a u64 at deposit. Check whether you defined input and output utxos correctly, for a deposit the amounts of output utxos need to be bigger than the amounts of input utxos`);
            }
            try {
                this.publicAmountSpl.toArray("be", 8);
            }
            catch (error) {
                throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_NOT_U64, "constructor", `Public amount spl ${this.publicAmountSpl} needs to be a u64 at deposit. Check whether you defined input and output utxos correctly, for a deposit the amounts of output utxos need to be bigger than the amounts of input utxos`);
            }
            if (!this.publicAmountSol.eq(BN_0) && recipientSol) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SOL_RECIPIENT_DEFINED, "constructor", "");
            }
            if (!this.publicAmountSpl.eq(BN_0) && recipientSpl) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SPL_RECIPIENT_DEFINED, "constructor", "");
            }
            if (!this.publicAmountSol.eq(BN_0) && !senderSol) {
                throw new TransactionParametersError(TransactionErrorCode.SOL_SENDER_UNDEFINED, "constructor", "");
            }
            if (!this.publicAmountSpl.eq(BN_0) && !senderSpl) {
                throw new TransactionParametersError(TransactionErrorCode.SPL_SENDER_UNDEFINED, "constructor", "");
            }
        }
            ...
            if (!relayer)
                throw new TransactionParametersError(TransactionErrorCode.RELAYER_UNDEFINED, "constructor", "For a withdrawal a relayer needs to be provided.");
            ...
            if (!tmpSol.sub(FIELD_SIZE).lte(BN_0))
                throw new TransactionParametersError(TransactionParametersErrorCode.INVALID_PUBLIC_AMOUNT, "constructor", "");
            ...
            try {
                ...
                throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_NOT_U64, "constructor", "Public amount needs to be a u64 at deposit.");
            }
            try {
                if (!tmpSpl.eq(BN_0)) {
            ...
            if (!this.publicAmountSol.eq(BN_0) && !recipientSol) {
                throw new TransactionParametersError(TransactionErrorCode.SOL_RECIPIENT_UNDEFINED, "constructor", "");
            }
            if (!this.publicAmountSpl.eq(BN_0) && !recipientSpl) {
                throw new TransactionParametersError(TransactionErrorCode.SPL_RECIPIENT_UNDEFINED, "constructor", "");
            }
            // && senderSol.toBase58() != merkle tree token pda
            if (!this.publicAmountSol.eq(BN_0) && senderSol) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SOL_SENDER_DEFINED, "constructor", "");
            }
            if (!this.publicAmountSpl.eq(BN_0) && senderSpl) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SPL_SENDER_DEFINED, "constructor", "");
            }
        }
             ...
             */
            if (!relayer)
                throw new TransactionParametersError(TransactionErrorCode.RELAYER_UNDEFINED, "constructor", "For a transfer a relayer needs to be provided.");
            if (!this.publicAmountSpl.eq(BN_0))
                throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_SPL_NOT_ZERO, "constructor", `For a transfer public spl amount needs to be zero ${this.publicAmountSpl}`);
            const tmpSol = this.publicAmountSol;
            if (!tmpSol
                .sub(FIELD_SIZE)
                .mul(new BN(-1))
                .eq(relayer.getRelayerFee(ataCreationFee)))
                throw new TransactionParametersError(TransactionParametersErrorCode.PUBLIC_AMOUNT_SOL_NOT_ZERO, "constructor", `public amount ${tmpSol
                    .sub(FIELD_SIZE)
                    .mul(new BN(-1))}  should be ${relayer.getRelayerFee(ataCreationFee)}`);
            if (recipientSpl) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SPL_RECIPIENT_DEFINED, "constructor", "This is a transfer, no spl amount should be withdrawn. To withdraw an spl amount mark the transaction as withdrawal.");
            }
            if (recipientSol) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SOL_RECIPIENT_DEFINED, "constructor", "This is a transfer, no sol amount should be withdrawn. To withdraw an sol amount mark the transaction as withdrawal.");
            }
            if (senderSol) {
                throw new TransactionParametersError(TransactionParametersErrorCode.SOL_SENDER_DEFINED, "constructor", "");
            }
            ...
            throw new TransactionParametersError(TransactionParametersErrorCode.NO_ACTION_PROVIDED, "constructor", "");
        }
        this.accounts = {
            ...
            registeredVerifierPda: Transaction.getRegisteredVerifierPda(merkleTreeProgramId, this.verifierProgramId),
            authority: Transaction.getSignerAuthorityPda(merkleTreeProgramId, this.verifierProgramId),
            senderSpl: senderSpl,
    ...
    static getVerifierProgramId(verifierIdl) {
        ...
        if (!programIdObj || typeof programIdObj.value !== "string") {
            throw new TransactionParametersError(TransactionParametersErrorCode.PROGRAM_ID_CONSTANT_UNDEFINED, 'PROGRAM_ID constant not found in idl. Example: pub const PROGRAM_ID: &str = "Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS";');
        }
    ...
    static async fromBytes({ poseidon, utxoIdls, bytes, relayer, verifierIdl, assetLookupTable, verifierProgramLookupTable, }) {
        ...
        const getUtxos = (utxoBytesArray, utxoIdls) => {
            ...
            for (var [_, utxoBytes] of utxoBytesArray.entries()) {
                ...
                if (utxoBytes.subarray(128, 160).toString() !==
                    ...
                    if (!utxoIdls) {
                        throw new TransactionParametersError(TransactionParametersErrorCode.UTXO_IDLS_UNDEFINED, "fromBytes");
                    }
                    let idlIndex = TransactionParameters.findIdlIndex(new PublicKey(utxoBytes.subarray(128, 160)).toBase58(), utxoIdls);
                    // could add option to fetch idl from chain if not found
                    appDataIdl = utxoIdls[idlIndex];
                }
                utxos.push(Utxo.fromBytes({
                    poseidon,
                    bytes: utxoBytes,
                    appDataIdl,
                    assetLookupTable,
        ...
        if (relayer &&
            ...
            throw new TransactionParametersError(TransactionParametersErrorCode.RELAYER_INVALID, "fromBytes", "The provided relayer has a different public key as the relayer publickey decoded from bytes");
        }
        if (!relayer) {
            throw new TransactionParametersError(TransactionErrorCode.RELAYER_UNDEFINED, "fromBytes");
        }
        let action = Action.TRANSFER;
        if (decoded.recipientSol.toBase58() !== AUTHORITY.toBase58() ||
    ...
    static async getTxParams({ tokenCtx, publicAmountSpl = BN_0, publicAmountSol = BN_0, action, userSplAccount = AUTHORITY, account, utxos, inUtxos, 
    // for unshield
    ...
    appUtxo, addInUtxos = true, addOutUtxos = true, verifierIdl, mergeUtxos = false, message, assetLookupTable, verifierProgramLookupTable, separateSolUtxo = false, }) {
        if (action === Action.TRANSFER && !outUtxos && !mergeUtxos)
            throw new TransactionParametersError(UserErrorCode.SHIELDED_RECIPIENT_UNDEFINED, "getTxParams", "Recipient outUtxo not provided for transfer");
        if (action !== Action.SHIELD && !(relayer === null || relayer === void 0 ? void 0 : relayer.getRelayerFee(ataCreationFee))) {
            // TODO: could make easier to read by adding separate if/cases
            throw new TransactionParametersError(RelayerErrorCode.RELAYER_FEE_UNDEFINED, "getTxParams", `No relayerFee provided for ${action.toLowerCase()}}`);
        }
        if (!account) {
            throw new TransactionParametersError(CreateUtxoErrorCode.ACCOUNT_UNDEFINED, "getTxParams", "account for change utxo is undefined");
        }
        ...
        if (addInUtxos) {
            inputUtxos = selectInUtxos({
                ...
                outUtxos,
                utxos,
                relayerFee: relayer === null || relayer === void 0 ? void 0 : relayer.getRelayerFee(ataCreationFee),
                action,
        ...
        let txParams = new TransactionParameters({
            ...
            senderSol: action === Action.SHIELD ? provider.wallet.publicKey : undefined,
    ...
    assignAccounts() {
        ...
        if (!this.assetPubkeys)
            throw new TransactionParametersError(TransactionErrorCode.ASSET_PUBKEYS_UNDEFINED, "assignAccounts assetPubkeys undefined", "assignAccounts");
        if (this.action.toString() === Action.UNSHIELD.toString() ||
            this.action.toString() === Action.TRANSFER.toString()) {
            this.accounts.senderSpl = MerkleTreeConfig.getSplPoolPdaToken(this.assetPubkeys[1], merkleTreeProgramId);
            ...
            if (!this.accounts.recipientSpl) {
                ...
                if (!((_a = this.publicAmountSpl) === null || _a === void 0 ? void 0 : _a.eq(BN_0))) {
                    throw new TransactionError(TransactionErrorCode.SPL_RECIPIENT_UNDEFINED, "assignAccounts", "Spl recipientSpl is undefined while public spl amount is != 0.");
                }
            }
            if (!this.accounts.recipientSol) {
                // AUTHORITY is used as place holder
                this.accounts.recipientSol = AUTHORITY;
                if (!this.publicAmountSol.eq(BN_0) &&
                    !((_b = this.publicAmountSol) === null || _b === void 0 ? void 0 : _b.sub(FIELD_SIZE).mul(new BN(-1)).sub(new BN(this.relayer.getRelayerFee(this.ataCreationFee))).eq(BN_0))) {
                    throw new TransactionParametersError(TransactionErrorCode.SOL_RECIPIENT_UNDEFINED, "assignAccounts", "Sol recipientSpl is undefined while public spl amount is != 0.");
                }
            }
        }
        else {
            if (this.action.toString() !== Action.SHIELD.toString()) {
                throw new TransactionParametersError(TransactionErrorCode.ACTION_IS_NO_DEPOSIT, "assignAccounts", "Action is withdrawal but should not be. Spl & sol senderSpl accounts are provided and a relayer which is used to identify transfers and withdrawals. For a deposit do not provide a relayer.");
            }
            this.accounts.recipientSpl = MerkleTreeConfig.getSplPoolPdaToken(this.assetPubkeys[1], merkleTreeProgramId);
            this.accounts.recipientSol =
                MerkleTreeConfig.getSolPoolPda(merkleTreeProgramId).pda;
            if (!this.accounts.senderSpl) {
                /// assigning a placeholder account
                this.accounts.senderSpl = AUTHORITY;
                if (!((_c = this.publicAmountSpl) === null || _c === void 0 ? void 0 : _c.eq(BN_0))) {
                    throw new TransactionParametersError(TransactionErrorCode.SPL_SENDER_UNDEFINED, "assignAccounts", "Spl senderSpl is undefined while public spl amount is != 0.");
                }
            }
    ...
    static getEscrowPda(verifierProgramId) {
        return PublicKey.findProgramAddressSync([anchor.utils.bytes.utf8.encode("escrow")], verifierProgramId)[0];
    }
    static getAssetPubkeys(inputUtxos, outputUtxos) {
        ...
        let assetPubkeys = [SystemProgram.programId];
        if (inputUtxos) {
            inputUtxos.map((utxo) => {
                let found = false;
        ...
        if ((!inputUtxos && !outputUtxos) ||
            ((inputUtxos === null || inputUtxos === void 0 ? void 0 : inputUtxos.length) == 0 && (outputUtxos === null || outputUtxos === void 0 ? void 0 : outputUtxos.length) == 0)) {
            throw new TransactionError(TransactionErrorCode.NO_UTXOS_PROVIDED, "getAssetPubkeys", "No input or output utxos provided.");
        }
        ...
        if (assetPubkeys.length > N_ASSET_PUBKEYS) {
            throw new TransactionError(TransactionErrorCode.EXCEEDED_MAX_ASSETS, "getAssetPubkeys", `Utxos contain too many different assets ${assetPubkeys.length} > max allowed: ${N_ASSET_PUBKEYS}`);
        }
    ...
    async getTxIntegrityHash(poseidon) {
        if (!this.relayer)
            throw new TransactionError(TransactionErrorCode.RELAYER_UNDEFINED, "getTxIntegrityHash", "");
        if (!this.accounts.recipientSpl)
            throw new TransactionError(TransactionErrorCode.SPL_RECIPIENT_UNDEFINED, "getTxIntegrityHash", "");
        if (!this.accounts.recipientSol)
            throw new TransactionError(TransactionErrorCode.SOL_RECIPIENT_UNDEFINED, "getTxIntegrityHash", "");
        if (!this.relayer.getRelayerFee(this.ataCreationFee))
            throw new TransactionError(TransactionErrorCode.RELAYER_FEE_UNDEFINED, "getTxIntegrityHash", "");
        if (this.encryptedUtxos &&
            this.encryptedUtxos.length > 128 * this.verifierConfig.out)
            throw new TransactionParametersError(TransactionParametersErrorCode.ENCRYPTED_UTXOS_TOO_LONG, "getTxIntegrityHash", `Encrypted utxos are too long: ${this.encryptedUtxos.length} > ${128 * this.verifierConfig.out}`);
        if (!this.encryptedUtxos) {
        ...
        if (this.encryptedUtxos) {
            ...
            throw new TransactionError(TransactionErrorCode.ENCRYPTING_UTXOS_FAILED, "getTxIntegrityHash", "");
        }
    }
        ...
        if (encryptedUtxos) {
            ...
            for (var utxo in this.outputUtxos) {
                if (this.outputUtxos[utxo].appDataHash.toString() !== "0" &&
                    ...
                    throw new TransactionError(TransactionErrorCode.UNIMPLEMENTED, "encryptUtxos", "Automatic encryption for utxos with application data is not implemented.");
