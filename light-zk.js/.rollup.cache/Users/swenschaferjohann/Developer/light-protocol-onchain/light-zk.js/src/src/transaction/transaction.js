...
import { BN, BorshAccountsCoder, Program, utils } from "@coral-xyz/anchor";
import { N_ASSET_PUBKEYS } from "../utxo";
import { merkleTreeProgramId, TransactionErrorCode, TransactionError, ProviderErrorCode, SolMerkleTreeErrorCode, TransactionParameters, firstLetterToUpper, createAccountObject, firstLetterToLower, hashAndTruncateToCircuit, MINT, sendVersionedTransactions, AUTHORITY, MerkleTreeConfig, TRANSACTION_MERKLE_TREE_SWITCH_TRESHOLD, BN_0, } from "../index";
...
export class Transaction {
    ...
    constructor({ provider, shuffleEnabled = false, params, appParams, }) {
        var _a;
        if (!provider)
            throw new TransactionError(TransactionErrorCode.PROVIDER_UNDEFINED, "constructor");
        if (!provider.poseidon)
            throw new TransactionError(TransactionErrorCode.POSEIDON_HASHER_UNDEFINED, "constructor", "Poseidon hasher in provider undefined.");
        if (!provider.solMerkleTree)
            throw new TransactionError(ProviderErrorCode.SOL_MERKLE_TREE_UNDEFINED, "constructor", "Merkle tree not set in provider");
        if (!provider.wallet)
            throw new TransactionError(TransactionErrorCode.WALLET_UNDEFINED, "constructor", "Wallet not set in provider.");
        if (!params) {
            throw new TransactionError(TransactionErrorCode.TX_PARAMETERS_UNDEFINED, "constructor");
        }
        if (!params.verifierIdl)
            throw new TransactionError(TransactionErrorCode.VERIFIER_IDL_UNDEFINED, "constructor", "");
        if (params.verifierConfig.in.toString() === "4" && !appParams)
            throw new TransactionError(TransactionErrorCode.APP_PARAMETERS_UNDEFINED, "constructor", "For application transactions application parameters need to be specified.");
        if (appParams && params.verifierConfig.in.toString() !== "4")
            throw new TransactionError(TransactionErrorCode.INVALID_VERIFIER_SELECTED, "constructor", "For application transactions, an application-enabled verifier (like verifier two) is required.");
        ...
        if (params.action === Action.SHIELD) {
            ...
            if ((wallet === null || wallet === void 0 ? void 0 : wallet.publicKey.toBase58()) !==
                params.relayer.accounts.relayerPubkey.toBase58() &&
                (wallet === null || wallet === void 0 ? void 0 : wallet.publicKey.toBase58()) !==
                    ((_a = params.accounts.signingAddress) === null || _a === void 0 ? void 0 : _a.toBase58())) {
                throw new TransactionError(TransactionErrorCode.WALLET_RELAYER_INCONSISTENT, "constructor", "The wallet used in your Node.js or Browser environment does not match the wallet used for the senderFee when setting up the relayer during the deposit process. They need to be the same.");
            }
        }
    ...
    async compileAndProve() {
        await this.compile();
        if (!this.params)
            throw new TransactionError(TransactionErrorCode.TX_PARAMETERS_UNDEFINED, "compileAndProve");
        await this.getProof();
        if (this.appParams)
    ...
    async compile() {
        ...
        if (!this.provider.solMerkleTree)
            throw new TransactionError(ProviderErrorCode.SOL_MERKLE_TREE_UNDEFINED, "getProofInput");
        await this.params.getTxIntegrityHash(this.provider.poseidon);
        if (!this.params.txIntegrityHash)
            throw new TransactionError(TransactionErrorCode.TX_INTEGRITY_HASH_UNDEFINED, "compile");
        ...
        this.proofInput = {
            ...
            inPrivateKey: (_a = this.params.inputUtxos) === null || _a === void 0 ? void 0 : _a.map((x) => x.account.privkey),
            ...
            outPubkey: (_f = this.params.outputUtxos) === null || _f === void 0 ? void 0 : _f.map((x) => x.account.pubkey),
            inIndices: this.getIndices(this.params.inputUtxos),
            outIndices: this.getIndices(this.params.outputUtxos),
            inAppDataHash: (_g = this.params.inputUtxos) === null || _g === void 0 ? void 0 : _g.map((x) => x.appDataHash),
            outAppDataHash: (_h = this.params.outputUtxos) === null || _h === void 0 ? void 0 : _h.map((x) => x.appDataHash),
        ...
        if (this.appParams) {
            ...
            this.proofInput.publicAppVerifier = hashAndTruncateToCircuit(TransactionParameters.getVerifierProgramId(this.appParams.verifierIdl).toBuffer());
            this.proofInput = {
                ...this.appParams.inputs,
                ...this.proofInput,
                inPublicKey: (_p = (_o = this.params) === null || _o === void 0 ? void 0 : _o.inputUtxos) === null || _p === void 0 ? void 0 : _p.map((utxo) => utxo.account.pubkey),
            };
        }
    }
    getMint() {
        if (this.params.publicAmountSpl.eq(BN_0)) {
            ...
            throw new TransactionError(TransactionErrorCode.GET_MINT_FAILED, "getMint", "Failed to retrieve mint. The transaction parameters should contain 'assetPubkeysCircuit' after initialization, but it's missing.");
        }
    }
    async getProof() {
    ...
    async getAppProof() {
        if (!this.appParams)
            throw new TransactionError(TransactionErrorCode.APP_PARAMETERS_UNDEFINED, "getAppProof");
        if (!this.appParams.path)
            throw new TransactionError(TransactionErrorCode.FIRST_PATH_APP_UNDEFINED, "getAppProof", "The app path is not defined. Please ensure it is specified in 'appParams'.");
    ...
    async getProofInternal(params, firstPath) {
        if (!this.proofInput)
            throw new TransactionError(TransactionErrorCode.PROOF_INPUT_UNDEFINED, "getProofInternal");
        if (!this.params)
            throw new TransactionError(TransactionErrorCode.NO_PARAMETERS_PROVIDED, "getProofInternal");
        if (!this.params.verifierIdl)
            throw new TransactionError(TransactionErrorCode.NO_PARAMETERS_PROVIDED, "getProofInternal", "verifierIdl is missing in TransactionParameters");
        ...
        try {
            ...
            throw new TransactionError(TransactionErrorCode.PROOF_GENERATION_FAILED, "getProofInternal", error);
        }
        console.timeEnd(`${prefix} Proving ${params.verifierIdl.name} circuit`);
        const res = await prover.verify();
        if (res !== true) {
            throw new TransactionError(TransactionErrorCode.INVALID_PROOF, "getProofInternal");
        }
    ...
    static getTransactionHash(params, poseidon) {
        ...
        if (!params.txIntegrityHash)
            throw new TransactionError(TransactionErrorCode.TX_INTEGRITY_HASH_UNDEFINED, "getTransactionHash");
        const inputHasher = poseidon.F.toString(poseidon((_a = params === null || params === void 0 ? void 0 : params.inputUtxos) === null || _a === void 0 ? void 0 : _a.map((utxo) => utxo.getCommitment(poseidon))));
        const outputHasher = poseidon.F.toString(poseidon((_b = params === null || params === void 0 ? void 0 : params.outputUtxos) === null || _b === void 0 ? void 0 : _b.map((utxo) => utxo.getCommitment(poseidon))));
    ...
    async getRootIndex() {
        if (!this.provider.solMerkleTree)
            throw new TransactionError(ProviderErrorCode.SOL_MERKLE_TREE_UNDEFINED, "getRootIndex", "");
        if (!this.provider.solMerkleTree.merkleTree)
            throw new TransactionError(SolMerkleTreeErrorCode.MERKLE_TREE_UNDEFINED, "getRootIndex", "The Merkle tree is not defined in the 'provider.solMerkleTree' object.");
        if (this.provider.provider && this.provider.solMerkleTree.merkleTree) {
            this.merkleTreeProgram = new Program(IDL_MERKLE_TREE_PROGRAM, merkleTreeProgramId, this.provider.provider);
            ...
            if (this.transactionInputs.rootIndex === undefined) {
                throw new TransactionError(TransactionErrorCode.ROOT_NOT_FOUND, "getRootIndex", `Root index not found for root${root}`);
            }
            if (merkle_tree_account_data.nextIndex.gte(TRANSACTION_MERKLE_TREE_SWITCH_TRESHOLD)) {
                ...
                const nextTransactionMerkleTreePubkey = MerkleTreeConfig.getTransactionMerkleTreePda(nextTrancactionMerkleTreeIndex);
    ...
    getIndices(utxos) {
        if (!this.params.assetPubkeysCircuit)
            throw new TransactionError(TransactionErrorCode.ASSET_PUBKEYS_UNDEFINED, "getIndices", "");
        let inIndices = [];
        utxos.map((utxo) => {
            let tmpInIndices = [];
    ...
    static getMerkleProofs(provider, inputUtxos) {
        if (!provider.solMerkleTree)
            throw new TransactionError(SolMerkleTreeErrorCode.MERKLE_TREE_UNDEFINED, "getMerkleProofs", "");
            ...
            if (inputUtxo.amounts[0].gt(BN_0) || inputUtxo.amounts[1].gt(BN_0)) {
                ...
                if (inputUtxo.index || inputUtxo.index == 0) {
                    if (inputUtxo.index < 0) {
                        throw new TransactionError(TransactionErrorCode.INPUT_UTXO_NOT_INSERTED_IN_MERKLE_TREE, "getMerkleProofs", `Input commitment ${inputUtxo.getCommitment(provider.poseidon)} was not found. Was the local merkle tree synced since the utxo was inserted?`);
                    }
    ...
    static getSignerAuthorityPda(merkleTreeProgramId, verifierProgramId) {
        return PublicKey.findProgramAddressSync([merkleTreeProgramId.toBytes()], verifierProgramId)[0];
    }
    static getRegisteredVerifierPda(merkleTreeProgramId, verifierProgramId) {
        return PublicKey.findProgramAddressSync([verifierProgramId.toBytes()], merkleTreeProgramId)[0];
    }
    // TODO: evaluate whether we need this function
    ...
    async sendAndConfirmTransaction() {
        ...
        if (this.params.action !== Action.SHIELD) {
            ...
            if (!this.provider.provider)
                throw new TransactionError(ProviderErrorCode.ANCHOR_PROVIDER_UNDEFINED, "sendTransaction", "Provider.provider undefined");
            if (!this.params)
                throw new TransactionError(TransactionErrorCode.TX_PARAMETERS_UNDEFINED, "sendTransaction", "");
            if (!this.params.relayer)
                throw new TransactionError(TransactionErrorCode.RELAYER_UNDEFINED, "sendTransaction", "");
            if (this.transactionInputs.rootIndex === undefined) {
                throw new TransactionError(TransactionErrorCode.ROOT_INDEX_NOT_FETCHED, "sendTransaction", "");
            }
            if (!((_a = this.remainingAccounts) === null || _a === void 0 ? void 0 : _a.leavesPdaPubkeys)) {
                throw new TransactionError(TransactionErrorCode.REMAINING_ACCOUNTS_NOT_CREATED, "sendTransaction", "Run await getPdaAddresses() before invoking sendTransaction");
            }
            response = await sendVersionedTransactions(instructions, this.provider.provider.connection, this.provider.lookUpTables.versionedTransactionLookupTable, this.provider.wallet);
        }
        if (response.error)
    ...
    async getInstructions(params) {
        var _a;
        const verifierProgram = TransactionParameters.getVerifierProgram(params.verifierIdl, this.provider.provider);
        if (!this.transactionInputs.publicInputs)
            throw new TransactionError(TransactionErrorCode.PUBLIC_INPUTS_UNDEFINED, "getInstructions");
        if (!verifierProgram)
            throw new TransactionError(TransactionErrorCode.VERIFIER_PROGRAM_UNDEFINED, "getInstructions");
        ...
        if (this.params.ataCreationFee) {
            if (!this.params.accounts.recipientSpl)
                throw new TransactionError(TransactionErrorCode.SPL_RECIPIENT_UNDEFINED, "getInstructions", "Probably sth in the associated token address generation went wrong");
            if (!this.params.accounts.recipientSol)
                throw new TransactionError(TransactionErrorCode.SPL_RECIPIENT_UNDEFINED, "getInstructions", "Probably sth in the associated token address generation went wrong");
            let ix = createAssociatedTokenAccountInstruction(this.params.relayer.accounts.relayerPubkey, this.params.accounts.recipientSpl, this.params.accounts.recipientSol, MINT);
        ...
        for (let i = 0; i < instructionNames.length; i++) {
            ...
            let inputs = createAccountObject(inputObject, params.verifierIdl.accounts, accountName);
            ...
            let appAccounts = {};
            if ((_a = this.appParams) === null || _a === void 0 ? void 0 : _a.accounts) {
            ...
            instructions === null || instructions === void 0 ? void 0 : instructions.push(ix);
        }
        return instructions;
    }
    ...
    async closeVerifierState() {
        ...
        if (!this.provider.wallet)
            throw new TransactionError(TransactionErrorCode.WALLET_UNDEFINED, "closeVerifierState", "Cannot use closeVerifierState without wallet");
        if (!this.params)
            throw new TransactionError(TransactionErrorCode.TX_PARAMETERS_UNDEFINED, "closeVerifierState", "");
        if (!this.params.verifierIdl)
            throw new TransactionError(TransactionErrorCode.VERIFIER_PROGRAM_UNDEFINED, "closeVerifierState", "");
        if (this.appParams) {
            const transaction = new SolanaTransaction().add(await ((_a = this.appParams) === null || _a === void 0 ? void 0 : _a.verifier.verifierProgram.methods.closeVerifierState().accounts({
                ...this.params.accounts,
            }).instruction()));
            return await this.provider.wallet.sendAndConfirmTransaction(transaction);
        }
        else {
            const transaction = new SolanaTransaction().add(await TransactionParameters.getVerifierProgram((_b = this.params) === null || _b === void 0 ? void 0 : _b.verifierIdl, this.provider.provider)
    ...
    getPdaAddresses() {
        ...
        if (!this.params.verifierIdl)
            throw new TransactionError(TransactionErrorCode.VERIFIER_IDL_UNDEFINED, "getPdaAddresses", "");
        if (!this.params.relayer)
            throw new TransactionError(TransactionErrorCode.RELAYER_UNDEFINED, "getPdaAddresses", "");
        if (!this.remainingAccounts)
            throw new TransactionError(TransactionErrorCode.REMAINING_ACCOUNTS_NOT_CREATED, "getPdaAddresses", "Remaining accounts undefined");
        ...
        for (var i in nullifiers) {
            this.remainingAccounts.nullifierPdaPubkeys.push({
                isSigner: false,
                isWritable: true,
                pubkey: Transaction.getNullifierPdaPublicKey(nullifiers[i], merkleTreeProgramId),
            });
        }
        this.remainingAccounts.leavesPdaPubkeys = [];
        for (var j = 0; j < this.transactionInputs.publicInputs.outputCommitment.length; j += 2) {
            this.remainingAccounts.leavesPdaPubkeys.push({
                ...
                ], merkleTreeProgramId)[0],
            });
        }
        if (this.appParams) {
            this.params.accounts.verifierState = PublicKey.findProgramAddressSync([signer.toBytes(), utils.bytes.utf8.encode("VERIFIER_STATE")], TransactionParameters.getVerifierProgramId(this.appParams.verifierIdl))[0];
        }
        else {
            this.params.accounts.verifierState = PublicKey.findProgramAddressSync([signer.toBytes(), utils.bytes.utf8.encode("VERIFIER_STATE")], this.params.verifierProgramId)[0];
        }
    }
    static getNullifierPdaPublicKey(nullifier, merkleTreeProgramId) {
        return PublicKey.findProgramAddressSync([Uint8Array.from([...nullifier]), utils.bytes.utf8.encode("nf")], merkleTreeProgramId)[0];
    }
    ...
    static getTokenAuthority() {
        return PublicKey.findProgramAddressSync([utils.bytes.utf8.encode("spl")], merkleTreeProgramId)[0];
    }
}
