...
export class MerkleTreeConfig {
    constructor({ payer, connection, }) {
        this.payer = payer;
        this.merkleTreeProgram = new Program(IDL_MERKLE_TREE_PROGRAM, merkleTreeProgramId);
    ...
    async initializeNewTransactionMerkleTree(oldTransactionMerkleTree, newTransactionMerkleTree) {
        ...
        const tx = await this.merkleTreeProgram.methods
            ...
            .signers([this.payer])
            .transaction();
        const txHash = await sendAndConfirmTransaction(this.connection, tx, [this.payer], confirmConfig);
    ...
    async checkTransactionMerkleTreeIsInitialized(transactionMerkleTreePda) {
        ...
        assert(transactionMerkleTreeAccountInfo != null, "merkleTreeAccountInfo not initialized");
        // zero values
        // index == 0
    ...
    getMerkleTreeAuthorityPda() {
        this.merkleTreeAuthorityPda = PublicKey.findProgramAddressSync([anchor.utils.bytes.utf8.encode("MERKLE_TREE_AUTHORITY")], this.merkleTreeProgram.programId)[0];
    ...
    async initMerkleTreeAuthority(authority, transactionMerkleTree) {
        ...
        const tx = await this.merkleTreeProgram.methods
            ...
            .signers([authority])
            .transaction();
        const txHash = await sendAndConfirmTransaction(this.connection, tx, [authority ? authority : this.payer], confirmConfig);
        // assert(
    ...
    async updateMerkleTreeAuthority(newAuthority, test = false) {
        ...
        if (test != true) {
            ...
            assert.equal(merkleTreeAuthority.enablePermissionlessSplTokens, merkleTreeAuthorityPrior.enablePermissionlessSplTokens);
            assert.equal(merkleTreeAuthority.enableNfts, merkleTreeAuthorityPrior.enableNfts);
            assert.equal(merkleTreeAuthority.pubkey.toBase58(), newAuthority.toBase58());
        }
        return txHash;
    }
    ...
    async getRegisteredVerifierPda(verifierPubkey) {
        ...
        this.registeredVerifierPdas.push({
            registeredVerifierPda: PublicKey.findProgramAddressSync([verifierPubkey.toBuffer()], this.merkleTreeProgram.programId)[0],
    ...
    async checkVerifierIsRegistered(verifierPubkey) {
        ...
        assert(registeredVerifierAccountInfo.pubkey.toBase58() ==
    ...
    async getPoolTypePda(poolType) {
        ...
        this.poolTypes.push({
            tokenPdas: [],
            poolPda: PublicKey.findProgramAddressSync([Buffer.from(poolType), anchor.utils.bytes.utf8.encode("pooltype")], this.merkleTreeProgram.programId)[0],
            poolType: poolType,
        });
    ...
    async checkPoolRegistered(poolPda, poolType, mint = null) {
        ...
        assert.equal(registeredTokenConfigAccount.poolType.toString(), poolType.toString());
        assert.equal(registeredTokenConfigAccount.index.toString(), (merkleTreeAuthorityPdaAccountInfo.registeredAssetIndex.toNumber() - 1).toString());
        if (mint !== null) {
            assert.equal(registeredTokenConfigAccount.assetPoolPubkey.toBase58(), poolPda.token.toBase58());
            ...
            assert.equal(registeredTokenConfigAccount.assetPoolPubkey.toBase58(), poolPda.pda.toBase58());
        }
    }
    ...
    async getTokenAuthority() {
        this.tokenAuthority = PublicKey.findProgramAddressSync([anchor.utils.bytes.utf8.encode("spl")], this.merkleTreeProgram.programId)[0];
