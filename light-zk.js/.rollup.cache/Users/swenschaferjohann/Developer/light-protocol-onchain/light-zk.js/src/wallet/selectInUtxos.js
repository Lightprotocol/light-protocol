...
import { CreateUtxoErrorCode, RelayerErrorCode, SelectInUtxosError, SelectInUtxosErrorCode, TransactionErrorCode, Action, getUtxoArrayAmount, TOKEN_REGISTRY, BN_0, } from "../index";
// TODO: turn these into static user.class methods
export const getAmount = (u, asset) => {
    return u.amounts[u.assets.indexOf(asset)];
};
export const getUtxoSum = (utxos, asset) => {
...
const selectBiggestSmallest = (filteredUtxos, assetIndex, sumOutSpl, threshold) => {
    var selectedUtxos = [];
    var selectedUtxosAmount = BN_0;
    var selectedUtxosSolAmount = BN_0;
...
export function selectInUtxos({ utxos, publicMint, publicAmountSpl, publicAmountSol, poseidon, relayerFee, inUtxos, outUtxos = [], action, numberMaxInUtxos, numberMaxOutUtxos, }) {
    var _a, _b;
    if (!publicMint && publicAmountSpl)
        throw new SelectInUtxosError(CreateUtxoErrorCode.NO_PUBLIC_MINT_PROVIDED, "selectInUtxos", "Public mint not set but public spl amount");
    if (publicMint && !publicAmountSpl)
        throw new SelectInUtxosError(CreateUtxoErrorCode.PUBLIC_SPL_AMOUNT_UNDEFINED, "selectInUtxos", "Public spl amount not set but public mint");
    if (action === Action.UNSHIELD && !publicAmountSpl && !publicAmountSol)
        throw new SelectInUtxosError(CreateUtxoErrorCode.NO_PUBLIC_AMOUNTS_PROVIDED, "selectInUtxos", "No public amounts defined");
    if (action === Action.UNSHIELD && !relayerFee)
        throw new SelectInUtxosError(RelayerErrorCode.RELAYER_FEE_UNDEFINED, "selectInUtxos", "Relayer fee undefined");
    if (action === Action.TRANSFER && !relayerFee)
        throw new SelectInUtxosError(RelayerErrorCode.RELAYER_FEE_UNDEFINED, "selectInUtxos", "Relayer fee undefined");
    if ((!utxos || utxos.length === 0) && action === Action.SHIELD)
        return [];
    else if (!utxos || utxos.length === 0)
        throw new SelectInUtxosError(TransactionErrorCode.NO_UTXOS_PROVIDED, "selectInUtxos", `No utxos defined for ${action}`);
    if (action === Action.SHIELD && relayerFee)
        throw new SelectInUtxosError(CreateUtxoErrorCode.RELAYER_FEE_DEFINED, "selectInUtxos", "Relayer fee should not be defined with shield");
    ...
    if (outUtxos.length > numberMaxOutUtxos - 1)
        throw new SelectInUtxosError(CreateUtxoErrorCode.INVALID_NUMER_OF_RECIPIENTS, "selectInUtxos", `outUtxos.length ${outUtxos.length}`);
    // check publicMint and recipients mints are all the same
    let mint = publicMint;
    for (var utxo of outUtxos) {
        if (!mint && ((_a = utxo.amounts[1]) === null || _a === void 0 ? void 0 : _a.gt(BN_0)))
            mint = utxo.assets[1];
        if (mint && mint.toBase58() !== utxo.assets[1].toBase58())
            throw new SelectInUtxosError(SelectInUtxosErrorCode.INVALID_NUMER_OF_MINTS, "selectInUtxos", `Too many different mints in recipients outUtxos ${utxo}`);
    }
    ...
    if (numberMaxInUtxos - selectedUtxosR.length < 0)
        throw new SelectInUtxosError(SelectInUtxosErrorCode.INVALID_NUMBER_OF_IN_UTXOS, "selectInUtxos");
    if (mint && mint != ((_b = TOKEN_REGISTRY.get("SOL")) === null || _b === void 0 ? void 0 : _b.mint)) {
        ...
        if (selectedUtxos.length === 0)
            throw new SelectInUtxosError(SelectInUtxosErrorCode.FAILED_TO_FIND_UTXO_COMBINATION, "selectInUtxos", `Failed to find any utxo of this token${publicMint}`);
        ...
        if (sumOutSol.gt(selectedUtxosSolAmount)) {
            ...
            // take utxo with smallest spl amount of utxos which satisfy
                ...
                throw new SelectInUtxosError(SelectInUtxosErrorCode.FAILED_TO_FIND_UTXO_COMBINATION, "selectInUtxos", `Could not find a utxo combination for spl token ${mint} and amount ${sumOutSpl} available ${getUtxoArrayAmount(mint, selectedUtxosR)} and sol amount ${sumOutSol} available ${getUtxoArrayAmount(SystemProgram.programId, selectedUtxosR)}`);
            }
            else {
    ...
    if (mint && !getUtxoArrayAmount(mint, selectedUtxosR).gte(sumOutSpl))
        throw new SelectInUtxosError(SelectInUtxosErrorCode.FAILED_TO_FIND_UTXO_COMBINATION, "selectInUtxos", `Failed to get spl amount requested ${sumOutSpl} possible ${getUtxoArrayAmount(mint, selectedUtxosR)}`);
    if (!getUtxoArrayAmount(SystemProgram.programId, selectedUtxosR).gte(sumOutSol))
        throw new SelectInUtxosError(SelectInUtxosErrorCode.FAILED_TO_FIND_UTXO_COMBINATION, "selectInUtxos", `Failed to get sol amount requested ${sumOutSol} possible ${getUtxoArrayAmount(SystemProgram.programId, selectedUtxosR)}`);
    if (selectedUtxosR.length > 0)
        return selectedUtxosR;
    else if (action === Action.SHIELD)
        return [];
    else
        throw new SelectInUtxosError(SelectInUtxosErrorCode.FAILED_TO_FIND_UTXO_COMBINATION, "selectInUtxos", "selectInUtxos failed to select utxos");
}
