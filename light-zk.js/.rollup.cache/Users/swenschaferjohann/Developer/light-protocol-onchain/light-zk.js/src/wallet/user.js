...
import { CreateUtxoErrorCode, UtxoErrorCode, ProviderErrorCode, RelayerErrorCode, TransactionErrorCode, TransactionParametersErrorCode, UserError, UserErrorCode, SIGN_MESSAGE, AUTHORITY, SelectInUtxosErrorCode, TOKEN_REGISTRY, Account, Utxo, convertAndComputeDecimals, Transaction, TransactionParameters, Action, createRecipientUtxos, TokenUtxoBalance, NACL_ENCRYPTED_COMPRESSED_UTXO_BYTES_LENGTH, decryptAddUtxoToBalance, fetchNullifierAccountInfo, getUserIndexTransactions, IDL_VERIFIER_PROGRAM_ZERO, IDL_VERIFIER_PROGRAM_ONE, MAX_MESSAGE_SIZE, IDL_VERIFIER_PROGRAM_STORAGE, AccountErrorCode, ProgramUtxoBalance, TOKEN_PUBKEY_SYMBOL, UtxoError, IDL_VERIFIER_PROGRAM_TWO, isProgramVerifier, decimalConversion, MerkleTreeConfig, BN_0, } from "../index";
import { bs58 } from "@coral-xyz/anchor/dist/cjs/utils/bytes";
// TODO: Utxos should be assigned to a merkle tree
export var ConfirmOptions;
 ...
 */
export class User {
    constructor({ provider, account, appUtxoConfig, verifierIdl = IDL_VERIFIER_PROGRAM_ZERO, }) {
        if (!provider.wallet)
            throw new UserError(UserErrorCode.NO_WALLET_PROVIDED, "constructor", "No wallet provided");
        if (!provider.lookUpTables.verifierProgramLookupTable ||
            !provider.solMerkleTree ||
            !provider.poseidon)
            throw new UserError(UserErrorCode.PROVIDER_NOT_INITIALIZED, "constructor", "Provider not properly initialized");
        this.provider = provider;
        this.account = account;
        if (appUtxoConfig && !isProgramVerifier(verifierIdl))
            throw new UserError(UserErrorCode.VERIFIER_IS_NOT_APP_ENABLED, "constructor", `appUtxo config is provided but there is no app enabled verifier defined. The defined verifier is ${verifierIdl.name}.`);
    ...
    async syncState(aes = true, balance, merkleTreePdaPublicKey) {
        ...
        if (!this.provider.provider)
            throw new UserError(UserErrorCode.PROVIDER_NOT_INITIALIZED, "syncState", "provider is undefined");
        // identify spent utxos
        for (var [, tokenBalance] of balance.tokenBalances) {
            for (var [key, utxo] of tokenBalance.utxos) {
                let nullifierAccountInfo = await fetchNullifierAccountInfo(utxo.getNullifier(this.provider.poseidon), this.provider.provider.connection);
        ...
        for (const trx of indexedTransactions) {
            ...
            for (let index = 0; index < trx.leaves.length; index += 2) {
                ...
                await decryptAddUtxoToBalance({
                    encBytes: Buffer.from(trx.encryptedUtxos.slice((index / 2) * 240, (index / 2) * 240 + NACL_ENCRYPTED_COMPRESSED_UTXO_BYTES_LENGTH)),
                    index: leftLeafIndex,
        ...
        this.transactionHistory = await getUserIndexTransactions(indexedTransactions, this.provider, this.balance.tokenBalances);
    ...
    async getUtxoInbox(latest = true) {
        if (latest) {
            await this.syncState(false, this.inboxBalance, MerkleTreeConfig.getTransactionMerkleTreePda());
        }
        return this.inboxBalance;
    }
    async getBalance(latest = true) {
        if (!this.account)
            throw new UserError(UserErrorCode.UTXOS_NOT_INITIALIZED, "getBalances", "Account not initialized");
        if (!this.provider)
            throw new UserError(UserErrorCode.USER_ACCOUNT_NOT_INITIALIZED, "Provider not initialized");
        if (!this.provider.poseidon)
            throw new UserError(TransactionParametersErrorCode.NO_POSEIDON_HASHER_PROVIDED, "Poseidon not initialized");
        if (!this.provider.solMerkleTree)
            throw new UserError(ProviderErrorCode.SOL_MERKLE_TREE_UNDEFINED, "getBalance", "Merkle Tree not initialized");
        if (!this.provider.lookUpTables.verifierProgramLookupTable)
            throw new UserError(RelayerErrorCode.LOOK_UP_TABLE_UNDEFINED, "getBalance", "Look up table not initialized");
        if (latest) {
            await this.syncState(true, this.balance, MerkleTreeConfig.getTransactionMerkleTreePda());
        }
        return this.balance;
    }
    /**
    ...
    async createShieldTransactionParameters({ token, publicAmountSpl, recipient, publicAmountSol, senderTokenAccount, minimumLamports = true, appUtxo, mergeExistingUtxos = true, verifierIdl, message, skipDecimalConversions = false, utxo, }) {
        ...
        if (publicAmountSpl && token === "SOL")
            throw new UserError(UserErrorCode.INVALID_TOKEN, "shield", "No public amount provided. Shield needs a public amount.");
        if (!publicAmountSpl && !publicAmountSol)
            throw new UserError(CreateUtxoErrorCode.NO_PUBLIC_AMOUNTS_PROVIDED, "shield", "No public amounts provided. Shield needs a public amount.");
        if (publicAmountSpl && !token)
            throw new UserError(UserErrorCode.TOKEN_UNDEFINED, "shield", "No public amounts provided. Shield needs a public amount.");
        if (!this.provider)
            throw new UserError(UserErrorCode.PROVIDER_NOT_INITIALIZED, "shield", "Provider not set!");
        let tokenCtx = TOKEN_REGISTRY.get(token);
        if (!tokenCtx)
            throw new UserError(UserErrorCode.TOKEN_NOT_FOUND, "shield", "Token not supported!");
        if (tokenCtx.isNative && senderTokenAccount)
            throw new UserError(UserErrorCode.TOKEN_ACCOUNT_DEFINED, "shield", "Cannot use senderTokenAccount for SOL!");
    ...
    async compileAndProveTransaction(appParams, shuffleEnabled = true) {
        if (!this.recentTransactionParameters)
            throw new UserError(UserErrorCode.TRANSACTION_PARAMTERS_UNDEFINED, "compileAndProveTransaction", "The method 'createShieldTransactionParameters' must be executed first to generate the parameters that can be compiled and proven.");
    ...
    async approve() {
        ...
        if (!this.recentTransactionParameters)
            throw new UserError(UserErrorCode.TRANSACTION_PARAMTERS_UNDEFINED, "compileAndProveTransaction", "The method 'createShieldTransactionParameters' must be executed first to approve SPL funds before initiating a shield transaction.");
        if (((_a = this.recentTransactionParameters) === null || _a === void 0 ? void 0 : _a.publicAmountSpl.gt(BN_0)) &&
            ((_b = this.recentTransactionParameters) === null || _b === void 0 ? void 0 : _b.action) === Action.SHIELD) {
            ...
            if (!tokenAccountInfo)
                throw new UserError(UserErrorCode.ASSOCIATED_TOKEN_ACCOUNT_DOESNT_EXIST, "shield", "AssociatdTokenAccount doesn't exist!");
            ...
            if ((_e = this.recentTransactionParameters) === null || _e === void 0 ? void 0 : _e.publicAmountSpl.gt(new BN(tokenBalance.amount.toString())))
                throw new UserError(UserErrorCode.INSUFFICIENT_BAlANCE, "shield", `Insufficient token balance! ${(_f = this.recentTransactionParameters) === null || _f === void 0 ? void 0 : _f.publicAmountSpl.toString()} bal: ${tokenBalance
                    .amount}`);
            try {
                const transaction = new SolanaTransaction().add(splToken.createApproveInstruction(this.recentTransactionParameters.accounts.senderSpl, AUTHORITY, this.provider.wallet.publicKey, (_g = this.recentTransactionParameters) === null || _g === void 0 ? void 0 : _g.publicAmountSpl.toNumber()));
                ...
                throw new UserError(UserErrorCode.APPROVE_ERROR, "shield", `Error approving token transfer! ${e.stack}`);
            }
        }
        else {
            this.approved = true;
    ...
    async sendTransaction() {
        var _a;
        if (!this.recentTransactionParameters)
            throw new UserError(UserErrorCode.TRANSACTION_PARAMTERS_UNDEFINED, "sendTransaction", "Unable to send transaction. The transaction must be compiled and a proof must be generated first.");
        if (((_a = this.recentTransactionParameters) === null || _a === void 0 ? void 0 : _a.action) === Action.SHIELD &&
            !this.approved)
            throw new UserError(UserErrorCode.SPL_FUNDS_NOT_APPROVED, "sendTransaction", "Please approve SPL funds before executing a shield with SPL tokens.");
        if (!this.recentTransaction)
            throw new UserError(UserErrorCode.TRANSACTION_UNDEFINED, "sendTransaction", "Unable to send transaction. The transaction must be compiled and a proof must be generated first.");
        let txResult;
        try {
            txResult = await this.recentTransaction.sendAndConfirmTransaction();
        }
        catch (e) {
            throw new UserError(TransactionErrorCode.SEND_TRANSACTION_FAILED, "shield", `Error in tx.sendTransaction ${e}`);
        }
        return txResult;
    }
    resetTxState() {
    ...
    async unshield({ token, publicAmountSpl, publicAmountSol, recipient = AUTHORITY, minimumLamports = true, confirmOptions = ConfirmOptions.spendable, }) {
     ...
     */
    async createUnshieldTransactionParameters({ token, publicAmountSpl, publicAmountSol, recipient = AUTHORITY, minimumLamports = true, }) {
        var _a, _b, _c;
        const tokenCtx = TOKEN_REGISTRY.get(token);
        if (!tokenCtx)
            throw new UserError(UserErrorCode.TOKEN_NOT_FOUND, "unshield", "Token not supported!");
        if (!publicAmountSpl && !publicAmountSol)
            throw new UserError(CreateUtxoErrorCode.NO_PUBLIC_AMOUNTS_PROVIDED, "unshield", "Need to provide at least one amount for an unshield");
        if (publicAmountSol && recipient.toBase58() == AUTHORITY.toBase58())
            throw new UserError(TransactionErrorCode.SOL_RECIPIENT_UNDEFINED, "getTxParams", "Please provide a recipient for unshielding SOL");
        if (publicAmountSpl && recipient.toBase58() == AUTHORITY.toBase58())
            throw new UserError(TransactionErrorCode.SPL_RECIPIENT_UNDEFINED, "getTxParams", "Please provide a recipient for unshielding SPL");
        if (publicAmountSpl && token == "SOL")
            throw new UserError(UserErrorCode.INVALID_TOKEN, "getTxParams", "public amount spl provided for SOL token");
        ...
        if (publicAmountSpl) {
            _publicSplAmount = convertAndComputeDecimals(publicAmountSpl, tokenCtx.decimals);
        }
        ...
        let solUtxos = (_c = this.balance.tokenBalances
            .get(SystemProgram.programId.toBase58())) === null || _c === void 0 ? void 0 : _c.utxos.values();
    ...
    async transfer({ token, recipient, amountSpl, amountSol, appUtxo, confirmOptions = ConfirmOptions.spendable, }) {
        if (!recipient)
            throw new UserError(UserErrorCode.SHIELDED_RECIPIENT_UNDEFINED, "transfer", "Please provide a shielded recipient for the transfer.");
        let recipientAccount = Account.fromPubkey(recipient, this.provider.poseidon);
    ...
    async createTransferTransactionParameters({ token, recipient, amountSpl, amountSol, appUtxo, message, inUtxos, outUtxos, verifierIdl, skipDecimalConversions, addInUtxos = true, addOutUtxos = true, }) {
        ...
        if (!amountSol && !amountSpl && !outUtxos && !inUtxos)
            throw new UserError(UserErrorCode.NO_AMOUNTS_PROVIDED, "createTransferTransactionParameters", "At least one amount should be provided for a transfer.");
        ...
        if (!token)
            throw new UserError(UserErrorCode.TOKEN_UNDEFINED, "createTransferTransactionParameters");
        const tokenCtx = TOKEN_REGISTRY.get(token);
        if (!tokenCtx)
            throw new UserError(UserErrorCode.TOKEN_NOT_FOUND, "createTransferTransactionParameters", "Token not supported!");
        const convertedPublicAmounts = decimalConversion({
            tokenCtx,
            skipDecimalConversions,
        ...
        if (recipient && !tokenCtx)
            throw new UserError(UserErrorCode.SHIELDED_RECIPIENT_UNDEFINED, "createTransferTransactionParameters");
        let _outUtxos = [];
        if (recipient) {
        ...
        let solUtxos = (_a = this.balance.tokenBalances
            .get(SystemProgram.programId.toBase58())) === null || _a === void 0 ? void 0 : _a.utxos.values();
        ...
        try {
            if (!seed) {
                if (provider.wallet) {
                    ...
                    throw new UserError(UserErrorCode.NO_WALLET_PROVIDED, "load", "No payer or browser wallet provided");
                }
            }
            ...
            throw new UserError(UserErrorCode.LOAD_ERROR, "load", `Error while loading user! ${e}`);
        }
    }
    ...
    async mergeAllUtxos(asset, confirmOptions = ConfirmOptions.spendable, latest = true) {
        ...
        if (!inboxTokenBalance)
            throw new UserError(UserErrorCode.EMPTY_INBOX, "mergeAllUtxos", `for asset ${asset} the utxo inbox is empty`);
        let utxosEntries = (_a = this.balance.tokenBalances
            .get(asset.toBase58())) === null || _a === void 0 ? void 0 : _a.utxos.values();
        let _solUtxos = (_b = this.balance.tokenBalances
            .get(new PublicKey(0).toBase58())) === null || _b === void 0 ? void 0 : _b.utxos.values();
        ...
        if (inboxUtxosEntries.length == 0)
            throw new UserError(UserErrorCode.EMPTY_INBOX, "mergeAllUtxos", `for asset ${asset} the utxo inbox is empty`);
        let assetIndex = asset.toBase58() === SystemProgram.programId.toBase58() ? 0 : 1;
        // sort inbox utxos descending
        inboxUtxosEntries.sort((a, b) => b.amounts[assetIndex].toNumber() - a.amounts[assetIndex].toNumber());
        let inUtxos = utxosEntries && asset.toBase58() !== new PublicKey(0).toBase58()
    ...
    async mergeUtxos(commitments, asset, confirmOptions = ConfirmOptions.spendable, latest = false) {
        ...
        if (commitments.length == 0)
            throw new UserError(UserErrorCode.NO_COMMITMENTS_PROVIDED, "mergeAllUtxos", `No commitmtents for merging specified ${asset}`);
        await this.getUtxoInbox(latest);
        await this.getBalance(latest);
        let inboxTokenBalance = this.inboxBalance.tokenBalances.get(asset.toString());
        if (!inboxTokenBalance)
        ...
        for (var commitment of commitments) {
            ...
            if (!utxo)
                throw new UserError(UserErrorCode.COMMITMENT_NOT_FOUND, "mergeUtxos", `commitment ${commitment} is it of asset ${asset} ?`);
        ...
        if (inUtxos.length > 10) {
            throw new UserError(UserErrorCode.TOO_MANY_COMMITMENTS, "mergeUtxos", `too many commitments provided to merge at once provided ${commitmentUtxos.length}, number of existing utxos ${Array.from(utxosEntries ? utxosEntries : []).length} > 10 (can only merge 10 utxos in one transaction)`);
        }
        let txParams = await TransactionParameters.getTxParams({
            tokenCtx: inboxTokenBalance.tokenData,
            action: Action.TRANSFER,
    ...
    async getTransactionHistory(latest = true) {
        try {
            ...
            throw new UserError(TransactionErrorCode.GET_USER_TRANSACTION_HISTORY_FAILED, "getLatestTransactionHistory", `Error while getting user transaction history ! ${error}`);
        }
    }
    ...
    async createStoreAppUtxoTransactionParameters({ token, amountSol, amountSpl, minimumLamports, senderTokenAccount, recipientPublicKey, appUtxo, stringUtxo, action, appUtxoConfig, skipDecimalConversions = false, }) {
        if (!appUtxo) {
            if (appUtxoConfig) {
                if (!token)
                    throw new UserError(UserErrorCode.TOKEN_UNDEFINED, "createStoreAppUtxoTransactionParameters");
                if (!amountSol)
                    throw new UserError(CreateUtxoErrorCode.PUBLIC_SOL_AMOUNT_UNDEFINED, "createStoreAppUtxoTransactionParameters");
                if (!amountSpl)
                    throw new UserError(CreateUtxoErrorCode.PUBLIC_SPL_AMOUNT_UNDEFINED, "createStoreAppUtxoTransactionParameters");
                const tokenCtx = TOKEN_REGISTRY.get(token);
                if (!tokenCtx)
                    throw new UserError(UserErrorCode.INVALID_TOKEN, "createStoreAppUtxoTransactionParameters");
                appUtxo = new Utxo({
                    poseidon: this.provider.poseidon,
                ...
                appUtxo = Utxo.fromString(stringUtxo, this.provider.poseidon, this.provider.lookUpTables.assetLookupTable, this.provider.lookUpTables.verifierProgramLookupTable);
            }
            else {
                throw new UserError(UserErrorCode.APP_UTXO_UNDEFINED, "createStoreAppUtxoTransactionParameters", "invalid parameters to generate app utxo");
            }
        }
        else {
        ...
        if (!appUtxo)
            throw new UserError(UserErrorCode.APP_UTXO_UNDEFINED, "createStoreAppUtxoTransactionParameters", `app utxo is undefined or could not generate one from provided parameters`);
        if (!token) {
        ...
        if (!token)
            throw new UserError(UserErrorCode.TOKEN_UNDEFINED, "createStoreAppUtxoTransactionParameters");
        const message = Buffer.from(await appUtxo.encrypt(this.provider.poseidon, MerkleTreeConfig.getEventMerkleTreePda(), false));
        if (message.length > MAX_MESSAGE_SIZE)
            throw new UserError(UserErrorCode.MAX_STORAGE_MESSAGE_SIZE_EXCEEDED, "storeData", `${message.length}/${MAX_MESSAGE_SIZE}`);
    ...
    async syncStorage(idl, aes = true) {
                ...
                for (var [, leaf] of data.leaves.entries()) {
                    try {
                        ...
                        if (decryptedUtxo !== null) {
                            ...
                            const nfExists = await fetchNullifierAccountInfo(decryptedUtxo.getNullifier(this.provider.poseidon), (_a = this.provider.provider) === null || _a === void 0 ? void 0 : _a.connection);
                            if (!nfExists) {
        ...
        for (var utxo of decryptedStorageUtxos) {
            ...
            if (!this.balance.programBalances.get(verifierAddress)) {
                this.balance.programBalances.set(verifierAddress, new ProgramUtxoBalance(utxo.verifierAddress, idl));
            }
    ...
    async storeData(message, confirmOptions = ConfirmOptions.spendable, shield = false) {
        ...
        if (shield) {
            ...
            const firstSolUtxo = (_a = this.balance.tokenBalances
                .get(SystemProgram.programId.toBase58())) === null || _a === void 0 ? void 0 : _a.utxos.values().next().value;
            if (firstSolUtxo) {
            ...
            if (inUtxos.length === 0 || inUtxos[0] === undefined)
                throw new UserError(SelectInUtxosErrorCode.FAILED_TO_SELECT_SOL_UTXO, "storeData");
    ...
    async executeAppUtxo({ appUtxos = [], inUtxos = [], outUtxos, action, programParameters, confirmOptions, addInUtxos = false, addOutUtxos, shuffleEnabled = false, }) {
        if (!programParameters.verifierIdl)
            throw new UserError(UtxoErrorCode.APP_DATA_IDL_UNDEFINED, "executeAppUtxo", `provided program parameters: ${programParameters}`);
