...
import { VERIFIER_PUBLIC_KEYS, MAX_U64, FIELD_SIZE, merkleTreeProgramId, BN_0, } from "../constants";
 ...
 */
export const getUserIndexTransactions = async (indexedTransactions, provider, tokenBalances) => {
    ...
    indexedTransactions.forEach((trx) => {
        ...
        const inSpentUtxos = [];
        const outSpentUtxos = [];
        spentUtxos === null || spentUtxos === void 0 ? void 0 : spentUtxos.forEach((sUtxo) => {
...
export const findMatchingInstruction = (instructions, publicKeys) => {
...
async function enrichParsedTransactionEvents(event, transactions) {
    ...
    let instruction = findMatchingInstruction(tx.transaction.message.instructions, VERIFIER_PUBLIC_KEYS);
    ...
    for (let innerInstruction of tx.meta.innerInstructions) {
        if (!instruction)
            instruction = findMatchingInstruction(innerInstruction.instructions, VERIFIER_PUBLIC_KEYS);
    }
    if (!instruction)
        return;
    ...
    let changeSolAmount = new BN(tx.meta.postBalances[solTokenPoolIndex] -
...
async function getTransactionsBatch({ connection, merkleTreeProgramId, batchOptions, transactions, }) {
    const signatures = await connection.getConfirmedSignaturesForAddress2(new PublicKey(merkleTreeProgramId), batchOptions, "confirmed");
 ...
 */
export async function fetchRecentTransactions({ connection, batchOptions = {
    limit: 1,
    before: undefined,
    until: undefined,
}, transactions = [], }) {
    ...
    return transactions.sort((a, b) => new BN(a.firstLeafIndex, "hex").toNumber() -
