...
import { checkMerkleTreeUpdateStateCreated, checkMerkleTreeBatchUpdateSuccess, } from "../test-utils/testChecks";
import { confirmConfig, DEFAULT_PROGRAMS, sleep, } from "../index";
import { ComputeBudgetProgram, PublicKey, Transaction, SystemProgram, sendAndConfirmTransaction, } from "@solana/web3.js";
export async function executeUpdateMerkleTreeTransactions({ signer, merkleTreeProgram, leavesPdas, transactionMerkleTree, connection, }) {
    var merkleTreeAccountPrior = await merkleTreeProgram.account.transactionMerkleTree.fetch(transactionMerkleTree);
    let merkleTreeUpdateState = (await PublicKey.findProgramAddressSync([
...
const sendAndConfirmTransactions = async (transactions, signer, connection) => {
    ...
    await Promise.all(transactions.map(async (tx) => {
        try {
            await sendAndConfirmTransaction(connection, tx, [signer], confirmConfig);
        }
        catch (err) {
            errors.push(err);
        }
 ...
 */
export async function executeMerkleTreeUpdateTransactions({ merkleTreeProgram, merkleTreeUpdateState, transactionMerkleTree, signer, connection, numberOfTransactions = 56 / 2, interrupt = false, }) {
    var counter = { value: 0 };
    var error = undefined;
