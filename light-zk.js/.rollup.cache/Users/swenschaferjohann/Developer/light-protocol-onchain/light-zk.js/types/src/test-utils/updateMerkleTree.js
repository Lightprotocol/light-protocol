...
import { executeUpdateMerkleTreeTransactions, MerkleTreeConfig, SolMerkleTree, } from "../merkleTree/index";
import { confirmConfig, merkleTreeProgramId } from "../constants";
import { IDL_MERKLE_TREE_PROGRAM } from "../idls/index";
import { Connection, Keypair } from "@solana/web3.js";
import { sleep } from "../index";
export async function updateMerkleTreeForTest(payer, url) {
    ...
    new anchor.Wallet(Keypair.generate()), confirmConfig);
    try {
        const merkleTreeProgram = new anchor.Program(IDL_MERKLE_TREE_PROGRAM, merkleTreeProgramId, anchorProvider && anchorProvider);
        ...
        while (leavesPdas.length === 0 && retries > 0) {
            if (retries !== 5)
                await sleep(1000);
            leavesPdas = await SolMerkleTree.getUninsertedLeavesRelayer(transactionMerkleTreePda, anchorProvider && anchorProvider);
            retries--;
        }
