...
import { CreateUtxoError, CreateUtxoErrorCode, TransactionErrorCode, TransactionParametersErrorCode, Action, Utxo, TransactionParameters, MINIMUM_LAMPORTS, BN_0, BN_1, BN_2, } from "../index";
// mint: PublicKey, expectedAmount: BN,
export const getUtxoArrayAmount = (mint, inUtxos) => {
    let inAmount = BN_0;
...
export const getRecipientsAmount = (mint, recipients) => {
    if (mint.toBase58() === SystemProgram.programId.toBase58()) {
        return recipients.reduce((sum, recipient) => sum.add(recipient.solAmount), BN_0);
    }
    else {
...
// --------------------------------------------------------------------------
export function createOutUtxos({ poseidon, inUtxos, outUtxos = [], publicMint, publicAmountSpl, publicAmountSol, relayerFee, changeUtxoAccount, action, appUtxo, numberMaxOutUtxos, assetLookupTable, verifierProgramLookupTable, separateSolUtxo = false, }) {
    var _a, _b;
    if (!poseidon)
        throw new CreateUtxoError(TransactionParametersErrorCode.NO_POSEIDON_HASHER_PROVIDED, "createOutUtxos", "Poseidon not initialized");
    if (relayerFee) {
        publicAmountSol = publicAmountSol
            ? publicAmountSol.add(relayerFee)
            : relayerFee;
    }
    const assetPubkeys = !inUtxos && action === Action.SHIELD
        ? [
    ...
    if (!assetPubkeys)
        throw new CreateUtxoError(TransactionErrorCode.ASSET_PUBKEYS_UNDEFINED, "constructor");
    ...
    if (outUtxos.length > numberMaxOutUtxos - 1) {
        throw new CreateUtxoError(CreateUtxoErrorCode.INVALID_NUMER_OF_RECIPIENTS, "createOutUtxos", `Number of recipients greater than allowed: ${outUtxos.length} allowed ${1}`);
    }
    outUtxos.map((outUtxo) => {
        if (!assetPubkeys.find((x) => { var _a; return x.toBase58() === ((_a = outUtxo.assets[1]) === null || _a === void 0 ? void 0 : _a.toBase58()); })) {
            throw new CreateUtxoError(CreateUtxoErrorCode.INVALID_RECIPIENT_MINT, "createOutUtxos", `Mint ${outUtxo.assets[1]} does not exist in input utxos mints ${assetPubkeys}`);
        }
    });
    ...
    let publicSolAssetIndex = assets.findIndex((x) => x.asset.toBase58() === SystemProgram.programId.toBase58());
    // remove duplicates
    const key = "asset";
    ...
    if (action === Action.UNSHIELD) {
        if (!publicAmountSol && !publicAmountSpl)
            throw new CreateUtxoError(CreateUtxoErrorCode.NO_PUBLIC_AMOUNTS_PROVIDED, "createOutUtxos", "publicAmountSol not initialized for unshield");
        if (!publicAmountSpl)
            publicAmountSpl = BN_0;
        if (!publicAmountSol)
            throw new CreateUtxoError(CreateUtxoErrorCode.PUBLIC_SOL_AMOUNT_UNDEFINED, "constructor");
        if (publicAmountSpl && !publicMint)
            throw new CreateUtxoError(CreateUtxoErrorCode.NO_PUBLIC_MINT_PROVIDED, "createOutUtxos", "publicMint not initialized for unshield");
        ...
        if (relayerFee)
            throw new CreateUtxoError(CreateUtxoErrorCode.RELAYER_FEE_DEFINED, "createOutUtxos", "Shield and relayer fee defined");
        if (!publicAmountSpl)
            publicAmountSpl = BN_0;
        if (!publicAmountSol)
    ...
    for (var j in outUtxos) {
        if (outUtxos[j].assets[1] && !outUtxos[j].amounts[1]) {
            throw new CreateUtxoError(CreateUtxoErrorCode.SPL_AMOUNT_UNDEFINED, "createOutUtxos", `Mint defined while splAmount is undefinedfor recipient ${outUtxos[j]}`);
        }
    ...
    if (separateSolUtxo && publicSplAssets.length > 0) {
         ...
         */
        const preliminarySolAmount = assets[publicSolAssetIndex].sumIn.sub(MINIMUM_LAMPORTS.mul(BN_2));
    ...
    for (var x = 0; x < nrOutUtxos; x++) {
        ...
        let splAmount = ((_a = publicSplAssets[x]) === null || _a === void 0 ? void 0 : _a.sumIn) ? publicSplAssets[x].sumIn : BN_0;
    ...
    if (outputUtxos.length > numberMaxOutUtxos) {
        throw new CreateUtxoError(CreateUtxoErrorCode.INVALID_OUTPUT_UTXO_LENGTH, "createOutUtxos", `Probably too many input assets possibly in combination with an incompatible number of shielded recipients`);
    }
    return outputUtxos;
}
/**
 ...
 */
export function createRecipientUtxos({ recipients, poseidon, assetLookupTable, verifierProgramLookupTable, }) {
    var _a, _b, _c, _d;
    var outputUtxos = [];
    // create recipient output utxos, one for each defined recipient
    for (var j in recipients) {
        if (recipients[j].mint && !recipients[j].splAmount) {
            throw new CreateUtxoError(CreateUtxoErrorCode.SPL_AMOUNT_UNDEFINED, "createOutUtxos", `Mint defined while splAmount is undefined for recipient ${recipients[j]}`);
        }
 ...
 */
export function validateUtxoAmounts({ assetPubkeys, inUtxos, outUtxos, publicAmountSol, publicAmountSpl, action, }) {
    ...
    for (const [index, assetPubkey] of assetPubkeys.entries()) {
        ...
        var sumOut = action === Action.TRANSFER && outUtxos.length === 0
            ? sumIn
            : getUtxoArrayAmount(assetPubkey, outUtxos);
        var sumInAdd = assetPubkey.toBase58() === SystemProgram.programId.toBase58()
            ? sumIn.add(_publicAmountSol)
            : index < 2
                ? sumIn.add(_publicAmountSpl)
                : sumIn;
        var sumOutAdd = assetPubkey.toBase58() === SystemProgram.programId.toBase58()
        ...
        if (sumInAdd.lt(BN_0))
            throw new CreateUtxoError(CreateUtxoErrorCode.RECIPIENTS_SUM_AMOUNT_MISSMATCH, "validateUtxoAmounts", `utxos don't cover the required amount for asset ${assetPubkey.toBase58()} sumIn ${sumIn}  public amount: ${assetPubkey.toBase58() === SystemProgram.programId.toBase58()
                ? publicAmountSol
                : publicAmountSpl} action: ${action}`);
        if (!sumInAdd.gte(sumOutAdd)) {
            throw new CreateUtxoError(CreateUtxoErrorCode.RECIPIENTS_SUM_AMOUNT_MISSMATCH, "validateUtxoAmounts", `for asset ${assetPubkey.toBase58()} sumOut ${sumOut} greather than sumIN ${sumIn}`);
        }
    }
    return assets;
}
