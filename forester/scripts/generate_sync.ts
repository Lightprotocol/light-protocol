...
async function createCompressedAccount(rpc: Rpc, payer: Signer) {
  ...
  console.log("compress tx sig", sig);
}
...
async function performCreateAddresses(
  rpc: Rpc,
  payer: Signer,
  bobPublicKey: PublicKey
) {
  for (
    ...
    const createAccountPromises = Array.from(
      { length: NUMBER_OF_CONCURRENT_TRANSFERS },
      (_, j) => createAccountAsync(i + j, rpc, payer, bobPublicKey)
    );
    await Promise.all(createAccountPromises);
  }
}

async function runPrefillLoop() {
  while (true) {
    try {
