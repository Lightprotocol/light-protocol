...
async fn test_indexer() {
    ...
    let config = Config {
        ...
        merkle_tree_pubkey: env_accounts.merkle_tree_pubkey,
    ...
    assert_ne!(get_queue_length(&config), 0);
    info!(
        "Nullifying queue of {} accounts...",
        get_queue_length(&config)
    );
    let _ = nullify(indexer, &config).await;
    assert_eq!(get_queue_length(&config), 0);
}

fn get_queue_length(config: &Config) -> usize {
    let client = RpcClient::new(SERVER_URL);
