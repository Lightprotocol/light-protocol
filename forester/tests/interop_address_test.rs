...
>(
    ...
    trees: &[Pubkey],
    addresses: &[Pubkey],
    photon_indexer: &PhotonIndexer<INDEXED_ARRAY_SIZE, R>,
) {
...
async fn test_photon_interop_address() {
    ...
    let mut env =
        ...
        .await;
    let rpc = SolanaRpcConnection::new(forester_config.external_services.rpc_url, None);
    let photon_indexer = PhotonIndexer::<500, SolanaRpcConnection>::new(
