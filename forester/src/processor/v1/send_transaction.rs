...
pub async fn send_batched_transactions<T: TransactionBuilder + Send + Sync + 'static, R: Rpc>(
    ...
    // // Process chunks concurrently with a producer-consumer pattern
    // let (tx_sender, mut tx_receiver) = tokio::sync::mpsc::channel::<Vec<Transaction>>(10);
    //
    // // Producer task: build transactions
    // let producer_handle = {
    //     let operation_cancel_signal_clone = Arc::clone(&operation_cancel_signal);
    //     let timeout_deadline = data.timeout_deadline;
    //     let tree_merkle_tree = tree_accounts.merkle_tree;
    //     let transaction_builder = Arc::clone(&transaction_builder);
    //     let payer_clone = payer.insecure_clone();
    //     let derivation_clone = *derivation;
    //     let recent_blockhash = data.recent_blockhash;
    //     let last_valid_block_height = data.last_valid_block_height;
    //     let priority_fee = data.priority_fee;
    //     let build_config = config.build_transaction_batch_config;
    //
    //     tokio::spawn(async move {
    //         for work_chunk in data
    //             .work_items
    //             .chunks(build_config.batch_size as usize)
    //         {
    //             if operation_cancel_signal_clone.load(Ordering::SeqCst) {
    //                 trace!(tree = %tree_merkle_tree, "Producer: Global cancellation signal received, stopping batch processing.");
    //                 break;
    //             }
    //             if Instant::now() >= timeout_deadline {
    //                 trace!(tree = %tree_merkle_tree, "Producer: Reached global timeout deadline before processing next chunk, stopping.");
    //                 break;
    //             }
    //
    //             trace!(tree = %tree_merkle_tree, "Producer: Processing chunk of size {}", work_chunk.len());
    ...
    //                 Ok(res) => res,
    //                 Err(e) => {
    //                     error!(tree = %tree_merkle_tree, "Producer: Failed to build transaction batch: {:?}", e);
    //                     operation_cancel_signal_clone.store(true, Ordering::SeqCst);
    //                     break;
    //                 }
    //             };
    //             trace!(tree = %tree_merkle_tree, "Producer: Built {} transactions in {:?}", transactions_to_send.len(), build_start_time.elapsed());
    //
    //             if Instant::now() >= timeout_deadline {
    //                 trace!(tree = %tree_merkle_tree, "Producer: Reached global timeout deadline after building transactions, stopping.");
    //                 break;
    //             }
    //
    //             if transactions_to_send.is_empty() {
    //                 trace!(tree = %tree_merkle_tree, "Producer: Built batch resulted in 0 transactions, skipping send for this chunk.");
    //                 continue;
    //             }
    //
    //             // Send transactions to consumer
    //             if tx_sender.send(transactions_to_send).await.is_err() {
    //                 trace!(tree = %tree_merkle_tree, "Producer: Consumer dropped, stopping production.");
    //                 break;
    //             }
    //         }
    //     })
    // };
    //
    // // Consumer task: send transactions
    // let consumer_handle = {
    ...
    //
    //     tokio::spawn(async move {
    //         while let Some(transactions_to_send) = tx_receiver.recv().await {
    //             execute_transaction_chunk_sending(
    //                 transactions_to_send,
    //                 Arc::clone(&pool_clone),
    //                 max_concurrent_sends,
    //                 timeout_deadline,
    //                 Arc::clone(&operation_cancel_signal_clone),
    //                 Arc::clone(&num_sent_transactions_clone),
    //             )
    //             .await;
    //         }
    //     })
