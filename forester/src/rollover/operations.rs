...
use anchor_lang::{AccountDeserialize, InstructionData, system_program, ToAccountMetas};
...
use account_compression::{AddressMerkleTreeAccount, AddressMerkleTreeConfig, AddressQueueConfig, NullifierQueueConfig, QueueAccount, StateMerkleTreeConfig};
use account_compression::utils::constants::{STATE_MERKLE_TREE_CANOPY_DEPTH, STATE_MERKLE_TREE_HEIGHT};
use light_hasher::Poseidon;
use light_merkle_tree_reference::MerkleTree;
use light_registry::sdk::{create_rollover_address_merkle_tree_instruction, create_rollover_state_merkle_tree_instruction, CreateRolloverMerkleTreeInstructionInputs};
use light_test_utils::{create_account_instruction, get_indexed_merkle_tree};
use light_test_utils::address_merkle_tree_config::{get_address_bundle_config, get_state_bundle_config};
use light_test_utils::indexer::{AddressMerkleTreeAccounts, Indexer, StateMerkleTreeAccounts, StateMerkleTreeBundle};
...
pub async fn is_tree_ready_for_rollover<R: RpcConnection>(
    ...
    let account = rpc.get_account(tree_pubkey).await?.unwrap();
    let merkle_tree_metadata = AddressMerkleTreeAccount::try_deserialize(&mut account.data.as_ref())?;

    let is_already_rolled_over = merkle_tree_metadata.metadata.rollover_metadata.rolledover_slot != u64::MAX;
    ...
    let height = 26;
    let threshold = ((1 << height) * merkle_tree_metadata.metadata.rollover_metadata.rollover_threshold / 100) as usize;
...
pub async fn create_rollover_state_merkle_tree_instructions<R: RpcConnection>(
    ...
    let (state_merkle_tree_rent_exemption, queue_rent_exemption) =
        get_rent_exemption_for_state_merkle_tree_and_queue(rpc.deref_mut(), &merkle_tree_config, &queue_config)
            .await;
    let create_nullifier_queue_instruction = create_account_instruction(
        authority,
        queue_rent_exemption.size,
