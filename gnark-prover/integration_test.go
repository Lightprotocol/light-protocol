...
func TestParsingEmptyTreeWithOneLeaf(t *testing.T) {
	testInput := `
	{"roots":["0xeb3a12b56841d019b26e4452bf7db8cf474a2d63e21781c5d2f143db039924a"],
	"inPathIndices":[0],
	"inPathElements":[
		["0x0",
	...
	"leaves":["0x29176100eaa962bdc1fe6c654d6a3c130e96a4d1168b33848b897dc502820133"]
	}`
	...
	if len(tree.Roots) != len(proofData.Roots) ||
		len(tree.Leaves) != len(proofData.Leaves) ||
		len(tree.InPathElements) != len(proofData.InPathElements) ||
		len(tree.InPathIndices) != len(proofData.InPathIndices) {
		t.Errorf("Invalid shape: expected %d, got %d", len(tree.Roots), len(proofData.Roots))
	}

	for i, bi := range tree.Roots {
		if bi.String() != proofData.Roots[i].String() {
			t.Errorf("Invalid root: expected %s, got %s", bi.String(), proofData.Roots[i].String())
		}
	}

	for i, bi := range tree.Leaves {
		if bi.String() != proofData.Leaves[i].String() {
			t.Errorf("Invalid leaf: expected %s, got %s", bi.String(), proofData.Leaves[i].String())
		}
	}
