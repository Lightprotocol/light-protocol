...
func TestWrongMethod(t *testing.T) {
	response, err := http.Get("http://" + ProverAddress + "/prove")
	if err != nil {
		t.Fatal(err)
	}
...
func TestInclusionHappyPath26_1(t *testing.T) {
	...
	response, err := http.Post("http://"+ProverAddress+"/prove", "application/json", strings.NewReader(jsonString))
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	response, err := http.Post("http://"+ProverAddress+"/prove", "application/json", strings.NewReader(testInput))
	if err != nil {
		t.Fatal(err)
	}
	if response.StatusCode != http.StatusOK {
