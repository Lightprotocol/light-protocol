...
#[derive(Debug, Error)]
pub enum AddressStagingTreeError {
    #[error(
        "Sparse tree root mismatch: computed {computed:?}[..4] != expected {expected:?}[..4] (start_index={start_index})"
    )]
    SparseRootMismatch {
        computed: [u8; 32],
        expected: [u8; 32],
    ...
    #[error("Failed to build circuit inputs: {source} (next_index={next_index}, epoch={epoch}, tree={tree})")]
    CircuitInputs {
        source: light_prover_client::errors::ProverClientError,
        next_index: usize,
        epoch: u64,
        tree: String,
    },

    #[error("Subtrees are required for address staging tree")]
    MissingSubtrees,

    #[error("Failed to serialize new root: {0}")]
    RootSerialization(String),
}
...
impl AddressStagingTree {
    pub fn new(
        ...
        if computed_root != initial_root {
            return Err(AddressStagingTreeError::SparseRootMismatch {
    ...
    pub fn from_nodes(
        ...
        match subtrees {
            ...
            None => Err(AddressStagingTreeError::MissingSubtrees.into()),
        }
    }
    ...
    pub fn process_batch(
        ...
        .map_err(|e| AddressStagingTreeError::CircuitInputs {
            source: e,
            next_index,
            epoch,
            tree: tree.to_string(),
        ...
        let new_root = bigint_to_be_bytes_array::<32>(&inputs.new_root)
            .map_err(|e| AddressStagingTreeError::RootSerialization(e.to_string()))?;
