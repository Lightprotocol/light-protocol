...
exports.createVerifyingkeyRsFile = exports.createVerifyingkeyRsFileArgv =
  void 0;
 ...
 */
function getProofInputsFromSymFile(artifactPath, suffix = "") {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    function uniqueMaxSize(arr) {
      const uniqueArr = arr.reduce((acc, cur) => {
        ...
        const idx = acc.findIndex(
          (obj) => obj.inputName === inputName && obj.sumSize < sumSize
        );
        if (idx === -1) {
    ...
    keys.map((name) => {
      ...
      const inputName =
        dimension === 0 ? name : name.slice(0, name.indexOf("["));
      const size =
        dimension === 0
          ? [0]
          : (name.match(/\[(.*?)\]/g) || [])
    ...
    const r1cs = yield snarkjs.r1cs.exportJson(`${artifactPath}${suffix}.r1cs`);
...
function createStringRsIdlAccountStruct(
  ...
  circuitName,
  prefix = "ZK",
  isInstructionData = false
) {
  function camelToSnakeCase(str) {
    return str.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
  }
  function buildRustType(dimension, size, isInstructionData = false) {
...
function createVerifyingKeyRsFile(
  vKeyJsonPath,
  paths,
  appendingString,
  circuitName
  ...
  return __awaiter(this, void 0, void 0, function* () {
    yield fs.readFile(vKeyJsonPath, function (err, fd) {
      return __awaiter(this, void 0, void 0, function* () {
        if (err) {
        ...
        for (var path of paths) {
          let resFile = yield fs.openSync(path, "w");
          let s = `use groth16_solana::groth16::Groth16Verifyingkey;\nuse anchor_lang::prelude::*;\n\npub const VERIFYINGKEY_${(0,
          zk_js_1.camelToScreamingSnake)(
            circuitName
          ...
          (0, child_process_1.execSync)(`rustfmt ${path}`);
        }
      });
    });
  });
}
function createVerifyingkeyRsFileArgv() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (nrInputs == "app") {
      ...
      artifactPath =
        "./sdk/build-circuit/" + (0, zk_js_1.toCamelCase)(circuitName);
    } else {
      if (nrInputs == "2") {
    ...
    yield createVerifyingkeyRsFile(
      program,
      paths,
      vKeyJsonPath,
      vKeyRsPath,
...
exports.createVerifyingkeyRsFileArgv = createVerifyingkeyRsFileArgv;
function createVerifyingkeyRsFile(
  _program,
  paths,
  vKeyJsonPath,
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const ProofInputs = yield getProofInputsFromSymFile(artifactPath, suffix);
...
exports.createVerifyingkeyRsFile = createVerifyingkeyRsFile;
