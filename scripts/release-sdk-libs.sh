...
# Bump versions for sdk-libs crates and create PR
# Usage: ./scripts/release-sdk-libs.sh
#
...
echo "Analyzing changes for sdk-libs crates..."
echo ""
...
if [ ${#CRATES_TO_RELEASE[@]} -eq 0 ]; then
    echo "No sdk-libs crates have changes. Nothing to release."
    exit 0
fi

echo "========================================="
echo "SDK Libs Release Plan:"
echo "========================================="
...
BRANCH_NAME="release/sdk-libs-${TIMESTAMP}"
...
# Commit changes
git add -A
git commit -m "chore(sdk-libs): bump versions"
...
# Create PR body
PR_BODY="## SDK Libs Release

This PR bumps versions for sdk-libs crates based on conventional commits.
...
gh pr create \
  --title "chore(sdk-libs): Bump versions" \
  --body "$PR_BODY" \
  --label "release"
