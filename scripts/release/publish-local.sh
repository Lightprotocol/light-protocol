...
echo ""
echo "========================================="
echo "Phase 4: Creating bump versions PR"
echo "========================================="
echo ""

read -p "Create a bump versions PR? (y/N) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Skipping bump versions PR."
    echo ""
    echo "========================================="
...
# Build version changes summary for PR body
VERSION_CHANGES=""
for pkg in "${PACKAGES[@]}"; do
    VERSION=$(cargo metadata --format-version 1 --no-deps | jq -r ".packages[] | select(.name == \"$pkg\") | .version")
    VERSION_CHANGES="${VERSION_CHANGES}- ${pkg}: ${VERSION}\n"
done

BRANCH_NAME="chore/bump-versions"
