...
# Usage: ./scripts/release/create-release-pr.sh [target-branch]
# Arguments:
...
get_version_changes() {
    ...
    local grep_pattern='(program-libs|sdk-libs|prover/client|sparse-merkle-tree)/'
...
BRANCH_NAME="release/libs"
PR_TITLE="chore: bump lib versions"
...
# Commit changes
git add -A
git commit -m "chore(libs): bump versions"
...
PR_BODY="## Libs Release

This PR bumps versions for program-libs and sdk-libs crates.
...
---
*Generated by \`scripts/release/create-release-pr.sh\`*"
