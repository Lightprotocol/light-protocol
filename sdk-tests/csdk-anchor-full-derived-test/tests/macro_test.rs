...
async fn test_d8_all() {
    ...
    // Derive PDAs
    let (pda_single, _) = Pubkey::find_program_address(
        &[b"d8_all_single", owner.as_ref()],
        &ctx.program_id,
    );
    let (pda_multi, _) = Pubkey::find_program_address(
        &[b"d8_all_multi", owner.as_ref()],
        &ctx.program_id,
    );
...
async fn test_d9_ctx_account() {
    ...
    let (pda, _) = Pubkey::find_program_address(
        &[b"d9_ctx", authority.pubkey().as_ref()],
        &ctx.program_id,
    );
