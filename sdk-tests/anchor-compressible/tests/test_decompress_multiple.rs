...
async fn test_create_and_decompress_two_accounts() {
    ...
    let (compressed_token_account_address, _) = anchor_compressible::get_ctoken_signer_seeds(
        &combined_user.pubkey(),
...
async fn decompress_multiple_pdas_with_ctoken(
    ...
    let rpc_result = rpc
        .get_validity_proof(
            vec![
                c_user_pda.hash,
                c_game_pda.hash,
                compressed_token_account.account.hash,
            ],
            vec![],
            None,
        )
    ...
    let instruction =
        light_compressible_client::CompressibleInstruction::decompress_accounts_idempotent_with_ctoken(
            ...
            &[compressed_token_account.token.owner],
            &[(compressed_token_account, anchor_compressible::CTokenAccountVariant::CTokenSigner)],
            rpc_result,
            output_state_tree_info,
        )
        .unwrap();
    ...
    println!(
        "   - GameSession: {} type",
        decompressed_game_session.game_type
    );
    println!(
        "   - Token Account: {} bytes",
        token_account_data.data.len()
    );
}
...
async fn create_user_record_and_game_session(
    ...
    println!(
        "Compressed token account: mint={}, owner={}, amount={}",
        compressed_token_account.token.mint,
        compressed_token_account.token.owner,
