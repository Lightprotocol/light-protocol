...
async fn compress_record(
    ...
    let instruction = CompressibleInstruction::compress_accounts_idempotent(
        ...
        &[*user_record_pda],
        &[account],
        vec![anchor_compressible::get_user_record_seeds(&payer.pubkey()).0], // compressed_account
...
async fn compress_placeholder_record(
    ...
    let placeholder_seeds = anchor_compressible::get_placeholder_record_seeds(placeholder_id);
...
async fn compress_token_account_after_decompress(
    ...
    let (user_record_seeds, user_record_pubkey) =
        anchor_compressible::get_user_record_seeds(&user.pubkey());
    let (game_session_seeds, game_session_pubkey) =
        anchor_compressible::get_game_session_seeds(session_id);
