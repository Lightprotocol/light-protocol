...
use anchor_compressible_derived::{
    get_ctokensigner2_seeds, CTokenAccountVariant, CompressedAccountVariant,
};
use anchor_lang::{
...
async fn test_create_empty_compressed_account() {
    ...
    let decompressed_placeholder_record =
        PlaceholderRecord::try_deserialize(&mut &placeholder_data[..]).unwrap();
...
async fn create_game_session(
    ...
    let accounts = anchor_compressible_derived::accounts::CreateGameSession {
        player: payer.pubkey(),
    ...
    // Create instruction data
    let instruction_data = anchor_compressible_derived::instruction::CreateGameSession {
        session_id,
...
async fn decompress_multiple_pdas_with_ctoken(
    ...
    let decompressed_game_session = GameSession::try_deserialize(&mut &game_pda_data[..]).unwrap();
...
async fn create_user_record_and_game_session(
    ...
    let compressed_token_accounts_2 = rpc
...
async fn compress_record(
    ...
    let instruction = CompressibleInstruction::compress_accounts_idempotent(
        ...
        &[*user_record_pda],
        &[account],
        &anchor_compressible_derived::accounts::CompressAccountsIdempotent {
            fee_payer: payer.pubkey(),
            config: CompressibleConfig::derive_pda(&program_id, 0).0,
            rent_recipient: RENT_RECIPIENT,
...
async fn create_placeholder_record(
    ...
    // Create the instruction
    let accounts = anchor_compressible_derived::accounts::CreatePlaceholderRecord {
        user: payer.pubkey(),
    ...
    // Create instruction data
    let instruction_data = anchor_compressible_derived::instruction::CreatePlaceholderRecord {
        placeholder_id,
        name: name.to_string(),
        proof: rpc_result.proof,
        compressed_address,
...
async fn compress_game_session_with_custom_data(
    ...
    let original_game_session = GameSession::try_deserialize(&mut &game_pda_data[..]).unwrap();
...
async fn compress_token_account_after_decompress(
    ...
    let (_token_account_seeds, token_account_address) =
        anchor_compressible_derived::get_ctokensigner_seeds(&user.pubkey(), &mint);
    // For compression, use the generated authority seeds function
    let (token_authority_seeds, _authority_pda) =
        anchor_compressible_derived::get_ctokensigner_authority_seeds();
    ...
    let (token_authority_seeds_2, _authority_pda_2) =
        anchor_compressible_derived::get_ctokensigner2_authority_seeds();
    println!("012 token_account_address_2: {:?}", token_account_address_2);
