//! Tests for the transfer action in light-token-client.

use borsh::BorshDeserialize;
use light_client::rpc::Rpc;
use light_program_test::{LightProgramTest, ProgramTestConfig};
use light_token::instruction::derive_token_ata;
use light_token_client::actions::{CreateAta, CreateMint, MintTo, Transfer};
...
/// Test transferring tokens between Light Token accounts.
#[tokio::test]
...
async fn test_transfer_full_balance() {
    ...
    // Verify source account (zero balance)
    let source_data = rpc.get_account(source_ata).await.unwrap().unwrap();
    let source_state = Token::deserialize(&mut &source_data.data[..]).unwrap();
    let expected_source = get_expected_token(&source_state, mint, source_owner, 0, None, 0);
    assert_eq!(source_state, expected_source);

    // Verify destination account
    let dest_data = rpc.get_account(dest_ata).await.unwrap().unwrap();
    let dest_state = Token::deserialize(&mut &dest_data.data[..]).unwrap();
    let expected_dest = get_expected_token(&dest_state, mint, dest_owner, mint_amount, None, 0);
    assert_eq!(dest_state, expected_dest);
}
