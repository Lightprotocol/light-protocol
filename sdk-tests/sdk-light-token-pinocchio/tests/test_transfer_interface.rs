...
use sdk_light_token_pinocchio_test::{TransferInterfaceData, TRANSFER_INTERFACE_AUTHORITY_SEED};
use shared::PROGRAM_ID;
use solana_sdk::{
...
async fn test_transfer_interface_ctoken_to_ctoken_invoke() {
    ...
    rpc.create_and_send_transaction(&[instruction], &payer.pubkey(), &[&payer])
        .await
        .unwrap();
    let sender_ctoken = get_associated_token_address(&sender.pubkey(), &mint);
    ...
    rpc.create_and_send_transaction(&[instruction], &payer.pubkey(), &[&payer])
        .await
        .unwrap();
    let recipient_ctoken = get_associated_token_address(&recipient.pubkey(), &mint);
    ...
    let wrapper_accounts = vec![
        ...
        AccountMeta::new(sender.pubkey(), true), // authority (writable, signer)
        AccountMeta::new(payer.pubkey(), true), // payer
        AccountMeta::new_readonly(cpi_authority_pda, false),
        AccountMeta::new_readonly(Pubkey::default(), false), // system_program
    ];
...
async fn test_transfer_interface_spl_to_ctoken_invoke_signed() {
    ...
    let (authority_pda, _) =
        Pubkey::find_program_address(&[TRANSFER_INTERFACE_AUTHORITY_SEED], &PROGRAM_ID);
...
async fn test_transfer_interface_ctoken_to_ctoken_invoke_signed() {
    ...
    let source_ctoken = get_associated_token_address(&authority_pda, &mint);
    let instruction = CreateAssociatedTokenAccount {
        idempotent: false,
        payer: payer.pubkey(),
        owner: authority_pda,
    ...
    rpc.create_and_send_transaction(&[instruction], &payer.pubkey(), &[&payer])
        .await
        .unwrap();
    let dest_ctoken = get_associated_token_address(&recipient.pubkey(), &mint);
