...
/// Handler for freezing a Light Token account (invoke)
///
/// Account order:
...
/// Handler for freezing a Light Token account with PDA freeze authority (invoke_signed)
///
/// Account order:
/// - accounts[0]: token_account (writable)
/// - accounts[1]: mint
...
pub fn process_freeze_invoke_signed(accounts: &[AccountInfo]) -> Result<(), ProgramError> {
    ...
    let (pda, bump) = Address::find_program_address(&[FREEZE_AUTHORITY_SEED], &Address::from(ID));
