...
impl<'info> LightPreInit<'info, CreateDerivedMintsParams> for CreateDerivedMintsAccounts<'info> {
    fn light_pre_init(
        &mut self,
...
impl<'info> LightFinalize<'info, CreateDerivedMintsParams> for CreateDerivedMintsAccounts<'info> {
    fn light_finalize(
        &mut self,
