...
const artifiactPath = "./sdk/build-circuit/appTransaction";
const circuitName = "appTransaction";
/**
...
function parseAndAppendRustStruct(preparedInputs, circuitName, outPath) {
  ...
  fs.appendFile(outPath, `\n${structDefinition}`, (err) => {
    if (err) {
      console.error(
        "Failed to append the Rust code to VerifyingKey file!:",
        err,
      );
    } else {
...
async function main() {
  ...
  let file = await fs.readFile("./verifyingkey.json", async function (err, fd) {
    if (err) {
    ...
    let vKeyRsPath = "./programs/" + program + "/src/verifying_key.rs";
    console.log(vKeyRsPath);
    let resFile = await fs.openSync(vKeyRsPath, "w");
    ...
    parseAndAppendRustStruct(ProofInputs, circuitName + "Proof", vKeyRsPath);
    parseAndAppendRustStruct(PublicInputs, circuitName + "Public", vKeyRsPath);
