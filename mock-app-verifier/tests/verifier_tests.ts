...
describe("Verifier Two test", () => {
        ...
        for (var verifier in verifiers) {
            ...
            await token.approve(
                ...
                Transaction.getSignerAuthorityPda(
                    merkleTreeProgramId, verifiers[verifier].verifierProgram.programId
                ), //delegate
            ...
            let lightInstanceWithdrawal: LightInstance = {
                solMerkleTree: await SolMerkleTree.build({pubkey: MERKLE_TREE_KEY, poseidon: POSEIDON}),
                lookUpTable: LOOK_UP_TABLE,
                provider,
            };
    ...
    afterEach(async () => {     
        ...
        for (var tx in transactions) {
            await checkNfInserted(transactions[tx].params.nullifierPdaPubkeys, provider.connection);
        }
    })
    ...
    const sendTestTx = async (tx: Transaction, type: string, account?: string) => {
        ...
        if(type === "ProofVerificationFails") {
            assert.isTrue(e.logs.includes("Program log: error ProofVerificationFailed"))
        } else if (type === "Account") {
            assert.isTrue(e.logs.includes(`Program log: AnchorError caused by account: ${account}. Error Code: ConstraintSeeds. Error Number: 2006. Error Message: A seeds constraint was violated.`))
    ...
    it.skip("Wrong signer",async () => {
        for (var tx in transactions) {
            ...
            await provider.connection.confirmTransaction(await provider.connection.requestAirdrop(wrongSinger.publicKey, 1_000_000_000), "confirmed");
    ...
    it("Wrong registeredVerifierPda",async () => {
        for (var tx in transactions) {
            ...
            if (
                ...
                tmp_tx.params.accounts.registeredVerifierPda = REGISTERED_VERIFIER_ONE_PDA;
            } 
    ...
    it("Wrong authority",async () => {
        for (var tx in transactions) {
            ...
            tmp_tx.params.accounts.authority = Transaction.getSignerAuthorityPda(
                merkleTreeProgramId, SolanaKeypair.generate().publicKey
            )
    ...
    it("Wrong nullifier accounts",async () => {
        for (var tx in transactions) {
            ...
            for (var i = 0; i < tmp_tx.params.nullifierPdaPubkeys.length; i++) {
                tmp_tx.params.nullifierPdaPubkeys[i] = tmp_tx.params.nullifierPdaPubkeys[
                    (i + 1) % tmp_tx.params.nullifierPdaPubkeys.length
                ];
                await sendTestTx(tmp_tx, "Includes", "Program log: Passed-in pda pubkey != on-chain derived pda pubkey."); 
            } 
        }
    })
        ...
        for (var tx in transactions) {
            ...
            if(tmp_tx.params.leavesPdaPubkeys.length > 1) {
                for (var i = 0; i < tmp_tx.params.leavesPdaPubkeys.length; i++) {
                    tmp_tx.params.leavesPdaPubkeys[i] = tmp_tx.params.leavesPdaPubkeys[
                        (i + 1) % tmp_tx.params.leavesPdaPubkeys.length
                    ];
                    await sendTestTx(tmp_tx, "Includes", 'Program log: Instruction: InsertTwoLeaves'); 
                } 
            } else {
                tmp_tx.params.leavesPdaPubkeys[0] = {isSigner: false, isWritable: true, pubkey: SolanaKeypair.generate().publicKey};
                await sendTestTx(tmp_tx, "Includes",'Program JA5cjkRJ1euVi9xLWsCJVzsRzEkT8vcC4rqw9sVAo5d6 failed: Cross-program invocation with unauthorized signer or writable account'); 
            }
        }
    })
});